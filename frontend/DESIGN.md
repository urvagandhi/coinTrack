...
> Clarity  
> **Scope**: Component Primitives (`src/components/ui`), Public Surface (`src/app/(public)`), Design
> Lab Sandbox, and Motion Tokens (`system.css`).  
...
The production landing page (`src/app/(public)/landing/page.jsx`) exemplifies this architecture in
its pure form:
...
Located in `src/components/ui/primitives/`:
...
- Integrated currency switcher (`INR`, `USD`, `EUR`) and quick-add chips (`+₹1,000`, `+₹25,000`,
  `+₹1,00,000`).
