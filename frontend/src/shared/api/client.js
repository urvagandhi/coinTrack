...
export const tokenManager = {
  ...
  removeToken: () => {
    ...
    try {
      ...
      logger.warn('Failed to clear auth cookie:', e);
    }
  },
...
api.interceptors.response.use(
  ...
  async error => {
    ...
    if (
      ...
      originalRequest &&
      !originalRequest.__isRefreshRetry
    ) {
...
export const endpoints = {
  ...
  mutualFund: {
    ...
    sipContributionMandate: id =>
      `/api/mutual-fund/sip-contribution/mandate/${id}`,
    sipContributionDateRange: '/api/mutual-fund/sip-contribution/date-range',
    sipContributionFinYear: year =>
      `/api/mutual-fund/sip-contribution/financial-year/${year}`,
    ...
    redemptionFinYear: year =>
      `/api/mutual-fund/redemption/financial-year/${year}`,
...
export const authAPI = {
  ...
  completeGoogleProfile: async payload => {
    const { data } = await api.post(
      '/api/auth/oauth2/complete-profile',
      payload,
      noRetry
    );
    return unwrapResponse(data);
...
export const userAPI = {
  ...
  deleteAccount: async password => {
    const { data } = await api.delete(endpoints.users.deleteMe, {
      data: { password },
    });
    return unwrapResponse(data);
  },
...
export const brokerAPI = {
  ...
  saveZerodhaCredentials: async creds => {
    const { data } = await api.post(
      endpoints.brokers.zerodha.saveCredentials,
      creds
    );
    return unwrapResponse(data);
  },
  ...
  saveUpstoxCredentials: async creds => {
    const { data } = await api.post(
      endpoints.brokers.upstox.saveCredentials,
      creds
    );
    return unwrapResponse(data);
  },
  ...
  saveAngelOneCredentials: async creds => {
    const { data } = await api.post(
      endpoints.brokers.angelone.saveCredentials,
      creds
    );
    return unwrapResponse(data);
  },
...
export const twofaAPI = {
  ...
  verifyRecovery: async token => {
    const { data } = await api.post(
      endpoints.twofa.recoveryVerify,
      { token },
      noRetry
    );
    return unwrapResponse(data);
...
export const passwordAPI = {
  ...
  forgotVerify: async token => {
    const { data } = await api.post(
      endpoints.password.forgotVerify,
      { token },
      noRetry
    );
    return unwrapResponse(data);
...
export const totpAPI = {
  ...
  loginRecovery: async (tempToken, code) => {
    const { data } = await api.post(
      endpoints.auth.totp.loginRecovery,
      { tempToken, code },
      noRetry
    );
    return unwrapResponse(data);
  ...
  initiateReset: async currentCode => {
    const { data } = await api.post(
      endpoints.auth.totp.initiateReset,
      { code: currentCode },
      noRetry
    );
    return unwrapResponse(data);
  ...
  verifyReset: async code => {
    const { data } = await api.post(
      endpoints.auth.totp.verifyReset,
      { code },
      noRetry
    );
    return unwrapResponse(data);
  ...
  registerSetup: async tempToken => {
    const { data } = await api.post(
      endpoints.auth.totp.registerSetup,
      { tempToken },
      noRetry
    );
    return unwrapResponse(data);
  ...
  registerVerify: async (tempToken, code) => {
    const { data } = await api.post(
      endpoints.auth.totp.registerVerify,
      { tempToken, code },
      noRetry
    );
    return unwrapResponse(data);
...
export const goldSilverAPI = {
  getAll: async (params = {}) => {
    ...
    const qs = searchParams.toString();
    const { data } = await api.get(
      `${endpoints.goldSilver.list}${qs ? `?${qs}` : ''}`
    );
    return unwrapResponse(data) || {};
  },
  getSummary: async () => {
  ...
  updateRateSettings: async settingsData => {
    const { data } = await api.put(
      endpoints.goldSilver.rateSettings,
      settingsData
    );
    return unwrapResponse(data);
  },
  updateRateMode: async (id, payload) => {
    const { data } = await api.patch(
      endpoints.goldSilver.rateMode(id),
      payload
    );
    return unwrapResponse(data);
  },
  getPurityOptions: async metalType => {
    ...
    const { data } = await api.get(
      `${endpoints.goldSilver.purityOptions}${qs}`
    );
    return unwrapResponse(data) || [];
  },
  exportCSV: async (params = {}) => {
    ...
    const qs = searchParams.toString();
    const response = await api.get(
      `${endpoints.goldSilver.export}${qs ? `?${qs}` : ''}`,
      {
        responseType: 'blob',
      }
    );
...
export const mutualFundAPI = {
  ...
  getSchemes: async (params = {}) => {
    const searchParams = new URLSearchParams();
    if (params.includeRedeemed)
      searchParams.set('includeRedeemed', params.includeRedeemed);
    if (params.holderName) searchParams.set('holderName', params.holderName);
    const qs = searchParams.toString();
    const { data } = await api.get(
      `${endpoints.mutualFund.schemes}${qs ? `?${qs}` : ''}`
    );
    return unwrapResponse(data) || [];
  },
  getSchemeSummaries: async (params = {}) => {
    ...
    const qs = searchParams.toString();
    const { data } = await api.get(
      `${endpoints.mutualFund.schemeSummary}${qs ? `?${qs}` : ''}`
    );
    return unwrapResponse(data) || [];
  },
  ...
  updateScheme: async (id, schemeData) => {
    const { data } = await api.put(
      endpoints.mutualFund.updateScheme(id),
      schemeData
    );
    return unwrapResponse(data);
  },
  ...
  getSchemeNavForDate: async (id, date, isAfterCutoff = false) => {
    const { data } = await api.get(
      `${endpoints.mutualFund.schemeNavForDate(id)}?date=${date}&isAfterCutoff=${isAfterCutoff}`
    );
    return unwrapResponse(data);
  },
  ...
  getLumpsum: async (params = {}) => {
    ...
    const qs = searchParams.toString();
    const { data } = await api.get(
      `${endpoints.mutualFund.lumpsum}${qs ? `?${qs}` : ''}`
    );
    return unwrapResponse(data) || [];
  },
  getLumpsumPaginated: async (page = 0, size = 10) => {
    const { data } = await api.get(
      `${endpoints.mutualFund.lumpsumPage}?page=${page}&size=${size}`
    );
    return unwrapResponse(data);
  },
  ...
  updateLumpsum: async (id, txnData) => {
    const { data } = await api.put(
      endpoints.mutualFund.updateLumpsum(id),
      txnData
    );
    return unwrapResponse(data);
  },
  ...
  getSipMandates: async (params = {}) => {
    ...
    const qs = searchParams.toString();
    const { data } = await api.get(
      `${endpoints.mutualFund.sipMandate}${qs ? `?${qs}` : ''}`
    );
    return unwrapResponse(data) || [];
  },
  getSipMandatesByStatus: async status => {
    const { data } = await api.get(
      endpoints.mutualFund.sipMandateStatus(status)
    );
    return unwrapResponse(data) || [];
  },
  createSipMandate: async mandateData => {
    const { data } = await api.post(
      endpoints.mutualFund.sipMandate,
      mandateData
    );
    return unwrapResponse(data);
  },
  updateSipMandate: async (id, mandateData) => {
    const { data } = await api.put(
      endpoints.mutualFund.updateSipMandate(id),
      mandateData
    );
    return unwrapResponse(data);
  },
  stopSipMandate: async ({ id, date }) => {
    const { data } = await api.patch(endpoints.mutualFund.stopSipMandate(id), {
      date,
    });
    return unwrapResponse(data);
  },
  restartSipMandate: async ({ id, date }) => {
    const { data } = await api.patch(
      endpoints.mutualFund.restartSipMandate(id),
      { date }
    );
    return unwrapResponse(data);
  },
  deleteSipMandate: async id => {
    const { data } = await api.delete(
      endpoints.mutualFund.deleteSipMandate(id)
    );
    return unwrapResponse(data);
  },
  ...
  getSipContributions: async schemeId => {
    ...
    const { data } = await api.get(
      `${endpoints.mutualFund.sipContribution}${qs}`
    );
    return unwrapResponse(data) || [];
  },
  createSipContribution: async contributionData => {
    const { data } = await api.post(
      endpoints.mutualFund.sipContribution,
      contributionData
    );
    return unwrapResponse(data);
  },
  updateSipContribution: async (id, contributionData) => {
    const { data } = await api.put(
      endpoints.mutualFund.updateSipContribution(id),
      contributionData
    );
    return unwrapResponse(data);
  },
  deleteSipContribution: async id => {
    const { data } = await api.delete(
      endpoints.mutualFund.deleteSipContribution(id)
    );
    return unwrapResponse(data);
  },
  ...
  previewFifo: async ({ schemeId, date, units }) => {
    ...
    const { data } = await api.get(
      `${endpoints.mutualFund.redemptionPreviewFifo}?${searchParams.toString()}`
    );
    return unwrapResponse(data);
  },
  ...
  updateRedemption: async (id, txnData) => {
    const { data } = await api.put(
      endpoints.mutualFund.updateRedemption(id),
      txnData
    );
    return unwrapResponse(data);
  },
  deleteRedemption: async id => {
    const { data } = await api.delete(
      endpoints.mutualFund.deleteRedemption(id)
    );
    return unwrapResponse(data);
  },
  ...
  updateValuation: async (id, payload) => {
    const { data } = await api.put(
      endpoints.mutualFund.updateValuation(id),
      payload
    );
    return unwrapResponse(data);
  },
