...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const safety = setTimeout(() => {
      setIsInitializing(v => {
        if (v)
          logger.warn('Auth init timed out — releasing isInitializing gate');
        return false;
      });
    }, 8000);
  ...
  const initializeAuth = async () => {
    ...
    try {
      ...
      logger.warn('Auth initialization failed', {
        error: error.message,
        status: error.status,
      });
  ...
  const login = useCallback(async credentials => {
    ...
    try {
      ...
      if (response.requireTotpSetup) {
        logger.info('Login requires TOTP setup', {
          username: response.username,
        });
        dispatch({ type: AUTH_ACTIONS.SET_LOADING, payload: false });
        return {
  ...
  const googleLogin = useCallback(async (code, redirectUri) => {
    ...
    try {
      ...
      // CASE 1: TOTP required
      if (response.tempToken && !response.requireTotpSetup && !response.token) {
        logger.info('Google Login requires TOTP verification', {
          username: response.username,
        });
        dispatch({ type: AUTH_ACTIONS.SET_LOADING, payload: false });
        return {
  ...
  const logout = useCallback(async () => {
    ...
    try {
      ...
      logger.warn('Server logout failed (non-critical)', {
        error: error.message,
      });
    } finally {
      queryClient.clear();
  ...
  const verifyRecoveryLogin = useCallback(
    async (tempToken, code) => {
      try {
        ...
        return {
          success: false,
          error: error.message || 'Invalid recovery code',
        };
      }
    },
    [handleTotpLoginSuccess]
  ...
  const resetTotp = useCallback(async currentCode => {
    try {
      ...
      return {
        success: false,
        error: error.message || 'Reset initiation failed',
      };
    }
  }, []);

  const verifyResetTotp = useCallback(
    async code => {
      try {
        ...
        return {
          success: false,
          error: error.message || 'Reset verification failed',
        };
      }
    },
    [fetchUserProfile]
