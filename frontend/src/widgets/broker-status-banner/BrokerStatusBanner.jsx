...
import { brokerAPI } from '@/shared/api/client';
      ...
      {alerts.map(alert => (
        ...
        >
          <div className='flex items-center gap-3'>
            <AlertTriangle
              className='h-4 w-4 text-[hsl(var(--loss))]'
              strokeWidth={2}
            />
            <div>
              <p className='text-[11px] eyebrow text-[hsl(var(--loss))]'>
                Session Expired
              </p>
              <p className='text-[13px] text-foreground mt-0.5'>
                <span className='font-semibold'>
                  {BROKER_NAMES[alert.broker] || alert.broker}
                </span>
