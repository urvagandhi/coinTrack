...
import { useBrokerSummary } from '@/features/brokers';
...
export default function Sidebar({ onNavigate }) {
  ...
  const brokerBadge = (() => {
    if (hasExpired) {
      return {
        text: 'Reset',
        className: 'bg-[hsl(var(--loss))]/15 text-[hsl(var(--loss))]',
    ...
    if (hasExpiringSoon) {
      return {
        ...
        dotColor: 'bg-[hsl(var(--chart-4))]',
      };
    }
    if (connectedCount > 0) {
      return {
        text: `${connectedCount} live`,
        className: 'bg-[hsl(var(--gain))]/15 text-[hsl(var(--gain))]',
        dotColor: 'bg-[hsl(var(--gain))]',
      };
    }
    return null;
  })();
