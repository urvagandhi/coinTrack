...
export default function EpfTransactionDialog({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        const isW =
          !!initialData.withdrawalAmount && initialData.withdrawalAmount > 0;
        setFormData({
          transactionDate: initialData.transactionDate || '',
          mode: initialData.mode || 'AUTO_SALARY',
          isWithdrawal: isW,
          basicDA: initialData.basicDA
            ? String(initialData.basicDA)
            : settings?.defaultBasicDA
              ? String(settings.defaultBasicDA)
              : '',
          employeeContribution: initialData.employeeContribution
            ? String(initialData.employeeContribution)
            : '',
          employerEpfContribution: initialData.employerEpfContribution
            ? String(initialData.employerEpfContribution)
            : '',
          employerEpsContribution: initialData.employerEpsContribution
            ? String(initialData.employerEpsContribution)
            : '',
          vpfAmount: initialData.vpfAmount
            ? String(initialData.vpfAmount)
            : settings?.monthlyVpfAmount
              ? String(settings.monthlyVpfAmount)
              : '',
          withdrawalAmount: initialData.withdrawalAmount
            ? String(initialData.withdrawalAmount)
            : '',
        ...
        setFormData({
          ...
          basicDA: settings?.defaultBasicDA
            ? String(settings.defaultBasicDA)
            : '',
          vpfAmount: settings?.monthlyVpfAmount
            ? String(settings.monthlyVpfAmount)
            : '',
        });
      }
  ...
  if (basicNum > 0) {
    ...
    if (useActualEps) {
      ...
      calcEps =
        basicNum >= 15000
          ? 1250.0
          : Math.round(cappedBasic * 0.0833 * 100) / 100;
    }
    const totalEmployer = Math.round(basicNum * 0.12 * 100) / 100;
    calcEmployerEpf = Math.max(
      0,
      Math.round((totalEmployer - calcEps) * 100) / 100
    );
  }
  ...
  const handleSubmit = async e => {
    ...
    if (formData.isWithdrawal) {
      ...
      if (!basicNum || basicNum <= 0) {
        toast({
          title: 'Validation Error',
          description: 'Basic+DA is required for AUTO_SALARY mode.',
          variant: 'destructive',
        });
        return;
      }
      ...
      if (empC <= 0 && empEpf <= 0 && empEps <= 0 && vpf <= 0) {
        toast({
          title: 'Validation Error',
          description:
            'At least one contribution field must be provided in MANUAL_OVERRIDE mode.',
          variant: 'destructive',
        });
        return;
      }
    ...
    try {
      ...
      if (formData.isWithdrawal) {
        ...
        if (formData.employeeContribution !== '')
          payload.employeeContribution = Number(formData.employeeContribution);
        if (formData.employerEpfContribution !== '')
          payload.employerEpfContribution = Number(
            formData.employerEpfContribution
          );
        if (formData.employerEpsContribution !== '')
          payload.employerEpsContribution = Number(
            formData.employerEpsContribution
          );
        if (formData.vpfAmount !== '')
          payload.vpfAmount = Number(formData.vpfAmount);
      }
        ...
        <div className='flex items-center justify-between p-6 border-b border-border'>
          <div>
            ...
            <p className='text-[12px] text-muted-foreground font-mono uppercase tracking-[0.05em]'>
              {initialData
                ? `Txn #${initialData.transactionNo}`
                : 'Employee Provident Fund Ledger'}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <form id='epf-txn-form' onSubmit={handleSubmit} className='space-y-6'>
            ...
            <div className='flex items-center justify-between p-3 rounded-sm border border-border bg-muted/20'>
              ...
              <div className='flex items-center gap-2'>
                <button
                  type='button'
                  onClick={() =>
                    setFormData({ ...formData, isWithdrawal: false })
                  }
                ...
                <button
                  type='button'
                  onClick={() =>
                    setFormData({ ...formData, isWithdrawal: true })
                  }
                  className={`h-7 px-3 text-[11px] font-mono border rounded-xs transition-colors ${
                    formData.isWithdrawal
                ...
                {!formData.isWithdrawal && (
                  <div className='space-y-1.5'>
                    ...
                    <select
                      ...
                      className='ed-input w-full font-mono'
                    >
                      <option value='AUTO_SALARY'>
                        Auto Salary Split (Statutory Engine)
                      </option>
                      <option value='MANUAL_OVERRIDE'>
                        Manual Entry (Direct Fields)
                      </option>
                    </select>
                  </div>
                )}
            ...
            {formData.isWithdrawal ? (
              <div className='space-y-3'>
                ...
                <div className='space-y-1.5'>
                  <label className='eyebrow text-[hsl(var(--loss))]'>
                    Withdrawal Amount (₹) *
                  </label>
                  <input
                    type='text'
                  ...
                  {formData.withdrawalAmount &&
                    !isNaN(formData.withdrawalAmount) && (
                      <p className='text-[11px] font-mono text-[hsl(var(--loss))]'>
                        {formatIndianCurrency(formData.withdrawalAmount)}{' '}
                        (Deducted from EPF pool)
                      </p>
                    )}
                </div>
              </div>
              ...
              <div className='space-y-4'>
                <h3 className='text-[11px] font-mono uppercase text-muted-foreground tracking-[0.1em] border-b border-border/50 pb-1 flex items-center justify-between'>
                  ...
                  <span className='text-[10px] text-[hsl(var(--accent))] flex items-center gap-1'>
                    <Calculator className='h-3 w-3' /> Statutory Split Engine
                    Active
                  </span>
                </h3>
                    ...
                    <div className='grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-mono pt-1 border-t border-[hsl(var(--accent))]/20'>
                      <div>
                        ...
                        <span className='font-semibold text-foreground'>
                          {formatIndianCurrency(calcEmployee)}
                        </span>
                      </div>
                      <div>
                        <span className='text-muted-foreground block text-[9px] uppercase'>
                        ...
                        <span className='font-semibold text-[hsl(var(--gain))]'>
                          {formatIndianCurrency(calcEmployerEpf)}
                        </span>
                      </div>
                      <div>
                        <span className='text-muted-foreground block text-[9px] uppercase'>
                      ...
                      <div>
                        ...
                        <span className='font-bold text-[hsl(var(--gain))]'>
                          {formatIndianCurrency(calcTotalEpf)}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          employeeContribution: parseShortcutAmount(
                            e.target.value
                          ),
                        })
                      }
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          employerEpfContribution: parseShortcutAmount(
                            e.target.value
                          ),
                        })
                      }
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          employerEpsContribution: parseShortcutAmount(
                            e.target.value
                          ),
                        })
                      }
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          vpfAmount: parseShortcutAmount(e.target.value),
                        })
                      }
                      className='ed-input w-full font-mono'
                      placeholder='e.g. 5000'
