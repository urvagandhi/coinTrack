...
export default function ContactModal() {
  ...
  const dateString = now
    ...
    : '';
  const refNo = now
    ? `LTR-${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}`
    : 'LTR-—';
          ...
          <div className='border-b border-hairline px-6 sm:px-8 pt-5 pb-4'>
            <div className='flex items-start justify-between gap-4'>
              <div className='min-w-0'>
                ...
                <h3 className='font-serif text-[28px] sm:text-[34px] leading-[1.05] tracking-tight text-foreground flex items-baseline gap-3'>
                  <Mail
                    size={20}
                    className='text-[hsl(var(--accent))] translate-y-0.5'
                    aria-hidden='true'
                  />
                  <span>Contact the Desk</span>
                </h3>
                <p className='mt-2 font-serif italic text-[15px] text-muted-foreground leading-snug'>
                  A short note finds us quickest. We read every letter and
                  reply, typically within a working day.
                </p>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className='mt-3 flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 text-[11px]'>
              <p className='eyebrow'>
                Reference ·{' '}
                <span
                  className='font-mono text-foreground normal-case tracking-normal'
            ...
            <div className='space-y-1.5'>
              ...
              <textarea
                ...
                {...register('message', {
                  ...
                  minLength: {
                    value: 10,
                    message: 'A little more — at least ten characters',
                  },
                })}
