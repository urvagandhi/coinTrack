    ...
    <div className='space-y-5'>
      <div className='flex items-end justify-between pb-4 border-b border-border'>
        ...
        <span className='text-[11px] tabular-nums font-mono text-muted-foreground'>
          ...
          {instruments &&
            instruments.length > filtered.length &&
            ` of ${instruments.length}`}
        </span>
      </div>
            ...
            {filtered.map((inst, i) => {
              ...
              return (
                <li key={id}>
                  ...
                  >
                    <span className='index-num tnum w-8'>
                      {String(i + 1).padStart(3, '0')}
                    </span>
                    <div className='flex-1 min-w-0'>
                      <p className='text-[13px] font-medium text-foreground tracking-tight line-clamp-1'>
                        {inst.name}
                      </p>
                      ...
                      <dl className='grid grid-cols-2 sm:grid-cols-4 gap-4 text-[12px]'>
                        ...
                        <div>
                          ...
                          <dd className='font-mono tnum text-foreground'>
                            {inst.lastPrice ? `₹${inst.lastPrice}` : '—'}
                          </dd>
                        </div>
                        <div>
                          <dt className='eyebrow mb-1'>Min Invest</dt>
                          <dd className='font-mono tnum text-foreground'>
                            {inst.minimumPurchaseAmount
                              ? `₹${inst.minimumPurchaseAmount}`
                              : '—'}
                          </dd>
                        </div>
