        ...
        {steps.map((label, i) => (
          <div key={label} className='flex flex-col gap-1.5'>
            <div className='flex items-baseline gap-1.5'>
              <span
                className={`display-num text-[11px] ${i <= stepIndex ? 'text-foreground' : 'text-muted-foreground'}`}
              >
                {String(i + 1).padStart(2, '0')}
              </span>
              {i < stepIndex && (
                <Check size={11} className='text-[hsl(var(--gain))]' />
              )}
            </div>
            <div
              className={`h-[2px] ${i <= stepIndex ? 'bg-foreground' : 'bg-hairline'}`}
            />
            <p
              className={`text-[10px] uppercase tracking-[0.2em] ${i === stepIndex ? 'text-foreground font-medium' : 'text-muted-foreground'}`}
            >
              {label}
            </p>
          </div>
        ...
        <div className='space-y-6'>
          ...
          <div className='space-y-2'>
            ...
            <div className='flex items-stretch gap-0 border border-hairline'>
              ...
              >
                {copied ? (
                  <Check size={15} className='text-[hsl(var(--gain))]' />
                ) : (
                  <Copy size={15} />
                )}
              </button>
      ...
      {step === 'backup' && (
        <div className='space-y-6'>
          <div className='flex items-center gap-2'>
            ...
            <span className='eyebrow text-[hsl(var(--gain))]'>
              Setup complete
            </span>
          </div>

          <div>
            <p className='font-serif text-[22px] text-foreground leading-snug'>
              Save your backup codes
            </p>
            <p className='text-[13px] text-muted-foreground mt-1.5 leading-relaxed'>
              Use these if you lose access to your authenticator app. Each code
              works once.
            </p>
          </div>
          ...
          <div className='border border-hairline bg-muted/30 p-4 grid grid-cols-2 gap-2'>
            {backupCodes.map((code, i) => (
              ...
              >
                <span className='display-num text-[9px] text-muted-foreground'>
                  {String(i + 1).padStart(2, '0')}
                </span>
                <span>{code}</span>
              </div>
