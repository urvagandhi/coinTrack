...
function Skeleton({ className = '' }) {
  return (
    <div className={cn('animate-pulse bg-muted/60 rounded-sm', className)} />
  );
}
      ...
      <section className='ed-card relative px-8 py-14'>
        ...
        <div className='text-center max-w-sm mx-auto'>
          <Briefcase
            ...
            strokeWidth={1.25}
          />
          <p className='font-serif italic text-[22px] text-foreground mb-1'>
            No holdings on record.
          </p>
          <p className='text-[12px] text-muted-foreground'>
            Connect a broker to begin syncing positions.
          </p>
        </div>
      </section>
    );
    ...
    <section className='ed-card relative'>
      ...
      <header className='px-6 py-5 border-b border-hairline flex items-center justify-between'>
        ...
        {holdings.length > 10 && (
          <Link
            href='/portfolio'
            className='ed-link text-[12px] flex items-center gap-1.5'
          >
            View ledger
              ...
              return (
                <tr key={`${pos.symbol}-${pos.broker}`}>
                  ...
                  <td>
                    <span className='inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em] font-medium text-muted-foreground'>
                      <span
                        ...
                        style={{
                          background: BROKER_VAR[pos.broker] || 'currentColor',
                        }}
                      />
                  ...
                  <td className='text-right'>
                    <span
                      className={cn(
                        'block font-mono tnum text-[13px] font-semibold',
                        pnlUp
                          ? 'text-[hsl(var(--gain))]'
                          : 'text-[hsl(var(--loss))]'
                      )}
                    ...
                    <span
                      className={cn(
                        'block font-mono tnum text-[10px]',
                        pnlUp
                          ? 'text-[hsl(var(--gain))]'
                          : 'text-[hsl(var(--loss))]'
                      )}
                  ...
                  <td className='text-right pr-6'>
                    <span
                      className={cn(
                        'inline-flex items-center gap-0.5 font-mono tnum text-[11px] font-medium',
                        dayUp
                          ? 'text-[hsl(var(--gain))]'
                          : 'text-[hsl(var(--loss))]'
                      )}
      ...
      {holdings.length > 10 && (
        <div className='px-6 py-4 border-t border-hairline text-center'>
          <Link
            href='/portfolio'
            className='ed-link text-[12px] inline-flex items-center gap-1.5'
          >
