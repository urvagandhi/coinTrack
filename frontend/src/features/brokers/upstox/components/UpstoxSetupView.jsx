...
export function UpstoxSetupView() {
  ...
  useEffect(() => {
    ...
    if (!form.redirectUri && APP_URL) {
      setForm(p => ({
        ...p,
        redirectUri: `${APP_URL}/brokers/upstox/callback`,
      }));
    }
          ...
          {isExpired && !isConnected && (
            <div className='flex items-start gap-2 p-3 border-l-2 border-[hsl(var(--loss))] bg-[hsl(var(--loss))]/8'>
              ...
              <p className='text-[11.5px] text-[hsl(var(--loss))]'>
                Upstox tokens expire daily — reconnect to keep syncing.
              </p>
            </div>
          )}
          ...
          {isConnected ? (
            ...
            <>
              ...
              <p className='text-[11.5px] text-muted-foreground font-serif italic leading-relaxed'>
                You will be redirected to Upstox to authorize CoinTrack. After
                login, Upstox returns to your redirect URI with a code.
              </p>
              <button
                onClick={handleConnect}
                disabled={isConnecting}
              ...
              <p className='text-center text-[10px] tracking-[0.16em] uppercase text-muted-foreground'>
...
export default UpstoxSetupView;
