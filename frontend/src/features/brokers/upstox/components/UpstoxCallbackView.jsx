...
function CallbackInner() {
  ...
  const [state, setState] = useState({
    status: 'pending',
    message: 'Completing Upstox connection…',
  });
  ...
  useEffect(() => {
    ...
    if (!code) {
      setState({
        status: 'error',
        message: 'No authorization code returned from Upstox.',
      });
      const t = setTimeout(() => router.replace('/brokers/upstox'), 1800);
      return () => clearTimeout(t);
    }
...
export function UpstoxCallbackView() {
  return (
    <Suspense
      fallback={
        <div className='flex min-h-[60vh] items-center justify-center'>
...
export default UpstoxCallbackView;
