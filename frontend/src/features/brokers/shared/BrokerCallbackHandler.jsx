...
import { getBrokerByKey } from './brokerConfig';
import { useQueryClient } from '@tanstack/react-query';
import { CheckCircle2, Loader2, XCircle } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useEffect, useRef, useState } from 'react';

export function BrokerCallbackHandler({
  brokerKey,
  onCallback,
      ...
      <div className='ed-card relative max-w-md w-full px-8 py-12 text-center'>
        ...
        {status === 'loading' && (
          <div className='space-y-5'>
            <Loader2
              className='h-10 w-10 text-[hsl(var(--accent))] mx-auto animate-spin'
              strokeWidth={1.5}
            />
            <div>
              <p className='font-serif italic text-[24px] text-foreground mb-2'>
                Tokens in transit…
              </p>
              <p className='text-[12px] text-muted-foreground'>
                Exchanging credentials and reconciling your portfolio.
              </p>
            </div>
          </div>
        )}
        ...
        {status === 'success' && (
          <div className='space-y-5'>
            <div className='inline-flex h-14 w-14 items-center justify-center rounded-sm border-2 border-[hsl(var(--gain))]/40 bg-[hsl(var(--gain))]/8'>
              <CheckCircle2
                className='h-7 w-7 text-[hsl(var(--gain))]'
                strokeWidth={2}
              />
            </div>
            <div>
              <p className='font-serif italic text-[24px] text-foreground mb-2'>
                {broker?.displayName} connected.
              </p>
              <p className='text-[12px] text-muted-foreground'>
                Returning you to the directory…
              </p>
            </div>
          </div>
        )}
        ...
        {status === 'stalled' && (
          <div className='space-y-5'>
            <Loader2
              className='h-10 w-10 text-[hsl(var(--chart-4))] mx-auto animate-spin'
              strokeWidth={1.5}
            />
            <div>
              <p className='font-serif italic text-[24px] text-foreground mb-2'>
              ...
              <p className='text-[12px] text-muted-foreground'>
                Open DevTools → Network and click Retry. Look for{' '}
        ...
        {status === 'error' && (
          <div className='space-y-5'>
            <div className='inline-flex h-14 w-14 items-center justify-center rounded-sm border-2 border-[hsl(var(--loss))]/40 bg-[hsl(var(--loss))]/8'>
              <XCircle
                className='h-7 w-7 text-[hsl(var(--loss))]'
                strokeWidth={2}
              />
            </div>
            <div>
            ...
            <div className='flex justify-center gap-2 pt-2'>
              <button
                onClick={() => router.push(broker?.setupPath || '/brokers')}
                className='ed-btn ed-btn-primary'
              >
                Try again
              </button>
