...
function StatusPill({ isConnected, isExpiringSoon, isExpired, isLoading }) {
  ...
  if (isExpiringSoon)
    return <span className='ed-pill ed-pill-warn'>Expiring</span>;
  if (isConnected)
    return (
      ...
      <div className='p-6'>
        ...
        <div className='flex items-start gap-4 mb-5'>
          ...
          <div className='min-w-0 flex-1'>
            <h3 className='font-serif text-[26px] text-foreground leading-tight tracking-tight'>
              {broker.displayName}
            </h3>
            <p className='text-[12px] text-muted-foreground mt-1 font-serif italic'>
              {broker.tagline}
            </p>
          </div>
        </div>
        ...
        <div className='mb-5 pt-5 border-t border-border'>
          ...
          <ul className='grid grid-cols-2 gap-y-1.5 gap-x-3'>
            {broker.capabilities.map(cap => (
              ...
              >
                <Check
                  className='h-3 w-3 text-[hsl(var(--gain))] flex-shrink-0'
                  strokeWidth={2.5}
                />
                {cap}
              </li>
