...
import { useBrokerConnection } from '@/features/brokers/hooks/useBrokerConnection';
import { brokerAPI, portfolioAPI } from '@/shared/api/client';
import { BROKERS as BROKER_CONFIG } from '@/features/brokers/shared/brokerConfig';
import { useQueryClient } from '@tanstack/react-query';
import { AlertCircle, CheckCircle2, Info, Loader2 } from 'lucide-react';
import Link from 'next/link';
import { useEffect, useState } from 'react';
import { toast } from 'sonner';
import { BrokerSetupLayout } from '@/features/brokers/shared/BrokerSetupLayout';
import { CredentialField } from '@/features/brokers/shared/CredentialField';

const broker = BROKER_CONFIG.ANGEL_ONE;
const EMPTY_FORM = { apiKey: '', clientCode: '', password: '', totpSecret: '' };

export function AngelOneSetupView() {
  ...
  const isExpired =
    brokerStatus &&
    !brokerStatus.tokenActive &&
    brokerStatus.lastStatus !== null;
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      try {
        await portfolioAPI.manualRefresh();
      } catch {
        /* non-critical */
      }
      queryClient.invalidateQueries({ queryKey: ['brokers'] });
      ...
      if (msg.toLowerCase().includes('totp'))
        friendly =
          'The TOTP secret appears incorrect. Verify it matches what Angel One showed you.';
      else if (
        msg.toLowerCase().includes('credentials') ||
        msg.toLowerCase().includes('invalid')
      )
        friendly =
          'Client Code or password is incorrect. Please check and try again.';
      setError(friendly);
  ...
  const statusBadge = isConnected ? (
    ...
    <span className='ed-pill ed-pill-loss'>Expired</span>
  ) : credentialsSaved ? (
    <span className='ed-pill ed-pill-warn'>Credentials saved</span>
  ) : null;
      ...
      <div className='ed-card relative'>
        ...
        <div className='p-5 space-y-5'>
          ...
          {isExpired && !isConnected && (
            <div className='flex items-start gap-2 p-3 border-l-2 border-[hsl(var(--loss))] bg-[hsl(var(--loss))]/8'>
              <AlertCircle className='h-3.5 w-3.5 text-[hsl(var(--loss))] mt-0.5 flex-shrink-0' />
              <p className='text-[11.5px] text-[hsl(var(--loss))]'>
                Session expired — reconnect. The backend will auto-refresh going
                forward.
              </p>
            </div>
          )}
          ...
          {isConnected ? (
            <>
                ...
                <div className='flex-1'>
                  ...
                  {brokerStatus?.lastSuccessAt && (
                    <p className='text-[10px] font-mono text-muted-foreground mt-0.5 tnum'>
                      Last sync ·{' '}
                      {new Date(brokerStatus.lastSuccessAt).toLocaleString(
                        'en-IN'
                      )}
                    </p>
                  )}
              ...
              <div className='grid grid-cols-2 gap-2'>
                ...
                <button
                  ...
                  className='ed-btn ed-btn-ghost text-[hsl(var(--loss))]'
                >
                  {isDisconnecting ? (
                    <>
                      <Loader2 className='h-3.5 w-3.5 animate-spin' /> …
              ...
              <Link
                ...
                className='block text-center ed-link text-[12px]'
              >
                View portfolio
              </Link>
            </>
            ...
            <>
              ...
              <p className='text-[11.5px] text-muted-foreground font-serif italic leading-relaxed'>
                Angel One uses direct authentication — no browser redirect
                needed. CoinTrack connects instantly.
              </p>
              <button
                onClick={handleConnect}
                disabled={isConnecting}
            ...
            <>
              ...
              <div className='flex items-start gap-2 p-3 border-l-2 border-[hsl(var(--accent))] bg-[hsl(var(--accent-muted))]/40'>
                ...
                <p className='text-[11px] text-foreground leading-relaxed'>
                  Your TOTP secret is the base32 key shown when setting up TOTP
                  in Angel One — <strong>not</strong> the 6-digit code. Example:{' '}
                  <code className='font-mono text-[hsl(var(--accent))]'>
                    JBSWY3DPEHPK3PXP
                  </code>
                </p>
              </div>
              <button
                ...
                disabled={
                  isSaving ||
                  broker.credentialFields.some(f => !form[f.key]?.trim())
                }
                className='ed-btn ed-btn-primary w-full'
              >
                {isSaving ? (
...
export default AngelOneSetupView;
