...
import { emailAPI, portfolioAPI, totpAPI, twofaAPI, userAPI } from '@/shared/api/client';
...
const NOTIF_LABELS = {
  priceAlerts: {
    title: 'Price Alerts',
    body: 'Notify when watched symbols cross your levels.',
  },
  portfolioUpdates: {
    title: 'Portfolio Updates',
    body: 'Daily summary of net worth movement.',
  },
  marketNews: {
    title: 'Market News',
    body: 'Curated headlines for markets you follow.',
  },
  weeklyReports: {
    title: 'Weekly Reports',
    body: 'Performance digest delivered every Sunday.',
  },
};
...
export function ProfileView() {
    ...
    try {
      ...
      if (result?.alreadyVerified) {
        toast({
          ...
          description:
            'Your email is already verified — refresh to see the update.',
          variant: 'success',
        });
        ...
        toast({
          ...
          description: `A fresh link is on its way to ${profileData.email}.`,
          variant: 'success',
        });
      }
    } catch (err) {
  ...
  const handleNotif = (f, v) =>
    setProfileData(p => ({
      ...p,
      notifications: { ...p.notifications, [f]: v },
    }));
  const handlePwd = (f, v) => setPasswords(p => ({ ...p, [f]: v }));
  const togglePwdVisibility = f =>
  ...
  const changePasswordMutation = useMutation({
    mutationFn: data =>
      userAPI.changePassword(data.newPassword, data.currentPassword),
    onSuccess: () => {
      toast({
  ...
  const handleSaveProfile = async () => {
    ...
    if (cleanedMobile && !/^[6-9]\d{9}$/.test(cleanedMobile)) {
      toast({
        title: 'Validation',
        description: 'Invalid Indian mobile number',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (newEmail !== originalEmail) {
      try {
        ...
        toast({
          title: 'Verification Email Sent',
          description: 'Click the link sent to your new address.',
          variant: 'success',
        });
      } catch (e) {
        toast({
  ...
  const handleChangePassword = async () => {
    ...
    if (passwords.currentPassword === passwords.newPassword) {
      toast({
        title: 'Validation',
        description: 'New password must differ',
        variant: 'destructive',
      });
      return;
    }
    if (
      ...
      toast({
        title: 'Validation',
        description: 'Need uppercase and number',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleInitiate2FAReset = async () => {
    ...
    if (!currentTotpCode || currentTotpCode.length !== len) {
      toast({
        title: 'Validation',
        description: `Enter ${useBackupForReset ? '8-digit backup' : '6-digit'} code`,
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to initiate 2FA reset',
        variant: 'destructive',
      });
    } finally {
    ...
    <div className='space-y-10'>
      <header className='pb-6 border-b border-hairline'>
        ...
        <h1 className='display-serif text-[40px] md:text-[56px] text-foreground'>
          Profile{' '}
          <span className='italic text-[hsl(var(--accent))]'>&amp;</span>{' '}
          Identity
        </h1>
      </header>
        ...
        <aside className='space-y-6'>
          <article className='ed-card relative overflow-hidden'>
            ...
            <div className='bg-foreground text-background px-5 py-3 flex items-center justify-between'>
              <span className='text-[9px] tracking-[0.24em] uppercase font-mono'>
                CoinTrack · Subscriber
              </span>
              <span className='text-[9px] tracking-[0.18em] uppercase font-mono opacity-60'>
                № {(apiUser?.id || '0001').toString().slice(-4)}
              </span>
            </div>

            <div className='p-6 text-center'>
              <div className='relative inline-block group'>
                <div className='h-32 w-32 rounded-sm border-2 border-foreground flex items-center justify-center bg-muted relative overflow-hidden'>
              ...
              <div className='mt-5 pt-5 border-t border-hairline'>
                ...
                <p className='text-[12px] font-mono tnum text-foreground'>
            ...
            <div className='border-t border-hairline px-5 py-3 flex items-center justify-between'>
              ...
              <span className='font-mono text-[10px] text-[hsl(var(--accent))] tnum'>
                ⌘ {initials}
              </span>
            </div>
          </article>
              ...
              <dl className='space-y-3'>
                <div className='flex justify-between items-baseline pb-2 border-b border-border'>
                  ...
                  <dd className='font-mono tnum text-foreground font-medium'>
                    ₹
                    {portfolioSummary.totalInvestedValue?.toLocaleString(
                      'en-IN'
                    ) || '0'}
                  </dd>
                </div>
                <div className='flex justify-between items-baseline pb-2 border-b border-border'>
                  ...
                  <dd
                    className={cn(
                      ...
                      (portfolioSummary.totalUnrealizedPL ?? 0) >= 0
                        ? 'text-[hsl(var(--gain))]'
                        : 'text-[hsl(var(--loss))]'
                    )}
                  >
                    {(portfolioSummary.totalUnrealizedPL ?? 0) >= 0 ? '+' : ''}₹
                    {portfolioSummary.totalUnrealizedPL?.toLocaleString(
                      'en-IN'
                    ) || '0'}
                  </dd>
                </div>
          ...
          <section className='ed-card relative'>
            ...
            <header className='flex items-center justify-between px-6 py-4 border-b border-hairline'>
              <div className='flex items-baseline gap-3'>
                ...
                <h3 className='font-serif text-[22px] text-foreground leading-none'>
                  Personal Information
                </h3>
              </div>
              <button
                ...
                className={cn(
                  'ed-btn',
                  isEditing ? 'ed-btn-ghost' : 'ed-btn-info'
                )}
              >
                {isEditing ? (
                  <X className='h-3 w-3' />
            ...
            <div className='p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5'>
              ...
              {!isEditing && apiUser && !apiUser.isEmailVerified && (
                <div className='md:col-span-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-dashed border-border/60 rounded-md px-4 py-3'>
                  <p className='text-[12px] text-muted-foreground leading-snug'>
                    ...
                    isn&apos;t verified yet. Some security emails may be
              ...
              <ProfileField
                ...
                displayValue={
                  profileData.phoneNumber
                    ? `+91 ${profileData.phoneNumber.replace(/^\+91/, '')}`
                    : ''
                }
                prefix='+91'
                maxLength={10}
                ...
                <div className='border-t border-hairline pt-4'>
                  {!showDeleteConfirm ? (
                    <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3'>
                      <div>
                        <p className='eyebrow mb-1 text-[hsl(var(--loss))]'>
                          Danger Zone
                        </p>
                        <p className='text-[12px] text-muted-foreground leading-snug'>
                          Permanently delete your account and every record
                          across all modules. This cannot be undone.
                        </p>
                      </div>
                      <button
                        type='button'
                    ...
                    <div className='border-l-2 border-[hsl(var(--loss))] bg-[hsl(var(--loss))]/8 p-4 space-y-3'>
                      ...
                      <p className='text-[12px] text-muted-foreground leading-snug'>
                        Your profile, portfolio, ledgers (FD · PPF · EPF ·
                        Gold/Silver · Mutual Funds), notes, broker links and 2FA
                        will be erased immediately. A deletion audit record is
                        retained for compliance — nothing else survives.
                      </p>
                      <input
                        type='password'
                        ...
                        placeholder={
                          apiUser?.authProvider === 'GOOGLE'
                            ? 'Google account — leave blank'
                            : 'Confirm your password to enable deletion'
                        }
            ...
            <header className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-b border-hairline'>
              ...
              <div className='flex gap-2 flex-wrap'>
                <button
                  ...
                  className={cn(
                    'ed-btn',
                    showPasswordFields ? 'ed-btn-ghost' : 'ed-btn-gain'
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    'ed-btn',
                    show2FAResetFlow ? 'ed-btn-ghost' : 'ed-btn-warn'
                  )}
                >
            ...
            {show2FAResetFlow && (
              <div className='p-6 border-b border-hairline space-y-4'>
                  ...
                  <>
                        ...
                        {!showRecoveryConfirm ? (
                          ...
                          <div className='border-l-2 border-[hsl(var(--loss))] bg-[hsl(var(--loss))]/8 p-4 text-left'>
                            <p className='text-[11.5px] text-foreground mb-3'>
                              Send recovery link to{' '}
                              <span className='font-mono'>{user?.email}</span>?
                              Existing 2FA will be disabled.
                            </p>
                    ...
                    <div className='flex justify-end gap-2 pt-2'>
                      ...
                      <button
                        ...
                        disabled={
                          is2FALoading ||
                          currentTotpCode.length !== (useBackupForReset ? 8 : 6)
                        }
                ...
                {twoFAStep === 'newcode' && resetSetupData && (
                  <>
                    <p className='text-[12.5px] text-muted-foreground font-serif italic leading-relaxed'>
                      Scan this QR with your authenticator, then enter the new
                      6-digit code.
                    </p>
                    {resetSetupData.qrCodeBase64 && (
                      <div className='flex justify-center bg-white p-4 border border-border rounded-sm'>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={resetSetupData.qrCodeBase64}
                          alt='QR Code'
                          className='w-40 h-40'
                        />
                      </div>
                    ...
                    {resetSetupData.secret && (
                      <div className='text-center'>
                        ...
                        <code className='bg-muted px-3 py-1.5 text-[12px] font-mono text-foreground border border-border inline-block rounded-sm'>
                          {resetSetupData.secret}
                        </code>
                      </div>
                    )}
                    <div>
                      ...
                      <input
                        ...
                        value={newTotpCode}
                        onChange={e =>
                          setNewTotpCode(e.target.value.replace(/[^0-9]/g, ''))
                        }
                        className='w-full h-12 px-4 bg-background border border-input text-[20px] text-foreground rounded-sm focus:outline-none focus:border-[hsl(var(--accent))] font-mono tnum text-center tracking-[0.4em]'
                        placeholder='000000'
                      />
                    ...
                    <div className='flex justify-end gap-2 pt-2'>
                      ...
                      <button
                        ...
                        disabled={is2FALoading || newTotpCode.length !== 6}
                        className='ed-btn ed-btn-gain'
                      >
                        <Shield className='h-3 w-3' />
                ...
                {twoFAStep === 'backup' && (
                  <>
                    <div className='border-l-2 border-[hsl(var(--gain))] bg-[hsl(var(--gain))]/8 p-4'>
                      <p className='text-[13px] font-semibold text-[hsl(var(--gain))]'>
                        ✓ 2FA Reset Complete
                      </p>
                      <p className='text-[11.5px] text-foreground mt-1'>
                        Save your new backup codes — you will not see them
                        again.
                      </p>
                    </div>
                    <div className='grid grid-cols-2 gap-2'>
                      {newBackupCodes.map((code, i) => (
                        <div
                          key={i}
                          className='bg-muted border border-border px-3 py-2 text-center font-mono text-[13px] text-foreground rounded-sm tnum'
                        >
            ...
            <div className='p-6 border-t border-hairline'>
              <div className='flex items-baseline justify-between mb-4'>
                <div className='flex items-baseline gap-3'>
                  ...
                  <h4 className='font-serif text-[18px] text-foreground leading-none'>
                ...
                {totpStatusLoading ? (
                  ...
                  <span className='ed-pill ed-pill-gain'>
              ...
              {totpStatusLoading ? (
                ...
                <div className='px-5 py-4 border-l-2 border-[hsl(var(--gain))] bg-[hsl(var(--gain))]/8'>
                  <div className='flex items-start gap-3'>
                    <ShieldCheck
                      className='h-5 w-5 text-[hsl(var(--gain))] mt-0.5 flex-shrink-0'
                      strokeWidth={2}
                    />
                    <div className='flex-1'>
                      <p className='text-[13px] font-semibold text-[hsl(var(--gain))] mb-1'>
                        2FA is active
                      </p>
                      <p className='text-[12px] text-foreground leading-relaxed'>
                        Your account is protected. You will be asked for a code
                        when you log in.
                      </p>
                      {totpStatus.setupAt && (
                        <p className='text-[10px] font-mono text-muted-foreground mt-2 tnum'>
                          Setup on ·{' '}
                          {new Date(totpStatus.setupAt).toLocaleDateString(
                            'en-IN',
                ...
                <div className='px-5 py-4 border-l-2 border-[hsl(var(--loss))] bg-[hsl(var(--loss))]/8'>
                  <div className='flex items-start gap-3'>
                    <ShieldAlert
                      className='h-5 w-5 text-[hsl(var(--loss))] mt-0.5 flex-shrink-0'
                      strokeWidth={2}
                    />
                    <div className='flex-1'>
                      <p className='text-[13px] font-semibold text-[hsl(var(--loss))] mb-1'>
                        Action Required
                      </p>
                      <p className='text-[12px] text-foreground leading-relaxed'>
                        2FA is currently disabled. Enable it to secure your
                        account.
                      </p>
                    </div>
                  </div>
              ...
              {totpStatus?.enabled && (
                <div className='mt-5 pt-5 border-t border-border'>
                  ...
                  <p className='text-[11.5px] text-muted-foreground font-serif italic leading-relaxed'>
                    Resetting 2FA will generate a new secret key and invalidate
                    all previous backup codes. Use the{' '}
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <p className='text-[11.5px] text-muted-foreground font-serif italic mt-0.5'>
                        {meta.body}
                      </p>
                    </div>
                    <button
    ...
    <div className={className}>
      <div className='flex items-baseline justify-between mb-2'>
        <label className='eyebrow-strong'>
          {label}
          {required && (
            <span className='text-[hsl(var(--loss))] ml-0.5'>*</span>
          )}
        </label>
        {badge && (
