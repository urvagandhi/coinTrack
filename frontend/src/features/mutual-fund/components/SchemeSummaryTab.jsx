...
function StatusBadge({ scheme }) {
  ...
  if (statuses.includes('PARTIALLY_REDEEMED'))
    activeBadges.push('PARTIALLY REDEEMED');
...
export default function SchemeSummaryTab({ onNewScheme, onEditScheme }) {
  const [selectedSchemeForOverride, setSelectedSchemeForOverride] =
    React.useState(null);
  ...
  const processedSchemes = useMemo(() => {
    ...
    return schemes.map(scheme => {
      ...
      if (newStatuses.includes('FULLY_REDEEMED')) {
        ...
        newStatuses.includes('SIP') &&
        !schemesWithActiveMandates.has(scheme.schemeId)
      ) {
        ...
        if (
          !newStatuses.includes('LUMPSUM') &&
          (scheme.totalUnit > 0 || scheme.totalInvestment > 0)
        ) {
  ...
  const uniquePlatforms = useMemo(() => {
    const platforms = new Set(
      processedSchemes.map(s => s.platform || 'Other Platforms')
    );
                ...
                <tbody>
                  {platformSchemes.map(scheme => (
                    ...
                    >
                      ...
                      <td className='py-3 px-4 text-right font-mono text-[13px] text-foreground'>
                        <div className='flex items-center justify-end gap-1.5'>
                          ...
                          <button
                            onClick={e => {
                              e.stopPropagation();
                              setSelectedSchemeForOverride(scheme);
                            }}
                      ...
                      <td className='py-3 px-4 text-right font-mono text-[13px] text-foreground'>
                        {scheme.averageNav != null
                          ? `₹${scheme.averageNav.toFixed(2)}`
                          : '₹0.00'}
                      </td>
                      <td className='py-3 px-4 text-right font-mono text-[13px] text-foreground'>
