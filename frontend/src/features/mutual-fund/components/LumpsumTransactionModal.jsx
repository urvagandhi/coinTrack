...
import DataAccuracyWarning from '@/features/portfolio';
...
export default function LumpsumTransactionModal({
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (initialData?.id) {
        ...
        toast({
          title: 'Success',
          description: 'Lumpsum transaction updated successfully.',
        });
      } else {
        await mutualFundAPI.createLumpsum(payload);
        toast({
          title: 'Success',
          description: 'Lumpsum transaction added successfully.',
        });
      }
  ...
  const handleDelete = () => {
    toast({
      ...
      description:
        'Are you sure you want to delete this lumpsum entry? This action cannot be undone.',
      variant: 'warning',
      action: (
        <button
          onClick={async () => {
            ...
            try {
              ...
              toast({
                title: 'Success',
                description: 'Lumpsum entry deleted successfully.',
              });
              onSuccess();
              onClose();
            } catch (error) {
              toast({
                ...
                description:
                  ...
                  'Failed to delete lumpsum entry.',
                variant: 'destructive',
              });
            } finally {
              ...
              <div className='space-y-4'>
                ...
                <div className='grid grid-cols-2 gap-4 items-start animate-in slide-in-from-top-1 fade-in duration-200'>
                  ...
                  <div className='space-y-1.5 flex-1'>
                    ...
                    />
                    {formData.lumpsumInvestment &&
                      !isNaN(parseFloat(formData.lumpsumInvestment)) && (
                        ...
                        {formData.lumpsumInvestment &&
                          !isNaN(parseFloat(formData.lumpsumInvestment)) && (
                            <span className='flex justify-between font-medium text-accent'>
                              <span>Est. Allotted Units:</span>{' '}
                              <span>
                                {(
                                  getNetInvestment() / calculatedNavData.nav
                                ).toFixed(3)}
                              </span>
                            </span>
                          )}
