...
import { mutualFundAPI } from '@/shared/api/client';
...
export default function SipTab() {
  ...
  const stopMutation = useMutation({
    ...
    onError: () =>
      toast({
        title: 'Error',
        description: 'Failed to stop mandate.',
        variant: 'destructive',
      }),
  });
  ...
  const restartMutation = useMutation({
    ...
    onError: () =>
      toast({
        title: 'Error',
        description: 'Failed to renew mandate.',
        variant: 'destructive',
      }),
  });
  ...
  const data = useMemo(() => {
    ...
    sips.forEach(sip => {
      ...
      if (sip.contributionDate) {
        const y = Array.isArray(sip.contributionDate)
          ? sip.contributionDate[0]
          : new Date(sip.contributionDate).getFullYear();
        const mo = Array.isArray(sip.contributionDate)
          ? sip.contributionDate[1]
          : new Date(sip.contributionDate).getMonth() + 1;
        // Track min from actual contributions
        if (
          minYear === null ||
          y < minYear ||
          (y === minYear && mo < minMonth)
        ) {
          minYear = y;
          minMonth = mo;
        }
    ...
    while (currY < maxYear || (currY === maxYear && currM <= maxMonth)) {
      const monthName = new Date(currY, currM - 1).toLocaleString('default', {
        month: 'short',
      });
      monthColumns.push({
    ...
    const activeTotalAmount = activeMandates.reduce(
      (sum, m) => sum + (m.amount || 0),
      0
    );
    const stoppedTotalAmount = stoppedMandates.reduce(
      (sum, m) => sum + (m.amount || 0),
      0
    );
    ...
    monthColumns.forEach(col => {
      ...
      grandMonthTotals[col.key] =
        activeMonthTotals[col.key] + stoppedMonthTotals[col.key];
    });

    const activeTotalInvested = activeMandates.reduce(
      (sum, m) => sum + mandateTotals[m.id],
      0
    );
    const stoppedTotalInvested = stoppedMandates.reduce(
      (sum, m) => sum + mandateTotals[m.id],
      0
    );
      ...
      <>
        <section className='ed-card relative px-8 py-16 text-center max-w-md mx-auto'>
          ...
          <button
            ...
            className='ed-btn ed-btn-accent mt-6'
          >
      ...
      if (m.platform !== currentPlatform) {
        rows.push(
          <tr key={`sep-${m.platform}`} className='border-b border-border/40'>
            <td
              colSpan={8 + data.monthColumns.length}
              className='py-1.5 px-0 bg-muted'
            >
      ...
      rows.push(
        ...
        >
          ...
          <td className='py-3 px-4 sticky left-[200px] w-[250px] min-w-[250px] bg-background group-hover:bg-muted z-10'>
            <p className='font-serif text-[14px] text-foreground font-medium'>
              {m.schemeName}
            </p>
              ...
              <>
                ...
                {m.endDate && (
                  <div className='text-[11px] text-muted-foreground mt-0.5'>
                    E: {formatDate(m.endDate)}
                  </div>
                )}
                <div className='text-[11px] text-muted-foreground mt-0.5 font-medium'>
                  (
                  {formatContributionDuration(
                    Object.keys(data.contributionsByMandate[m.id] || {}).length
                  )}
                  )
                </div>
              </>
          ...
          {data.monthColumns.map(col => {
            ...
            const mEnd = Array.isArray(m.endDate)
              ...
              : m.endDate
                ? new Date(
                    new Date(m.endDate).getFullYear(),
                    new Date(m.endDate).getMonth(),
                    1
                  )
                : null;
            ...
            return (
              <td
                key={col.key}
                className='py-3 px-4 text-center font-mono text-[12px] text-muted-foreground min-w-[80px]'
              >
                {contribution ? (
                  <div
                    className={`font-medium cursor-pointer hover:underline flex items-center justify-center gap-1 ${
                      contribution.status === 'FAILED'
                        ? 'text-red-500'
                        : contribution.status === 'PENDING_NAV'
                          ? 'text-yellow-500'
                          : 'text-foreground'
                    }`}
                    onClick={e => {
                  ...
                  >
                    {contribution.status === 'FAILED' && (
                      <span className='text-[10px]' title='Failed'>
                        ⚠️
                      </span>
                    )}
                    {contribution.status === 'PENDING_NAV' && (
                      <span className='text-[10px]' title='Pending NAV'>
                        ⏳
                      </span>
                    )}
                    ₹{contribution.amount}
    ...
    <div className='space-y-8'>
      ...
      <div className='ed-card relative overflow-hidden flex flex-col'>
        ...
        <div className='p-4 border-b border-border bg-muted/20 flex justify-between items-center'>
          ...
          <div className='flex gap-2'>
            <button
              ...
              className='ed-btn ed-btn-ghost h-8 text-[11px] border border-border'
            >
            ...
            <button
              ...
              className='ed-btn ed-btn-accent h-8 text-[11px]'
            >
              <Plus className='h-3.5 w-3.5' strokeWidth={2.5} />
        ...
        <div className='px-4 py-2 bg-blue-500/10 border-b border-border text-[11px] text-blue-600 dark:text-blue-400 font-medium'>
          Note: Investment amounts are shown strictly as Net Investment, after
          deducting the applicable Government Stamp Duty (0.005% since July
          2020).
        </div>
          ...
          <table className='w-full text-left border-collapse min-w-max'>
            <thead>
              <tr className='border-b border-border bg-muted'>
                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground sticky left-0 w-[200px] min-w-[200px] bg-muted z-20'>
                  Holder / Platform
                </th>
                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground sticky left-[200px] w-[250px] min-w-[250px] bg-muted z-20'>
                  Scheme Name
                </th>
                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground sticky left-[450px] w-[120px] min-w-[120px] bg-muted z-20'>
                  Debited Bank
                </th>
                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground sticky left-[570px] w-[150px] min-w-[150px] bg-muted z-20'>
                  Start Date
                </th>
                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground text-right sticky left-[720px] w-[120px] min-w-[120px] bg-muted z-20'>
                  Amount
                </th>
                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground text-right sticky left-[840px] w-[130px] min-w-[130px] bg-muted z-20 border-r border-border shadow-[1px_0_0_0_rgba(0,0,0,0.1)]'>
                  Total Net Inv.
                </th>
                ...
                {data.monthColumns.map(col => (
                  <th
                    key={col.key}
                    className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground text-center'
                  >
                    {col.label}
                  </th>
                ))}

                <th className='py-3 px-4 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground text-right border-l border-border/30 sticky right-0 bg-muted z-20'>
                  Action
                </th>
              </tr>
            </thead>
            <tbody>
              ...
              {data.activeMandates.length > 0 && (
                <>
                  <tr className='bg-[hsl(var(--gain))]/5 border-y border-[hsl(var(--gain))]/20'>
                    <td
                      colSpan={8 + data.monthColumns.length}
                      className='py-2 px-0 bg-background'
                    >
              ...
              <tr className='bg-primary/5 border-t-2 border-primary/20 font-bold'>
                <td
                  className='py-4 px-4 sticky left-0 z-10 bg-muted w-[200px] min-w-[200px]'
                  colSpan={1}
                ></td>
                <td
                  className='py-4 px-4 sticky left-[200px] z-10 bg-muted w-[250px] min-w-[250px]'
                  colSpan={1}
                ></td>
                <td
                  className='py-4 px-4 sticky left-[450px] z-10 bg-muted w-[120px] min-w-[120px]'
                  colSpan={1}
                ></td>
                <td className='py-4 px-4 sticky left-[570px] z-10 bg-muted w-[150px] min-w-[150px] text-right'>
                  <span className='text-[13px] font-mono uppercase tracking-wider text-primary'>
                    Active Total
                  </span>
                </td>
                ...
                {data.monthColumns.map(col => (
                  <td
                    key={`total-active-${col.key}`}
                    className='py-4 px-4 text-center font-mono text-[13px] text-primary bg-muted/20'
                  >
                    {data.activeMonthTotals[col.key] > 0
                      ? formatCurrency(data.activeMonthTotals[col.key])
                      : '-'}
                  </td>
                ))}
              ...
              {data.stoppedMandates.length > 0 && (
                <>
                  <tr className='bg-amber-500/5 border-y border-amber-500/20'>
                    <td
                      colSpan={8 + data.monthColumns.length}
                      className='py-2 px-0 mt-4 bg-background'
                    >
              ...
              <tr className='bg-[hsl(var(--gain))]/10 border-t-2 border-[hsl(var(--gain))]/30 font-bold'>
                ...
                ></td>
                <td className='py-4 px-4 sticky left-[570px] z-10 bg-muted w-[150px] min-w-[150px] text-right'>
                  <span className='text-[13px] font-mono uppercase tracking-wider text-[hsl(var(--gain))]'>
                    Grand Total
                  </span>
                </td>
                ...
                {data.monthColumns.map(col => (
                  <td
                    key={`total-grand-${col.key}`}
                    className='py-4 px-4 text-center font-mono text-[13px] text-[hsl(var(--gain))] bg-muted/20'
                  >
                    {data.grandMonthTotals[col.key] > 0
                      ? formatCurrency(data.grandMonthTotals[col.key])
                      : '-'}
                  </td>
                ))}
                <td className='py-4 px-4 sticky right-0 bg-muted z-10 border-l border-border/30'></td>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm'>
          <div className='ed-card w-full max-w-md p-6 space-y-6'>
            ...
            <p className='text-sm text-muted-foreground font-mono'>
              {confirmModal.type === 'STOP'
                ? 'Please select the date when this SIP was stopped.'
                : 'Please select the new start date for this SIP.'}
              <br />
            ...
            <div className='space-y-2'>
              <label className='text-[11px] font-mono uppercase tracking-wider text-muted-foreground'>
                {confirmModal.type === 'STOP' ? 'Stopped Date' : 'Start Date'}{' '}
              ...
              <input
                ...
                onChange={e =>
                  setConfirmModal(prev => ({ ...prev, date: e.target.value }))
                }
              />
            </div>
