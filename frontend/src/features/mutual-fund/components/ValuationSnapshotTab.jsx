...
import ValuationSnapshotModal from './ValuationSnapshotModal';
...
export default function ValuationSnapshotTab() {
  ...
  const handleSuccess = () => {
    queryClient.invalidateQueries({ queryKey: ['mfValuations'] });
  };
    ...
    return (
      <section className='ed-card relative px-8 py-16 text-center max-w-md mx-auto'>
        ...
        <button
          ...
          className='ed-btn ed-btn-accent mx-auto'
        >
          <Plus className='h-4 w-4' />
          <span>Add Snapshot</span>
        </button>
        <ValuationSnapshotModal
          isOpen={modalOpen}
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex items-center justify-between bg-card p-3 rounded-sm border border-border'>
        ...
        <div className='flex gap-2 overflow-x-auto px-4 hide-scrollbar items-center'>
          ...
          <button
            ...
            className='text-[11px] font-mono font-bold tracking-widest flex items-center gap-1 hover:text-accent transition-colors'
          >
            ...
            <tbody>
              {displayedValuations.map(val => (
                ...
                >
                  ...
                  <td
                    className={`py-3 px-4 text-right font-mono text-[13px] font-semibold ${val.periodPL >= 0 ? 'text-[hsl(var(--gain))]' : 'text-[hsl(var(--loss))]'}`}
                  >
                    {val.periodPL >= 0 ? '+' : ''}
                    {formatCurrency(val.periodPL)}
                  </td>
                  <td
                    className={`py-3 px-4 text-right font-mono text-[13px] font-semibold ${val.periodPLPercent >= 0 ? 'text-[hsl(var(--gain))]' : 'text-[hsl(var(--loss))]'}`}
                  >
                    {val.periodPLPercent >= 0 ? '+' : ''}
                    {val.periodPLPercent?.toFixed(2) || '0.00'}%
                  </td>
                  <td className='py-3 px-4 text-center'>
                    <button
                      onClick={() => {
                        setEditingSnapshot(val);
                        setModalOpen(true);
                      }}
                      className='p-1.5 text-muted-foreground hover:text-accent hover:bg-muted rounded transition-colors inline-flex items-center'
                    >
