...
const getStatusBadge = status => {
  if (status === 'COMPLETED')
    return (
      <span className='text-[10px] bg-green-500/10 text-green-600 px-2 py-0.5 rounded'>
        Completed
      </span>
    );
  if (status === 'PENDING_NAV')
    return (
  ...
  if (status === 'FAILED')
    return (
      <span className='text-[10px] bg-red-500/10 text-red-600 px-2 py-0.5 rounded flex items-center gap-1 w-max'>
        Failed{' '}
        <span title='Needs manual intervention' className='cursor-help'>
          ⚠️
        </span>
      </span>
    );
...
export default function RedemptionTab() {
  ...
  const data = useMemo(() => {
    ...
    return baseData.filter(
      r => r.redemptionDate && getFinancialYear(r.redemptionDate) === selectedFy
    );
  }, [baseData, selectedFy]);
      ...
      (acc, txn) => ({
        netRedemption:
          ...
          (txn.sttAmount > 0 || txn.exitLoadDeducted > 0
            ? txn.netRedemptionValue || 0
            : txn.redemptionValue || 0),
  ...
  const groupedData = useMemo(() => {
    ...
    const groups = {};
    data.forEach(txn => {
      const fy = txn.redemptionDate
        ? getFinancialYear(txn.redemptionDate)
        : 'Unknown';
    ...
    return Object.keys(groups)
      ...
      .map(fy => {
        ...
        const groupTotals = groupTxns.reduce(
          (acc, txn) => ({
            netRedemption:
              acc.netRedemption +
              (txn.sttAmount > 0 || txn.exitLoadDeducted > 0
              ...
              <React.Fragment key={group.fy}>
                {selectedFy === 'ALL' && (
                  <tr className='bg-muted/40 border-b border-border'>
                    <td
                      colSpan={10}
                      className='py-2.5 px-4 font-mono font-semibold text-[11px] uppercase tracking-wider text-muted-foreground'
                    >
                      {group.fy}
                    </td>
                  </tr>
                ...
                {group.transactions.map(txn => (
                  ...
                  >
                    ...
                    <td className='py-3 px-4 text-right font-mono text-[13px] text-foreground'>
                      <p className='font-semibold text-[hsl(var(--loss))]'>
                        {txn.redemptionUnit != null
                          ? txn.redemptionUnit.toLocaleString('en-IN', {
                              minimumFractionDigits: 2,
                      ...
                      {txn.totalUnit != null && (
                        <p className='text-[10px] text-muted-foreground'>
                          (of{' '}
                          {txn.totalUnit.toLocaleString('en-IN', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 3,
                          })}{' '}
                          total)
                    ...
                    <td className='py-3 px-4 text-right font-mono text-[13px] text-muted-foreground'>
                      {txn.redemptionNav
                        ? formatCurrency(txn.redemptionNav)
                        : '-'}
                    </td>
                    <td className='py-3 px-4 text-right font-mono text-[13px] text-muted-foreground'>
                      ...
                      {txn.sttAmount > 0 || txn.exitLoadDeducted > 0 ? (
                        <div className='flex flex-col items-end'>
                          ...
                          <div className='flex flex-col items-end bg-muted/30 border border-border/50 rounded px-2 py-1 mt-1 min-w-[120px]'>
                            <span className='text-[9px] text-muted-foreground uppercase tracking-wider mb-0.5 border-b border-border/50 w-full text-right pb-0.5'>
                              Deductions
                            </span>
                            <div className='flex justify-between w-full gap-3 text-[10px] mt-0.5'>
                              ...
                              <span className='text-foreground'>
                                {formatCurrency(txn.redemptionValue)}
                              </span>
                            </div>
                            {txn.exitLoadDeducted > 0 && (
                              <div
                                className='flex justify-between w-full gap-3 text-[10px] text-[hsl(var(--loss))]'
                                title='Exit Load Deducted'
                              >
                                <span>Exit Load</span>
                                <span>
                                  - {formatCurrency(txn.exitLoadDeducted)}
                                </span>
                              </div>
                            )}
                            {txn.sttAmount > 0 && (
                              <div
                                className='flex justify-between w-full gap-3 text-[10px] text-[hsl(var(--loss))]'
                                title='Securities Transaction Tax (0.001%)'
                              >
                                <span>STT</span>
                    ...
                    <td
                      className={`py-3 px-4 text-right font-mono text-[13px] font-semibold ${txn.capitalGain >= 0 ? 'text-[hsl(var(--gain))]' : 'text-[hsl(var(--loss))]'}`}
                    >
                      {txn.capitalGain >= 0 ? '+' : ''}
                      {formatCurrency(txn.capitalGain)}
                      {txn.gainType && (
                        <span className='block text-[9px] text-muted-foreground font-normal tracking-widest mt-0.5'>
                          {txn.gainType}
                        </span>
                      )}
                    </td>
                ...
                {group.transactions.length > 0 && (
                  <tr className='border-t-2 border-border bg-muted/10 font-semibold'>
                    <td
                      colSpan={8}
                      className='py-4 px-4 text-right font-mono text-[12px] text-muted-foreground uppercase tracking-wider'
                    >
                    ...
                    <td
                      className={`py-4 px-4 text-right font-mono text-[14px] font-semibold ${group.totals.capitalGain >= 0 ? 'text-[hsl(var(--gain))] bg-[hsl(var(--gain))]/5' : 'text-[hsl(var(--loss))] bg-[hsl(var(--loss))]/5'}`}
                    >
                      {group.totals.capitalGain >= 0 ? '+' : ''}
                      {formatCurrency(group.totals.capitalGain)}
                    </td>
                  </tr>
                )}
              </React.Fragment>
