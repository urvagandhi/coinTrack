...
export default function SipMandateModal({
  isOpen,
  onClose,
  onSuccess,
  onDelete: _onDelete,
  ...
  }, [isOpen, initialData, defaultHolder]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (initialData?.id || initialData?.sipId) {
        await mutualFundAPI.updateSipMandate(
          initialData.id || initialData.sipId,
          payload
        );
        toast({
          title: 'Success',
          description: 'SIP Mandate updated successfully.',
        });
      } else {
        await mutualFundAPI.createSipMandate(payload);
        toast({
          title: 'Success',
          description: 'SIP Mandate created successfully.',
        });
      }
      ...
      action: (
        <button
          onClick={async () => {
            ...
            try {
              await mutualFundAPI.deleteSipMandate(
                initialData.id || initialData.sipId
              );
              toast({
                title: 'Success',
              ...
              toast({
                title: 'Error',
                description:
                  error.response?.data?.message || 'Failed to delete mandate.',
                variant: 'destructive',
              });
            } finally {
              setDeleteLoading(false);
              ...
              <div className='space-y-4'>
                ...
                <div className='space-y-1.5 pt-4'>
                  ...
                  <input
                    ...
                    disabled
                    placeholder={
                      formData.bank ? '' : 'Select a scheme to auto-fill bank'
                    }
                  ...
                  {!formData.active && (
                    <div className='mt-4 space-y-1.5 animate-in fade-in slide-in-from-top-1 bg-muted/20 p-3 rounded-md border border-border'>
                      ...
                      <p className='text-[11px] text-muted-foreground mt-1'>
                        If this mandate has been stopped, enter the date of the
                        last installment.
                      </p>
                    </div>
                  )}
