...
export default function LumpsumTab() {
  ...
  const data = useMemo(() => {
    ...
    return baseData.filter(
      r => r.investmentDate && getFinancialYear(r.investmentDate) === selectedFy
    );
  }, [baseData, selectedFy]);
  ...
  const groupedData = useMemo(() => {
    ...
    const groups = {};
    data.forEach(txn => {
      const fy = txn.investmentDate
        ? getFinancialYear(txn.investmentDate)
        : 'Unknown';
    ...
    return Object.keys(groups)
      ...
      .map(fy => ({
        ...
        totalInvestment: groups[fy].reduce(
          (sum, t) => sum + (t.lumpsumInvestment || 0),
          0
        ),
      }));
  ...
  return (
    <div className='ed-card relative overflow-hidden'>
      ...
      <div className='px-4 py-2 bg-blue-500/10 border-b border-border text-[11px] text-blue-600 dark:text-blue-400 font-medium'>
        ...
        deducting the applicable Government Stamp Duty (0.005% since July 2020).
      </div>
