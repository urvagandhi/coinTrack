...
export default function EpfSettingsDialog({
  isOpen,
  onClose,
  settings,
  onSaveSettings,
  ...
  const [employeeContributionRate, setEmployeeContributionRate] =
    useState('12.00');
  ...
  useEffect(() => {
    if (isOpen && settings) {
      setDefaultBasicDA(
        settings.defaultBasicDA !== undefined
          ? String(settings.defaultBasicDA)
          : '0'
      );
      setEmployeeContributionRate(
        settings.employeeContributionRate !== undefined
          ? String(settings.employeeContributionRate)
          : '12.00'
      );
      setUseActualSalaryForEps(!!settings.useActualSalaryForEps);
      setMonthlyVpfAmount(
        settings.monthlyVpfAmount !== undefined
          ...
          >
            ...
            <div className='space-y-1.5'>
              <label className='eyebrow'>
                Default Monthly Basic + DA (₹) [Optional]
              </label>
              <input
                type='text'
                value={defaultBasicDA}
                onChange={e =>
                  setDefaultBasicDA(parseShortcutAmount(e.target.value))
                }
                className='ed-input w-full font-mono text-base font-medium'
                placeholder='e.g. 50000'
              />
              <p className='text-[11px] text-muted-foreground'>
                Pre-populates the salary field when creating AUTO_SALARY monthly
                entries.
              </p>
            </div>
            ...
            <div className='space-y-1.5'>
              ...
              >
                ...
                <option value='10.00'>
                  10.00% (Eligible establishments rate)
                </option>
                <option value='8.00'>8.00% (Reduced rate)</option>
              </select>
              <p className='text-[11px] text-muted-foreground'>
                Statutory percentage deducted from Basic+DA into your EPF
                account.
              </p>
            </div>
            ...
            <div className='space-y-1.5'>
              <label className='eyebrow'>
                Default Monthly VPF Amount (₹) [Optional]
              </label>
              <input
                type='text'
                value={monthlyVpfAmount}
                onChange={e =>
                  setMonthlyVpfAmount(parseShortcutAmount(e.target.value))
                }
                className='ed-input w-full font-mono'
                placeholder='e.g. 5000'
              />
              <p className='text-[11px] text-muted-foreground'>
                Voluntary Provident Fund amount added on top of statutory 12%.
                Renders 100% into EPF pool.
              </p>
            </div>
          </form>
        </div>
