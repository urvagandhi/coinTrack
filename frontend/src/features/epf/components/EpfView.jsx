...
import EpfInterestRateDialog from './EpfInterestRateDialog';
import EpfSettingsDialog from './EpfSettingsDialog';
import EpfTransactionDialog from './EpfTransactionDialog';
import { Skeleton } from '@/shared/ui/feedback/Skeleton';
import { useToast } from '@/shared/ui/feedback/use-toast';
import FilterDropdown from '@/shared/ui/forms/FilterDropdown';
import { epfAPI, userAPI } from '@/shared/api/client';
import { generateFinancialYearOptions, getFinancialYear } from '@/shared/formatters';
      ...
      <div className='flex flex-col justify-center'>
        <span
          className={cn(
            ...
            isW
              ? 'text-[hsl(var(--loss))] group-hover:text-[hsl(var(--loss))]'
              : 'text-foreground group-hover:text-[hsl(var(--accent))]'
          )}
        >
          {isW
            ? `Withdrawal — ${formatCurrency(txn.withdrawalAmount)}`
            : txn.remarks ||
              (isAuto ? 'Auto Salary Split' : 'Manual Contribution')}
        </span>
...
export function EpfView() {
  ...
  const transactions = useMemo(
    () => (Array.isArray(txnData) ? txnData : (txnData?.content ?? [])),
    [txnData]
  );
  const allTransactions = useMemo(
    () =>
      Array.isArray(allTxnData) ? allTxnData : (allTxnData?.content ?? []),
    [allTxnData]
  );
  const totalPages =
    txnData?.totalPages ??
    txnData?.page?.totalPages ??
    (Array.isArray(txnData) ? 1 : 0);
  const totalElements =
    txnData?.totalElements ??
    txnData?.page?.totalElements ??
    transactions.length;
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'EPF Entry Created', variant: 'success' });
      invalidate();
    },
    onError: onErr,
  });
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'EPF Entry Updated', variant: 'success' });
      invalidate();
    },
    onError: onErr,
  });
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'EPF Entry Deleted', variant: 'success' });
      invalidate();
    },
    onError: onErr,
  });
  ...
  const handleDeleteTxn = () => {
    if (editingTxn) {
      toast({
        ...
        description:
          'This action will trigger a full dual-balance (EPF & EPS) recalculation.',
        variant: 'warning',
        action: (
          <button
            onClick={() => {
  ...
  const interestSheet = useMemo(() => {
    ...
    const activeRate =
      ratesData?.find(r => r.financialYear === financialYear)?.ratePercent ||
      8.25;
    ...
    for (let y = minYear; y < startYear; y++) {
      ...
      const yearRate =
        ratesData?.find(r => r.financialYear === yearFY)?.ratePercent || 8.25;

      // Get transactions of this year
      const yearTxns = sortedAll.filter(
        t => getFinancialYear(t.transactionDate) === yearFY
      );
        ...
        if (key) {
          ...
          txnMap.set(key, {
            ...
            employerEpfContribution:
              existing.employerEpfContribution +
              (txn.employerEpfContribution || 0),
      ...
      let yearEmpInterest =
        Math.round(((currentEmpClosing * 12 * yearRate) / 1200) * 100) / 100;
      let yearEmprInterest =
        Math.round(((currentEmprClosing * 12 * yearRate) / 1200) * 100) / 100;
      ...
      monthsMapping.forEach(m => {
        ...
        const empShare = txn
          ? txn.employeeContribution + txn.vpfAmount - withdrawalAmt / 2
          : 0;
        const emprShare = txn
          ? txn.employerEpfContribution - withdrawalAmt / 2
          : 0;
        ...
        yearEmpInterest +=
          Math.round(((empShare * m.monthsConsider * yearRate) / 1200) * 100) /
          100;
        yearEmprInterest +=
          Math.round(((emprShare * m.monthsConsider * yearRate) / 1200) * 100) /
          100;
      });

      currentEmpClosing = currentEmpClosing + yearEmpContrib + yearEmpInterest;
      currentEmprClosing =
        currentEmprClosing + yearEmprContrib + yearEmprInterest;
    }
    ...
    const _openingInterestEmp =
      Math.round(((openingEmployeeShare * 12 * activeRate) / 1200) * 100) / 100;
    const _openingInterestEmpr =
      Math.round(((openingEmployerShare * 12 * activeRate) / 1200) * 100) / 100;
    ...
    const selectedYearTxns = sortedAll.filter(
      t => getFinancialYear(t.transactionDate) === financialYear
    );
    ...
    monthsMapping.forEach(m => {
      ...
      const primaryTxn =
        groupedTxn && groupedTxn.txns.length > 0 ? groupedTxn.txns[0] : null;

      const withdrawalAmt = groupedTxn ? groupedTxn.withdrawalAmount : 0;
      const empShare = groupedTxn
        ? groupedTxn.employeeContribution +
          groupedTxn.vpfAmount -
          withdrawalAmt / 2
        : 0;
      const emprShare = groupedTxn
        ? groupedTxn.employerEpfContribution - withdrawalAmt / 2
        : 0;

      const empInterest =
        Math.round(((runningEmpBalance * activeRate) / 1200) * 100) / 100;
      const emprInterest =
        Math.round(((runningEmprBalance * activeRate) / 1200) * 100) / 100;
      ...
      const remarksList = groupedTxn
        ? groupedTxn.txns.map(t => t.remarks).filter(Boolean)
        : [];
      ...
      rows.push({
        ...
        details: primaryTxn
          ? primaryTxn.withdrawalAmount
            ? 'EPF Withdrawal'
            : primaryTxn.mode === 'AUTO_SALARY'
              ? 'Auto Salary Split'
              : 'Manual Entry'
          : null,
        remarks,
    ...
    const sumEmpShare = rows.reduce(
      (sum, r) => sum + (r.isOpening ? 0 : r.employeeShare || 0),
      0
    );
    const sumEmprEpf = rows.reduce(
      (sum, r) => sum + (r.isOpening ? 0 : r.employerEpf || 0),
      0
    );
    const sumEmprEps = rows.reduce(
      (sum, r) => sum + (r.isOpening ? 0 : r.employerEps || 0),
      0
    );
    const sumVpf = rows.reduce(
      (sum, r) => sum + (r.isOpening ? 0 : r.vpf || 0),
      0
    );
  ...
  const computedSummary = useMemo(() => {
    if (!financialYear && !modeFilter) {
      return {
        ...
        totalEmployerEpfContribution:
          summaryData?.totalEmployerEpfContribution || 0,
        totalEmployerEpsContribution:
          summaryData?.totalEmployerEpsContribution || 0,
    ...
    if (financialYear) {
      if (interestSheet) {
        accruedInterest =
          interestSheet.totalEmpInterest + interestSheet.totalEmprInterest;
    ...
    return {
      currentEpfBalance:
        latestEpfBalance || summaryData?.currentEpfBalance || 0,
      currentEpsBalance:
        latestEpsBalance || summaryData?.currentEpsBalance || 0,
  ...
  const fyOptions = useMemo(
    () => generateFinancialYearOptions(allTransactions),
    [allTransactions]
  );
      ...
      <header className='pb-6 border-b border-hairline flex flex-col md:flex-row md:items-end justify-between gap-6'>
        <div className='space-y-3'>
          ...
          <h1 className='display-serif text-[40px] md:text-[56px] text-foreground leading-none'>
            Employee Provident{' '}
            <span className='italic text-[hsl(var(--accent))]'>Fund</span>
          </h1>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-3 mt-4 pt-4 border-t border-border/50'>
            <div>
              ...
              <p className='font-mono text-lg font-bold text-foreground'>
                {formatCurrency(computedSummary.currentEpfBalance)}
              </p>
            </div>
            <div>
              <p className='eyebrow text-muted-foreground'>
              ...
              <p className='font-mono text-base font-semibold text-muted-foreground'>
                {formatCurrency(computedSummary.currentEpsBalance)}
              </p>
            </div>
            <div>
              <p className='eyebrow text-muted-foreground'>Employee Share</p>
              <p className='font-mono text-xs font-medium text-foreground'>
                {formatCurrency(computedSummary.totalEmployeeContribution)}
              </p>
            </div>
            <div>
              <p className='eyebrow text-muted-foreground'>
              ...
              <p className='font-mono text-xs font-medium text-[hsl(var(--gain))]'>
                {formatCurrency(computedSummary.totalEmployerEpfContribution)}
              </p>
            </div>
            <div>
              <p className='eyebrow text-muted-foreground'>
              ...
              <p className='font-mono text-xs font-medium text-[hsl(var(--accent))]'>
                {formatCurrency(computedSummary.totalEmployerEpsContribution)}
              </p>
            </div>
            <div>
              <p className='eyebrow text-muted-foreground'>
              ...
              <p
                className='font-mono text-xs font-semibold text-[hsl(var(--accent))]'
                title='Uncredited live projection for current financial year'
              >
        ...
        <div className='flex flex-wrap gap-2.5'>
            ...
            onClick={async () => {
              ...
              try {
                ...
                toast({
                  ...
                  description: 'EPF ledger downloaded as Excel (.xlsx).',
                });
              } catch (err) {
          ...
          <button
            onClick={openCreate}
            className='ed-btn ed-btn-accent flex items-center gap-1.5'
          >
        ...
        <div className='p-4 rounded-sm border border-[hsl(var(--chart-4))]/40 bg-[hsl(var(--chart-4))]/10 flex items-start gap-3'>
          ...
          <div className='space-y-0.5 text-[12px]'>
            ...
            <p className='text-muted-foreground leading-relaxed'>
              Your total employee contributions (including VPF) for the current
              financial year exceed ₹2,50,000. Under statutory IT rules,
              interest earned on the contribution amount exceeding ₹2.5L is
                ...
                <table className='w-full text-left border-collapse text-[11px] font-mono'>
                  <thead>
                    <tr className='border-b border-border bg-muted/30'>
                      <th className='py-3 px-2 font-mono text-[9px] uppercase tracking-[0.05em] text-muted-foreground'>
                        Month
                      </th>
                      <th className='py-3 px-2 font-mono text-[9px] uppercase tracking-[0.05em] text-muted-foreground'>
                        Date
                      ...
                      <th className='py-3 px-2 font-mono text-[9px] uppercase tracking-[0.05em] text-muted-foreground text-right'>
                        Basic + DA
                      </th>
                      <th className='py-3 px-2 font-mono text-[9px] uppercase tracking-[0.05em] text-muted-foreground text-right'>
                        Employee Share
                      ...
                      <th className='py-3 px-2 font-mono text-[9px] uppercase tracking-[0.05em] text-muted-foreground text-center'>
                        Rate
                      </th>
                      <th className='py-3 px-2 font-mono text-[9px] uppercase tracking-[0.05em] text-muted-foreground text-right'>
                        Emp Interest
                  ...
                  <tbody>
                      ...
                      return (
                        <tr
                          ...
                          onClick={e => {
                            ...
                            if (e.target.closest('.interactive-txn-badge'))
                              return;
                          ...
                          className={cn(
                            ...
                            row.isOpening
                              ? 'bg-[hsl(var(--accent))]/5 font-semibold text-muted-foreground'
                        ...
                        >
                              ...
                              <div className='flex flex-col gap-1.5'>
                                {row.txns.map(txn => {
                                  ...
                                  return (
                                    <div
                                      key={txn.id}
                                      className='py-1 text-[11px] font-mono leading-tight'
                                    >
                                      {formattedDate}
                                      {txn.remarks && (
                                ...
                                {row.txns.map(txn => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className='flex items-center justify-between'>
                                        <span
                                          className={cn(
                                            ...
                                            isW
                                              ? 'text-[hsl(var(--loss))] font-semibold'
                                              : 'text-foreground group-hover/item:text-[hsl(var(--accent))]'
                                          )}
                                        >
                                          {isW
                                            ? `Withdrawal: -${formatCurrency(txn.withdrawalAmount)}`
                                      ...
                                      {txn.remarks && (
                                        <span
                                          className='text-[9px] text-muted-foreground italic mt-0.5 block truncate max-w-[180px]'
                                          title={txn.remarks}
                                        >
                                          {txn.remarks}
                                        </span>
                          ...
                          <td className='py-3 px-2 text-right font-medium text-foreground'>
                            {row.isOpening ? (
                              ...
                              <div className='flex flex-col items-end'>
                                ...
                                <span className='text-[hsl(var(--loss))] font-medium'>
                                  -{formatCurrency(row.withdrawalAmount / 2)}
                                </span>
                              </div>
                            ) : row.employeeShare > 0 ? (
                              formatCurrency(row.employeeShare)
                          ...
                          <td className='py-3 px-2 text-right text-[hsl(var(--gain))]'>
                            {row.isOpening ? (
                              ...
                              <div className='flex flex-col items-end'>
                                {row.employerEpf > 0 && (
                                  <span className='text-[hsl(var(--gain))]'>
                          ...
                          <td className='py-3 px-2 text-right text-[hsl(var(--accent))]'>
                            {row.isOpening
                              ? '-'
                              : hasTxn
                                ? formatCurrency(row.employerEps)
                                : '-'}
                          </td>
                          <td className='py-3 px-2 text-right text-muted-foreground'>
                            {row.isOpening
                              ? '-'
                              : row.vpf
                                ? formatCurrency(row.vpf)
                                : '-'}
                          </td>
                          <td className='py-3 px-2 text-right font-bold text-foreground'>
                            {row.isOpening
                              ? formatCurrency(row.empShare + row.emprShare)
                              : hasTxn
                                ? formatCurrency(row.epfBalance)
                                : '-'}
                          </td>
                    ...
                    <tr className='bg-[hsl(var(--accent))]/5 border-b border-border font-semibold'>
                      <td
                        className='py-3 px-2 text-muted-foreground font-semibold'
                        colSpan={4}
                      >
                        Total Contributions (FY)
                      </td>
                      <td className='py-3 px-2 text-right font-semibold'>
                        {formatCurrency(interestSheet.sumEmpShare)}
                      </td>
                      <td className='py-3 px-2 text-right text-[hsl(var(--gain))] font-semibold'>
                        {formatCurrency(interestSheet.sumEmprEpf)}
                      </td>
                      <td className='py-3 px-2 text-right text-[hsl(var(--accent))] font-semibold'>
                        {formatCurrency(interestSheet.sumEmprEps)}
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-semibold'>
                        {formatCurrency(interestSheet.sumVpf)}
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-semibold'>
                        -
                      </td>
                      ...
                      <td className='py-3 px-2 text-center text-muted-foreground font-semibold'>
                        -
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-semibold'>
                        -
                    ...
                    <tr className='bg-[hsl(var(--accent))]/5 border-b border-border font-semibold text-[hsl(var(--accent))]'>
                      <td className='py-3 px-2 font-semibold' colSpan={4}>
                        Interest Credited (FY)
                      </td>
                      ...
                      <td className='py-3 px-2 text-right font-bold'>
                        {formatCurrency(interestSheet.totalInterestAdded)}
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-semibold'>
                        -
                      </td>
                      ...
                      <td className='py-3 px-2 text-right font-semibold'>
                        {formatCurrency(interestSheet.totalEmpInterest)}
                      </td>
                      <td className='py-3 px-2 text-right font-semibold'>
                        {formatCurrency(interestSheet.totalEmprInterest)}
                      </td>
                    </tr>
                    {/* TDS Row */}
                    ...
                    <tr className='bg-[hsl(var(--accent))]/10 border-b-2 border-border font-bold'>
                      <td
                        className='py-3 px-2 text-muted-foreground font-bold'
                        colSpan={4}
                      >
                        Closing Balance
                      </td>
                      <td className='py-3 px-2 text-right font-bold'>
                        {formatCurrency(interestSheet.closingEmp)}
                      </td>
                      <td className='py-3 px-2 text-right text-[hsl(var(--gain))] font-bold'>
                        {formatCurrency(interestSheet.closingEmpr)}
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-bold'>
                        -
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-bold'>
                        -
                      ...
                      <td className='py-3 px-2 text-right font-bold'>
                        {formatCurrency(
                          interestSheet.closingEmp + interestSheet.closingEmpr
                        )}
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-bold'>
                        -
                      </td>
                      <td className='py-3 px-2 text-center text-muted-foreground font-bold'>
                        -
                      </td>
                      <td className='py-3 px-2 text-right text-muted-foreground font-bold'>
                        -
                    ...
                    <tr className='bg-[hsl(var(--accent))]/15 font-black text-sm'>
                      <td
                        className='py-4 px-2 text-left text-muted-foreground font-bold'
                        colSpan={8}
                      >
                        EPF Balance
                      </td>
                      <td
                        className='py-4 px-2 text-right font-black text-[13px] text-foreground'
                        colSpan={1}
                      >
                ...
                <table className='w-full text-left border-collapse'>
                  ...
                  <tbody>
                    {transactions.map(txn => {
                      const isW =
                        !!txn.withdrawalAmount && txn.withdrawalAmount > 0;
                      return (
                        <tr
                          key={txn.id}
                        ...
                        >
                          <td className='py-3 px-4 text-[13px] font-mono whitespace-nowrap'>
                            {txn.transactionDate}
                          </td>
                          ...
                          <td className='py-3 px-4 text-right font-mono text-[12px] font-medium text-foreground'>
                            {isW
                              ? '-'
                              : formatCurrency(txn.employeeContribution)}
                          </td>
                          <td className='py-3 px-4 text-right font-mono text-[12px] text-[hsl(var(--gain))]'>
                            {isW
                              ? '-'
                              : formatCurrency(txn.employerEpfContribution)}
                          </td>
                          <td className='py-3 px-4 text-right font-mono text-[12px] text-[hsl(var(--accent))]'>
                            {isW
                              ? '-'
                              : formatCurrency(txn.employerEpsContribution)}
                          </td>
                          <td className='py-3 px-4 text-right font-mono text-[12px] text-muted-foreground'>
                            {txn.vpfAmount
                              ? formatCurrency(txn.vpfAmount)
                              : '-'}
                          </td>
