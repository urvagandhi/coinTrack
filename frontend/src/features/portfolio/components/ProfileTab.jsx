...
import { useBrokerConnection } from '@/features/brokers';
...
function StatusPill({ broker }) {
  ...
  const isExpired =
    !isActive &&
    broker.lastStatus !== null &&
    broker.lastStatus !== 'NEVER_SYNCED';
  ...
  if (needsReconnect)
    return <span className='ed-pill ed-pill-loss'>Reconnect needed</span>;
      ...
      <div className='p-6'>
        ...
        <div className='mb-6'>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-3 divide-x divide-border border-y border-border'>
            <div className='px-4 py-3'>
              ...
              <p className='text-[13px] font-medium text-foreground'>
                {relativeTime(broker.lastSuccessAt) || 'Never'}
              </p>
              {broker.lastSuccessAt && (
                <p className='text-[10px] font-mono text-muted-foreground/70 mt-0.5'>
                  {formatDateTime(broker.lastSuccessAt)}
                </p>
              )}
            </div>
            <div className='px-4 py-3'>
              ...
              <p
                className={cn('text-[13px] font-medium', {
                  ...
                  'text-[hsl(var(--chart-4))]':
                    broker.lastStatus === 'PARTIAL_FAILURE',
                  'text-muted-foreground':
                    !broker.lastStatus || broker.lastStatus === 'NEVER_SYNCED',
                })}
              >
                {broker.lastStatus === 'SUCCESS'
                  ? 'Success'
                  : broker.lastStatus === 'FAILURE'
                    ? 'Failed'
                    : broker.lastStatus === 'PARTIAL_FAILURE'
                      ? 'Partial'
                      : broker.lastStatus === 'NEVER_SYNCED'
          ...
          {broker.lastError && (
            <div className='mt-3 flex items-start gap-2 p-3 border-l-2 border-[hsl(var(--loss))] bg-[hsl(var(--loss))]/8'>
              ...
              <p className='text-[11px] text-[hsl(var(--loss))]'>
                {broker.lastError}
              </p>
            </div>
          )}
        ...
        {needsReconnect && (
          <div className='mb-4 flex items-start gap-2 p-3 border-l-2 border-[hsl(var(--chart-4))] bg-[hsl(var(--chart-4))]/8'>
            ...
            <p className='text-[11px] text-foreground/80'>
              Your {meta.name} session expired or was rejected. Credentials are
              still saved — just reauthorize to resume syncing.
            </p>
          </div>
        )}
        ...
        <div className='flex flex-wrap items-center gap-2 pt-5 border-t border-hairline'>
          {needsReconnect && meta.slug && (
            ...
            >
              <RefreshCw
                className={cn('h-3 w-3', reconnecting && 'animate-spin')}
              />
          ...
          <Link href='/portfolio?tab=holdings'>
            <button className='ed-btn ed-btn-ghost'>View Holdings</button>
          </Link>
        </div>
      </div>
    </article>
    ...
    return (
      <section className='ed-card relative px-8 py-14 text-center'>
        ...
        <Link2
          ...
          strokeWidth={1.5}
        />
        <h3 className='font-serif italic text-[24px] text-foreground mb-1'>
          No brokers on file.
        </h3>
        <p className='text-[12px] text-muted-foreground max-w-xs mx-auto mb-5'>
          Connect a brokerage account to view portfolio data, sync holdings, and
          track performance.
        </p>
        <Link href='/brokers'>
    ...
    <div className='space-y-5'>
      <div className='flex items-baseline justify-between pb-4 border-b border-border'>
        <div className='flex items-baseline gap-3'>
          ...
          <span className='index-num tnum'>
            [{String(brokers.length).padStart(2, '0')}]
          </span>
        </div>
      </div>
      ...
      <p className='font-serif italic text-[11px] text-muted-foreground text-center pt-2'>
        Zerodha sessions expire daily at 06:00 IST. Reconnect to refresh
        authorization.
      </p>
    </div>
  );
