...
const STATUS_STYLE = {
  ...
  PAUSED:
    'text-[hsl(var(--chart-4))] border-[hsl(var(--chart-4))]/30 bg-[hsl(var(--chart-4))]/10',
  CANCELLED:
    'text-[hsl(var(--loss))] border-[hsl(var(--loss))]/30 bg-[hsl(var(--loss))]/8',
};

export default function MfSipList({
  sips,
  unlinkedOrders,
  isLoading,
  onNavigate,
  ...
  if (!sips || sips.length === 0) {
    return (
      <section className='ed-card relative px-8 py-14 text-center'>
        ...
        <PiggyBank
          className='h-6 w-6 text-muted-foreground mx-auto mb-3'
          strokeWidth={1.5}
        />
        <p className='font-serif italic text-[20px] text-foreground'>
          No active SIPs.
        </p>
        <p className='text-[12px] text-muted-foreground mt-1'>
          Connect Zerodha Coin to begin tracking systematic plans.
        </p>
      </section>
    );
  }
      ...
      <section className='ed-card relative'>
        ...
        <header className='px-6 py-5 border-b border-hairline flex items-center justify-between'>
          <div className='flex items-baseline gap-3'>
            ...
            <h3 className='font-serif text-[20px] text-foreground leading-none'>
              Systematic Plans
            </h3>
          ...
          <table className='ed-table'>
            ...
            <tbody>
              {sips.map((sip, idx) => {
                ...
                const next = sip.nextInstalmentDate
                  ? new Date(sip.nextInstalmentDate)
                  : null;
                ...
                return (
                  <Fragment key={sip.sipId || idx}>
                    <tr
                      className={cn(
                        'cursor-pointer',
                        isExpanded && 'bg-[hsl(var(--accent-muted))]/30'
                      )}
                      onClick={() => setExpanded(isExpanded ? null : sip.sipId)}
                    >
                      <td className='pl-6 w-8'>
                        {isExpanded ? (
                          <ChevronDown className='h-3.5 w-3.5 text-[hsl(var(--accent))]' />
                        ) : (
                      ...
                      <td>
                        <p
                          className='font-medium text-foreground text-[13px] tracking-tight line-clamp-1'
                          title={sip.fund}
                        >
                          {sip.fund}
                        </p>
                        <p className='text-[10px] font-mono text-muted-foreground/70 mt-1'>
                          {sip.folio || '—'} · {sip.tradingSymbol || '—'} · ID{' '}
                          {sip.sipId}
                        </p>
                      </td>
                      <td className='text-right'>
                        <p className='font-mono tnum text-foreground font-medium'>
                          {formatCurrency(sip.instalmentAmount)}
                        </p>
                        <p className='eyebrow mt-0.5'>{sip.frequency}</p>
                      </td>
                      <td className='text-right'>
                        <p className='font-mono tnum text-[12px]'>
                          {next
                    ...
                    {isExpanded && (
                      <tr>
                        <td
                          colSpan='6'
                          className='bg-muted/50 px-6 py-5 border-y border-hairline'
                        >
                          <div className='ml-2 pl-4 border-l-2 border-[hsl(var(--accent))]/40'>
                            ...
                            {!sip.executions || sip.executions.length === 0 ? (
                              <p className='text-[12px] font-serif italic text-muted-foreground py-2'>
                              ...
                              <table className='w-full text-[12px]'>
                                <thead>
                                  <tr className='text-muted-foreground'>
                                    <th className='text-left pb-2 font-medium uppercase tracking-[0.1em] text-[10px]'>
                                      Date
                                    </th>
                                    <th className='text-right pb-2 font-medium uppercase tracking-[0.1em] text-[10px]'>
                                      Amount
                                    </th>
                                    <th className='text-right pb-2 font-medium uppercase tracking-[0.1em] text-[10px]'>
                                      NAV
                                    </th>
                                    <th className='text-center pb-2 font-medium uppercase tracking-[0.1em] text-[10px]'>
                                      Status
                                    </th>
                                    <th className='text-right pb-2 font-medium uppercase tracking-[0.1em] text-[10px]'>
                                      Order Ref
                                ...
                                <tbody>
                                  {sip.executions.map((exec, eIdx) => (
                                    ...
                                    >
                                      ...
                                      <td className='py-2 text-right font-mono tnum text-foreground'>
                                      ...
                                      <td className='py-2 text-right font-mono text-[10px] text-muted-foreground/70'>
                                        {exec.orderId}
                                      </td>
                                    </tr>
                                  ))}
      ...
      {unlinkedOrders && unlinkedOrders.length > 0 && (
        <section className='ed-card relative px-6 py-5'>
          ...
          <div className='flex items-baseline gap-3 mb-2'>
            ...
            <h3 className='font-serif text-[16px] text-foreground leading-none'>
              Unlinked SIP Orders
            </h3>
          </div>
          <p className='text-[12px] text-muted-foreground font-serif italic'>
            {unlinkedOrders.length} order
            {unlinkedOrders.length !== 1 ? 's' : ''} could not be matched to an
            active SIP plan.
          </p>
        </section>
      )}
