...
const eventTheme = type => {
  ...
  if (
    ...
    type === 'SIP_STATUS_CANCELLED'
  ) {
    return {
      color: 'text-[hsl(var(--loss))]',
      accent: 'border-[hsl(var(--loss))]',
  ...
  if (type.includes('HOLDING')) {
    return {
      color: 'text-[hsl(var(--neutral))]',
      accent: 'border-[hsl(var(--neutral))]',
      icon: type === 'HOLDING_INCREASED' ? ArrowUpRight : ArrowDownRight,
    };
  }
  return {
    color: 'text-muted-foreground',
...
export default function MfTimeline({
  events,
  isLoading,
  onNavigate,
  initialContext,
  ...
  const [highlighted, setHighlighted] = useState(
    initialContext?.highlightSipId || null
  );
          ...
          {grouped.map(([month, items], gi) => (
            ...
            >
              <div className='px-6 py-3 border-b border-border bg-muted/40 flex items-baseline gap-3'>
                ...
                <h4 className='font-serif text-[18px] italic text-foreground leading-none'>
                  {month}
                </h4>
                <span className='ml-auto eyebrow tnum'>
                  ...
                  return (
                    <li key={key}>
                      <button
                        type='button'
                        onClick={() =>
                          hasDetail && setExpandedKey(isOpen ? null : key)
                        }
                        className={cn(
                          'w-full text-left flex gap-4 px-6 py-4 transition-colors',
                          !isLast && 'border-b border-border',
                          isHighlight &&
                            'bg-[hsl(var(--accent-muted))]/40 border-l-2 border-l-[hsl(var(--accent))]',
                          hasDetail &&
                            'hover:bg-[hsl(var(--accent-muted))]/30 cursor-pointer',
                      ...
                      >
                        <div className='flex flex-col items-center flex-shrink-0 pt-0.5'>
                          ...
                          {!isLast && (
                            <span className='flex-1 w-px bg-border mt-1 min-h-[12px]' />
                          )}
                        </div>
                        <div className='flex-1 min-w-0'>
                          <div className='flex items-baseline justify-between gap-3 mb-1'>
                            <p
                              className={cn(
                                'text-[13px] font-semibold tracking-tight inline-flex items-center gap-2',
                                theme.color
                              )}
                            >
                          ...
                          <div className='flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1.5 text-[11px] text-muted-foreground'>
                            ...
                            {e.quantity != null && (
                              <span className='font-mono tnum'>
                                {Number(e.quantity).toFixed(3)} units
                              </span>
                            )}
                            {e.sipId && (
                              <span className='font-mono tnum'>
                          ...
                          <dl className='grid grid-cols-2 sm:grid-cols-4 gap-4 text-[12px]'>
                            {e.fund && (
                              <div className='col-span-2 sm:col-span-4'>
                                ...
                                <dd className='text-foreground font-serif'>
                                  {e.fund}
                                </dd>
                              </div>
                            )}
