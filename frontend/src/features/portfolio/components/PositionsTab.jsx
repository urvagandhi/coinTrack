...
import { usePortfolioPositions } from '@/features/portfolio';
import { formatCurrency } from '@/shared/formatters';
import { getShortCompanyName } from '@/features/portfolio';
  ...
  const equity = useMemo(
    () =>
      positions.filter(
        p =>
          !p.isDerivative &&
          !p.instrumentType?.includes('FUT') &&
          !p.instrumentType?.includes('OPT')
      ),
    [positions]
  );
  const deriv = useMemo(
            ...
            <table className='ed-table'>
              ...
              <tbody>
                {active.map((p, i) => {
                  ...
                  const stockName = getShortCompanyName(
                    p.tradingsymbol || p.symbol
                  );
                  const isExpiringSoon =
                    p.expiry && new Date(p.expiry) - new Date() < 2 * 86400000;
                  const isExpiringWeek =
                    p.expiry && new Date(p.expiry) - new Date() < 7 * 86400000;
                  const positionType =
                    ...
                    <tr key={`${p.symbol || p.tradingsymbol}-${broker}-${i}`}>
                      <td>
                        <div className='flex items-baseline gap-3'>
                          <span className='index-num tnum w-6'>
                            {String(i + 1).padStart(2, '0')}
                          </span>
                          <div className='flex flex-col'>
                            <span className='font-medium text-foreground tracking-tight'>
                              {stockName || p.tradingsymbol || p.symbol}
                            </span>
                            <span
                              className={cn(
                      ...
                      <td>
                        <span className='inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em] font-medium text-muted-foreground'>
                          <span
                            ...
                            style={{
                              background: BROKER_VAR[broker] || 'currentColor',
                            }}
                          />
                          ...
                          <td>
                            <span
                              className={cn(
                                ...
                                p.instrumentType?.includes('FUT')
                                  ? 'bg-[hsl(var(--neutral))]/10 text-[hsl(var(--neutral))] border-[hsl(var(--neutral))]/30'
                                  : p.optionType === 'CE'
                      ...
                      <td className='text-right font-mono tnum'>
                        {Math.abs(p.quantity ?? p.netQuantity ?? 0)}
                      </td>
                      <td className='text-right font-mono tnum text-muted-foreground'>
                        {formatCurrency(p.averagePrice ?? p.buyAverage)}
                      </td>
                      <td className='text-right font-mono tnum text-foreground'>
                        {formatCurrency(p.lastPrice ?? p.currentPrice)}
                      </td>
                      <td className='text-right'>
                        <span
                          className={cn(
                            'font-mono tnum text-[13px] font-medium',
                            pnlPos
                              ? 'text-[hsl(var(--gain))]'
                              : 'text-[hsl(var(--loss))]'
                          )}
                      ...
                      <td className='text-right pr-6'>
                        <span
                          className={cn(
                            'font-mono tnum text-[12px] font-medium',
                            dayPos
                              ? 'text-[hsl(var(--gain))]'
                              : 'text-[hsl(var(--loss))]'
                          )}
