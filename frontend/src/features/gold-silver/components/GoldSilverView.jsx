...
import GoldSilverDialog from './GoldSilverDialog';
import MarketRateDialog from './MarketRateDialog';
import RateSettingsDialog from './RateSettingsDialog';
import RateDisclosureBanner from './RateDisclosureBanner';
...
import { goldSilverAPI } from '@/shared/api/client';
...
function StatusBadge({ status, highlight, daysToMaturity }) {
  if (status === 'ACTIVE' && (!highlight || highlight === 'NONE')) {
    return (
      <span className='inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono border bg-[hsl(var(--gain))]/10 text-[hsl(var(--gain))] border-[hsl(var(--gain))]/30'>
        ACTIVE
      </span>
    );
  }
  if (status === 'MATURED') {
    return (
      <span className='inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono border bg-blue-500/10 text-blue-500 border-blue-500/30'>
        MATURED
      </span>
    );
  }
  ...
  return (
    <span
      ...
      title={daysToMaturity ? `${daysToMaturity} days left` : ''}
    >
      {icon}
      {status}
    </span>
      ...
      <div>
        <div className='flex items-start justify-between mb-4'>
          <div>
            <div className='flex items-center gap-2 mb-1'>
              ...
              <RateModeBadge
                mode={item.rateSource}
                onClick={() => onToggleRateMode(item)}
              />
            </div>
            ...
            <p className='text-[12px] text-muted-foreground flex items-center gap-1.5 flex-wrap'>
              <span>
                {item.purchasedFrom} • {item.purchaseDate}
              </span>
              {item.remarks && (
        ...
        <div className='grid grid-cols-2 gap-4 pb-3 border-b border-border/60'>
          ...
          <div>
            ...
            {item.currentValue ? (
              <div
                className={cn(
                  'flex items-center gap-1 text-[13px] font-mono font-medium',
                  isProfit
                    ? 'text-[hsl(var(--gain))]'
                    : 'text-[hsl(var(--loss))]'
                )}
        ...
        {item.maturityDate && (
          <div className='pt-2 text-[11px] font-mono text-muted-foreground'>
            Maturity:{' '}
            <span className='text-foreground'>{item.maturityDate}</span>
          </div>
        )}
      </div>
              ...
              >
                ...
                <td className='py-3 px-4'>
                  <div className='flex items-center gap-1.5 flex-wrap'>
                    <span
                      className={cn(
                        'px-1 rounded-sm text-[9px] font-mono border',
                        item.metalType === 'GOLD'
                          ? 'bg-amber-500/10 text-amber-600 border-amber-500/30'
                          : 'bg-slate-500/10 text-slate-400 border-slate-500/30'
                      )}
                    ...
                    <span className='text-[10px] font-mono text-muted-foreground'>
                      {item.purityLabel || `${item.purity}K`}
                    </span>
                    <RateModeBadge
                      mode={item.rateSource}
                      onClick={() => onToggleRateMode(item)}
                ...
                <td className='py-3 px-4 text-right'>
                  {item.currentValue ? (
                    <div
                      className={cn(
                        'flex flex-col items-end text-[12px] font-mono font-medium',
                        item.profitLoss >= 0
                          ? 'text-[hsl(var(--gain))]'
                          : 'text-[hsl(var(--loss))]'
                      )}
...
export function GoldSilverView() {
  ...
  const [marketRateDialogState, setMarketRateDialogState] = useState({
    isOpen: false,
    metalType: 'GOLD',
  });
  ...
  const { data: currentRates, refetch: _refetchRates } = useQuery({
  ...
  const totalPages =
    ...
    (Array.isArray(pageData) ? 1 : 0);
  const totalElements =
    pageData?.totalElements ?? pageData?.page?.totalElements ?? items.length;
  ...
  const updateMarketRateMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'Manual Rates Updated', variant: 'success' });
      invalidate();
    },
    onError: onErr,
  });
  const updateRateModeMutation = useMutation({
    mutationFn: ({ id, rateSource }) =>
      goldSilverAPI.updateRateMode(id, { rateSource }),
    onSuccess: () => {
      toast({ title: 'Rate Mode Updated', variant: 'success' });
      invalidate();
    },
    onError: onErr,
  });
  ...
  const handleForceRefreshRates = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Live market prices fetched & holdings recomputed.',
        variant: 'success',
      });
      invalidate();
    } catch (err) {
      ...
      toast({
        ...
        description: err?.message || 'Failed to refresh rates.',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    <div className='space-y-8'>
      <header className='pb-6 border-b border-hairline flex flex-col md:flex-row md:items-end justify-between gap-6'>
        <div className='space-y-3'>
          ...
          <h1 className='display-serif text-[40px] md:text-[56px] text-foreground leading-none'>
            Gold &{' '}
            <span className='italic text-[hsl(var(--accent))]'>Silver</span>
          </h1>
              ...
              {summaryData.currentValue > 0 && (
                <div>
                  ...
                  >
                    <span>
                      {isTotalProfit ? '+' : ''}
                      {formatCurrency(summaryData.overallProfitLoss)}
                    </span>
                    <span className='text-[12px] font-medium mb-1'>
                      ({summaryData.overallReturnPercent}%)
                    </span>
                  </div>
                </div>
              )}
        ...
        <div className='flex flex-col gap-3'>
          <div className='flex flex-col sm:flex-row gap-3'>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align='end'
                className='w-52 bg-card border-border rounded-sm shadow-md z-50'
              >
                <DropdownMenuItem
                  onClick={() => setIsRateSettingsOpen(true)}
                  className='cursor-pointer text-foreground hover:bg-muted text-[12px]'
                >
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  toast({
                    ...
                    description: 'Records downloaded as Excel (.xlsx).',
                  });
                } catch (err) {
            ...
            <div className='flex justify-end gap-3 text-[11px] font-mono text-muted-foreground mt-1'>
              <span>
                ...
                <span className='text-amber-500 font-semibold'>
                  {formatWeight(summaryData.totalGoldWeight)} Au
                </span>
              </span>
              <span>•</span>
              <span>
                <span className='text-slate-400 font-semibold'>
                  {formatWeight(summaryData.totalSilverWeight)} Ag
                </span>
              </span>
            </div>
          )}
      ...
      <div className='ed-card p-4 bg-muted/20 border border-border flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4'>
        <div className='flex items-center gap-6 flex-wrap'>
          ...
          <div className='space-y-1'>
            <div className='flex items-center gap-2'>
              ...
              <span className='eyebrow text-amber-600 dark:text-amber-400 font-semibold'>
                Gold Live Rates
              </span>
              {goldSnapshot?.localPremiumPercent && (
                <span className='text-[10px] font-mono text-muted-foreground'>
                  (+{goldSnapshot.localPremiumPercent}% Local Premium)
                </span>
              )}
            </div>
            <div className='flex items-center gap-4 flex-wrap font-mono'>
              <div>
                ...
                <span className='text-[14px] font-bold text-foreground'>
                  {goldSnapshot?.effectiveBaseRate
                    ? `₹${goldSnapshot.effectiveBaseRate.toFixed(2)}/g`
                    : '—'}
                </span>
              </div>
              ...
              <div>
                ...
                <span className='text-[14px] font-bold text-foreground'>
                  {goldSnapshot?.effectiveBaseRate
                    ? `₹${(goldSnapshot.effectiveBaseRate * 0.916).toFixed(2)}/g`
                    : '—'}
                </span>
              </div>
              <div className='w-px h-6 bg-border/60' />
              <div>
                ...
                <span className='text-[14px] font-bold text-foreground'>
                  {goldSnapshot?.effectiveBaseRate
                    ? `₹${(goldSnapshot.effectiveBaseRate * 0.75).toFixed(2)}/g`
                    : '—'}
                </span>
              </div>
            </div>
          ...
          <div className='space-y-1'>
            <div className='flex items-center gap-2'>
              ...
              <span className='eyebrow text-slate-400 font-semibold'>
                Silver Live Rates
              </span>
              {silverSnapshot?.localPremiumPercent && (
                <span className='text-[10px] font-mono text-muted-foreground'>
                  (+{silverSnapshot.localPremiumPercent}% Local Premium)
                </span>
              )}
            </div>
            <div className='flex items-center gap-4 flex-wrap font-mono'>
              <div>
                ...
                <span className='text-[14px] font-bold text-foreground'>
                  {silverSnapshot?.effectiveBaseRate
                    ? `₹${silverSnapshot.effectiveBaseRate.toFixed(2)}/g`
                    : '—'}
                </span>
              </div>
              <div className='w-px h-6 bg-border/60' />
              <div>
                ...
                <span className='text-[14px] font-bold text-foreground'>
                  {silverSnapshot?.effectiveBaseRate
                    ? `₹${(silverSnapshot.effectiveBaseRate * 0.925).toFixed(2)}/g`
                    : '—'}
                </span>
              </div>
            </div>
        ...
        <div className='flex items-center gap-3 text-[11px] font-mono text-muted-foreground self-end lg:self-auto'>
          ...
          {goldSnapshot?.fetchedAt && (
            <span>
              Rate As Of: {new Date(goldSnapshot.fetchedAt).toLocaleString()}
            </span>
          )}
        </div>
      </div>
      ...
      <MarketRateDialog
        ...
        onClose={() =>
          setMarketRateDialogState({ isOpen: false, metalType: 'GOLD' })
        }
        onSave={async data => {
