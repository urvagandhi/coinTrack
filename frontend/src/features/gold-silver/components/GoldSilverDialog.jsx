...
import { Loader2, X, Calculator, RefreshCw } from 'lucide-react';
...
import { formatCurrency as formatIndianCurrency } from '@/shared/formatters';
...
export default function GoldSilverDialog({
  ...
  useEffect(() => {
    if (isOpen) {
      goldSilverAPI
        .getPurityOptions(formData.metalType)
    ...
    if (isOpen) {
      if (initialData) {
        setFormData({
          ...
          ratePerGram: initialData.ratePerGram
            ? String(initialData.ratePerGram)
            : '',
          netWeight: initialData.netWeight ? String(initialData.netWeight) : '',
          makingChargePercent: initialData.makingChargePercent
            ? String(initialData.makingChargePercent)
            : '',
          stoneOtherCharges: initialData.stoneOtherCharges
            ? String(initialData.stoneOtherCharges)
            : '',
          gstPercent:
            initialData.gstPercent !== undefined
              ? String(initialData.gstPercent)
              : '3.00',
          currentMarketRate: initialData.currentMarketRate
            ? String(initialData.currentMarketRate)
            : '',
  ...
  const handleSubmit = async e => {
    ...
    try {
      await onSave({
        ...
        makingChargePercent: formData.makingChargePercent
          ? Number(formData.makingChargePercent)
          : 0,
        stoneOtherCharges: formData.stoneOtherCharges
          ? Number(formData.stoneOtherCharges)
          : 0,
        gstPercent: formData.gstPercent ? Number(formData.gstPercent) : 3,
        currentMarketRate: formData.currentMarketRate
          ? Number(formData.currentMarketRate)
          : undefined,
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                    ...
                    <select
                      ...
                      onChange={e => {
                        ...
                        setFormData({
                          ...
                          purity: opt
                            ? String(opt.karatValue || opt.fineness || 24)
                            : formData.purity,
                        });
                      }}
                      ...
                      {purityOptions.map(opt => (
                        <option key={opt.id} value={opt.id}>
                          {opt.label ||
                            opt.displayName ||
                            opt.code ||
                            `Purity ${((opt.purityFactor || 1) * 100).toFixed(1)}%`}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <div className='flex items-center gap-2 p-1 border border-border rounded-sm bg-muted/20'>
                      <button
                        type='button'
                        onClick={() =>
                          setFormData({ ...formData, rateSource: 'LIVE' })
                        }
                        className={cn(
                          'flex-1 py-1 text-[11px] font-mono rounded-xs transition-colors text-center',
                          formData.rateSource === 'LIVE'
                            ? 'bg-[hsl(var(--accent))] text-white font-medium shadow-xs'
                      ...
                      <button
                        type='button'
                        onClick={() =>
                          setFormData({ ...formData, rateSource: 'MANUAL' })
                        }
                        className={cn(
                          'flex-1 py-1 text-[11px] font-mono rounded-xs transition-colors text-center',
                  ...
                  {formData.rateSource === 'LIVE' && (
                    <div className='sm:col-span-2 p-2.5 rounded-sm bg-blue-500/10 border border-blue-500/20 text-[11px] font-mono text-blue-600 dark:text-blue-400 flex items-center justify-between'>
                      <span>
                        ⓘ Powered by <strong>GoldAPI.io</strong> spot rate +
                        local duty/premium. Automatically recomputes value
                        daily.
                      </span>
                    </div>
                  )}
                  ...
                  {formData.rateSource === 'LIVE' && (
                    <div className='sm:col-span-2 p-2.5 rounded-sm border border-emerald-500/30 bg-emerald-500/5 text-[11px] font-mono text-emerald-600 dark:text-emerald-400 flex items-center gap-2'>
                      ...
                      <span>
                        Current value will automatically recompute off live
                        market rate scaled by purity factor.
                      </span>
                    </div>
                  )}
                </div>
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          stoneOtherCharges: e.target.value,
                        })
                      }
                      className='ed-input w-full font-mono'
                      placeholder='0.00'
            ...
            <div className='space-y-4 flex-1'>
              <div className='flex justify-between items-center text-[12px]'>
                ...
                <span className='font-mono text-foreground'>
                  {formatIndianCurrency(preview.metalAmount)}
                </span>
              </div>
              <div className='flex justify-between items-center text-[12px]'>
                <span className='text-muted-foreground'>Making Charges</span>
                <span className='font-mono text-foreground'>
                  +{formatIndianCurrency(preview.makingChargeAmount)}
                </span>
              </div>
              <div className='flex justify-between items-center text-[12px]'>
                <span className='text-muted-foreground'>Other Charges</span>
                <span className='font-mono text-foreground'>
                  +{formatIndianCurrency(formData.stoneOtherCharges || 0)}
                </span>
              </div>
              <div className='pt-2 border-t border-dashed border-border/60 flex justify-between items-center text-[12px]'>
                ...
                <span className='font-mono font-medium text-foreground'>
                  {formatIndianCurrency(preview.totalAmount)}
                </span>
              </div>
              <div className='flex justify-between items-center text-[12px]'>
                ...
                <span className='font-mono text-[hsl(var(--loss))]'>
                  +{formatIndianCurrency(preview.gstAmount)}
                </span>
              </div>
