...
const DEFAULT_DUTY_PREMIUM = 15.0; // 15% Statutory Import Duty
...
export default function RateSettingsDialog({ isOpen, onClose, onSaved }) {
    ...
    if (isOpen) {
      ...
      Promise.all([
        ...
        .then(([rates, settings]) => {
          ...
          const currentGoldPrem =
            settings?.goldLocalPremiumPercent ??
            goldSnap?.localPremiumPercent ??
            DEFAULT_DUTY_PREMIUM;
          const currentSilverPrem =
            settings?.silverLocalPremiumPercent ??
            silverSnap?.localPremiumPercent ??
            DEFAULT_DUTY_PREMIUM;
          ...
          setGoldRetailRate(
            (goldBase * (1 + currentGoldPrem / 100)).toFixed(2)
          );
          setSilverRetailRate(
            (silverBase * (1 + currentSilverPrem / 100)).toFixed(2)
          );
        })
        .finally(() => {
  ...
  const handleSubmit = async e => {
    ...
    if (isNaN(goldPremVal) || goldPremVal < 0)
      goldPremVal = DEFAULT_DUTY_PREMIUM;
    if (isNaN(silverPremVal) || silverPremVal < 0)
      silverPremVal = DEFAULT_DUTY_PREMIUM;
    ...
    try {
      ...
      toast({
        title: 'Market Rates Calibrated',
        description: 'Holdings recomputed using calibrated local rate anchor.',
        variant: 'success',
      });
      if (onSaved) onSaved();
      onClose();
          ...
          <div className='p-3 border border-border/80 rounded-sm bg-muted/20 text-[11px] font-mono text-muted-foreground leading-relaxed flex items-start gap-2'>
            ...
            <span>
              ...
              <strong className='text-foreground'>
                15.00% statutory import duty
              </strong>
              . Enter today's local jeweller 24K rate in your city (e.g.
              Goodreturns) to automatically calibrate live spot market feeds.
            </span>
          </div>

          {/* Calibration Input Toggle */}
          <div className='flex items-center justify-between bg-muted/30 p-1 border border-border rounded-sm'>
            <span className='eyebrow text-muted-foreground ml-2'>
              Input Mode:
            </span>
                ...
                <div className='space-y-1.5'>
                  <div className='flex justify-between items-center text-[11px] font-mono text-muted-foreground'>
                    ...
                    <span className='text-[hsl(var(--accent))]'>
                      Calculated Premium: +{goldPremium}%
                    </span>
                  </div>
                  <input
                    type='number'
                ...
                <div className='space-y-1.5'>
                  <div className='flex justify-between items-center text-[11px] font-mono text-muted-foreground'>
                    ...
                    <span className='text-foreground'>
                      Calibrated Rate: ₹{goldRetailRate}/g
                    </span>
                  </div>
                  <input
                    type='number'
              ...
              {inputMode === 'RATE' ? (
                <div className='space-y-1.5'>
                  <div className='flex justify-between items-center text-[11px] font-mono text-muted-foreground'>
                    ...
                    <span className='text-[hsl(var(--accent))]'>
                      Calculated Premium: +{silverPremium}%
                    </span>
                  </div>
                  <input
                    type='number'
                ...
                <div className='space-y-1.5'>
                  <div className='flex justify-between items-center text-[11px] font-mono text-muted-foreground'>
                    <span>Local Premium % over Spot Benchmark</span>
                    <span className='text-foreground'>
                      Calibrated Rate: ₹{silverRetailRate}/g
                    </span>
                  </div>
                  <input
                    type='number'
