...
import { ppfAPI } from '@/shared/api/client';
import { getFinancialYear } from '@/shared/formatters';
...
const CREDIT_PARTICULAR_TYPES = [
  {
    ...
    defaultText: 'UPI / Bank Transfer',
  },
  {
    value: 'INTEREST_CREDIT',
    label: 'Interest Credit',
    defaultText: 'Interest Credited',
  },
];
...
export default function PpfDialog({
    ...
    if (isOpen && !initialData && formData.transactionDate) {
      ...
      if (formData.type === 'CREDIT') {
        if (formData.particularType === 'INTEREST_CREDIT') {
          setFormData(prev => ({
            ...prev,
            remarks: `Annual Interest FY ${fy}`,
          }));
        } else if (formData.particularType === 'DEPOSIT') {
          setFormData(prev => ({
            ...prev,
            remarks: `Contribution for FY ${fy}`,
          }));
        }
      }
    }
  ...
  const availableParticularTypes =
  ...
  const handleTypeChange = newType => {
    setFormData(prev => {
      const validTypes =
        newType === 'CREDIT' ? CREDIT_PARTICULAR_TYPES : DEBIT_PARTICULAR_TYPES;
      const isCurrentValid = validTypes.some(
        pt => pt.value === prev.particularType
      );
      const nextParticularType = isCurrentValid
        ? prev.particularType
        : validTypes[0].value;
      const defaultText =
        validTypes.find(pt => pt.value === nextParticularType)?.defaultText ||
        '';
      ...
      return {
        ...
        particulars:
          !prev.particulars ||
          prev.particulars === 'Deposit' ||
          prev.particulars === 'Withdrawal'
            ? defaultText
            : prev.particulars,
      };
    });
  ...
  const handleParticularTypeChange = newParticularType => {
    setFormData(prev => {
      ...
      return {
        ...
        particulars:
          !prev.particulars ||
          CREDIT_PARTICULAR_TYPES.concat(DEBIT_PARTICULAR_TYPES).some(
            pt => pt.defaultText === prev.particulars
          )
  ...
  const handleSubmit = async e => {
    ...
    if (
      formData.type === 'DEBIT' &&
      formData.particularType === 'WITHDRAWAL' &&
      withdrawalStatus
    ) {
      if (!withdrawalStatus.withdrawalAllowed) {
        toast({
          ...
          description:
            withdrawalStatus.errorMessage ||
            'You are not eligible for a withdrawal.',
          variant: 'destructive',
        });
        return;
      }
      if (
        withdrawalStatus.maxWithdrawalAmount &&
        amt > withdrawalStatus.maxWithdrawalAmount
      ) {
        toast({
          title: 'Limit Exceeded',
          description: `Maximum allowed withdrawal is ${formatIndianCurrency(withdrawalStatus.maxWithdrawalAmount)}`,
          variant: 'destructive',
        });
        return;
      }
                ...
                {formData.amount &&
                  ...
                  Number(formData.amount) > 0 && (
                    <p className='text-[11px] font-mono text-[hsl(var(--accent))] mt-1'>
                      ...
                      {formatInIndianWords(formData.amount)
                        ? ` (${formatInIndianWords(formData.amount)})`
                        : ''}
                    </p>
                  )}
                  ...
                  withdrawalStatus && (
                    <div className='mt-2 p-2 bg-muted rounded-md text-xs border border-border'>
                      {!withdrawalStatus.withdrawalAllowed ? (
                        <p className='text-[hsl(var(--loss))] font-medium'>
                          ⚠️{' '}
                          {withdrawalStatus.errorMessage ||
                        ...
                        <p className='text-muted-foreground'>
                          ...
                          {withdrawalStatus.maxWithdrawalAmount > 0 && (
                            <span className='block mt-1 font-mono text-[hsl(var(--accent))]'>
                              Max Limit:{' '}
