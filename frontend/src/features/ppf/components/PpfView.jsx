...
import PpfDialog from './PpfDialog';
import PpfSettingsDialog from './PpfSettingsDialog';
...
export function PpfView() {
  ...
  const transactions = Array.isArray(txnData)
    ? txnData
    : (txnData?.content ?? []);
  const totalPages =
    txnData?.totalPages ??
          ...
          <div className='flex flex-wrap gap-x-8 gap-y-4 mt-4'>
            <div>
              <p className='eyebrow text-muted-foreground'>
                {financialYear
                  ? `FY ${financialYear} Ending Balance`
                  : 'Current Balance'}
              </p>
            ...
            <div>
              <p className='eyebrow text-muted-foreground'>
                {financialYear
                  ? `Deposits in FY ${financialYear}`
                  : 'Total Deposits'}
              </p>
              <p className='font-mono text-sm font-semibold text-[hsl(var(--gain))]'>
                {formatCurrency(computedSummary.totalDeposits)}
              </p>
            </div>
            <div>
              <p className='eyebrow text-muted-foreground'>
                {financialYear
                  ? `Interest Credited in FY ${financialYear}`
                  : 'Interest Credited'}
              </p>
              <p className='font-mono text-sm font-semibold text-[hsl(var(--accent))]'>
                {formatCurrency(computedSummary.totalInterestCredited)}
              </p>
            </div>
          </div>
        </div>
            ...
            onClick={async () => {
              ...
              try {
                ...
                toast({
                  ...
                  description: 'PPF ledger downloaded as Excel (.xlsx).',
                });
              } catch (err) {
