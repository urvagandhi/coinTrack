      ...
      <header className='border-b border-hairline relative z-10'>
        <div className='max-w-7xl mx-auto px-4 sm:px-8 py-4 flex items-center justify-between gap-4'>
          <Link href='/' className='flex items-center gap-2.5 group'>
            ...
            <span className='hidden sm:inline display-num text-[10px] text-muted-foreground ml-1'>
              VOL.04 · ERRATA
            </span>
          </Link>
        ...
        <div className='w-full max-w-6xl mx-auto'>
          <div className='grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10'>
            ...
            <aside className='lg:col-span-5 lg:border-l border-hairline lg:pl-10'>
              ...
              <p
                className='text-[12px] text-muted-foreground font-mono mb-6'
                suppressHydrationWarning
              >
                {authReady
                  ? `${(showAuthed ? ARCHIVED_AUTH : ARCHIVED_GUEST).length} entries — sorted by folio`
                  : 'Loading index…'}
              </p>

              <ul
                className='divide-y divide-hairline border-y border-hairline min-h-[180px]'
                suppressHydrationWarning
              >
                {authReady &&
                  (showAuthed ? ARCHIVED_AUTH : ARCHIVED_GUEST).map(item => (
                    <li key={item.href}>
                      <Link
                        href={item.href}
          ...
          <div className='mt-12 pt-5 border-t border-hairline grid grid-cols-1 sm:grid-cols-3 gap-3 text-[12px]'>
            ...
            <div className='sm:text-center'>
              ...
              <p className='font-mono text-foreground' suppressHydrationWarning>
                {refNo}
              </p>
            </div>
      ...
      <footer className='border-t border-hairline relative z-10'>
        <div className='max-w-7xl mx-auto px-4 sm:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2'>
          <p className='eyebrow' suppressHydrationWarning>
            &copy; {year} CoinTrack — Printed in Browser
          </p>
          <p className='text-[11px] text-muted-foreground font-mono'>
            If you believe this is in error, please{' '}
            <a
              href='mailto:support@cointrack.app'
