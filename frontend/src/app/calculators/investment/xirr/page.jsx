    ...
    >
      ...
      >
            ...
            <div key={index} className='flex gap-2 items-end'>
              ...
              <div className='flex-1'>
                <label className='text-xs text-muted-foreground'>
                  Amount (-ve for outflow)
                </label>
                <input
                  type='number'
                  value={cf.amount}
                  onChange={e =>
                    updateCashFlow(index, 'amount', e.target.value)
                  }
                  className='flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm'
                />
              </div>
        ...
        <div className='text-xs text-muted-foreground'>
          <strong>Tip:</strong> Use negative amounts for investments (outflows)
        ...
        {result && (
          <div className='space-y-6'>
            <div className='text-center p-4 bg-primary/10 rounded-lg'>
              ...
              <p
                className={`text-3xl font-bold ${result.xirr >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <ResultMetric
                ...
                value={formatPercentage(
                  (result.absoluteReturn / result.totalInvested) * 100
                )}
                variant={result.absoluteReturn >= 0 ? 'success' : 'error'}
              />
            </div>
