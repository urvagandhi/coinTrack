  ...
  return (
    <li className='flex gap-5 pb-6 last:pb-0 border-b border-border last:border-b-0'>
      <div className='flex flex-col items-center pt-1 flex-shrink-0'>
        ...
        <span className='index-num tnum mt-1 text-muted-foreground/60'>
        ...
        <div className='flex flex-col md:flex-row md:items-end gap-4'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              ...
              <p className='text-[13px] text-muted-foreground mt-2 font-serif italic'>
                {broker.tagline}
              </p>
            </div>
          </div>
        ...
        <div className='space-y-6'>
          <div className='flex items-baseline gap-3'>
            ...
            <span className='eyebrow ml-2'>
              {broker.setupSteps.length} steps
            </span>
          </div>
          ...
          <ol className='space-y-6'>
            {broker.setupSteps.map((step, i) => (
              <SetupStep
                ...
                number={i + 1}
                step={step}
                total={broker.setupSteps.length}
              />
            ))}
          </ol>
          ...
          <div className='ed-card relative px-5 py-4 flex items-start gap-3'>
            ...
            <ShieldCheck
              className='h-4 w-4 mt-0.5 flex-shrink-0 text-[hsl(var(--accent))]'
              strokeWidth={2}
            />
            <p className='text-[11.5px] leading-relaxed text-muted-foreground font-serif italic'>
              Your credentials are encrypted with AES-256-GCM. CoinTrack reads
              portfolio data only — placement, modification, or cancellation of
              orders is impossible by design.
            </p>
          </div>
        </div>
