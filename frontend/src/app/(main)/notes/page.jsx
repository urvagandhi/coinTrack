...
const NOTE_ACCENT = {
  default: {
    tint: 'hsl(var(--card))',
    stripe: 'hsl(var(--muted-foreground))',
    dot: 'hsl(var(--muted-foreground))',
  },
  rose: {
    tint: 'hsl(346 70% 50% / 0.14)',
    stripe: 'hsl(346 70% 50%)',
    dot: 'hsl(346 70% 50%)',
  },
  orange: {
    tint: 'hsl(24 92% 52% / 0.14)',
    stripe: 'hsl(24 92% 52%)',
    dot: 'hsl(24 92% 52%)',
  },
  amber: {
    tint: 'hsl(var(--accent) / 0.18)',
    stripe: 'hsl(var(--accent))',
    dot: 'hsl(var(--accent))',
  },
  lime: {
    tint: 'hsl(80 65% 45% / 0.16)',
    stripe: 'hsl(80 65% 45%)',
    dot: 'hsl(80 65% 45%)',
  },
  emerald: {
    tint: 'hsl(var(--gain) / 0.14)',
    stripe: 'hsl(var(--gain))',
    dot: 'hsl(var(--gain))',
  },
  cyan: {
    tint: 'hsl(189 75% 45% / 0.15)',
    stripe: 'hsl(189 75% 45%)',
    dot: 'hsl(189 75% 45%)',
  },
  blue: {
    tint: 'hsl(var(--neutral) / 0.15)',
    stripe: 'hsl(var(--neutral))',
    dot: 'hsl(var(--neutral))',
  },
  violet: {
    tint: 'hsl(265 65% 58% / 0.16)',
    stripe: 'hsl(265 65% 58%)',
    dot: 'hsl(265 65% 58%)',
  },
  fuchsia: {
    tint: 'hsl(310 75% 58% / 0.16)',
    stripe: 'hsl(310 75% 58%)',
    dot: 'hsl(310 75% 58%)',
  },
};
...
export default function NotesPage() {
  ...
  const allTags = ['all', ...new Set(notes.flatMap(n => n.tags || []))].slice(
    0,
    9
  );
  ...
  const optimistic = async updateFn => {
    ...
    const prev = queryClient.getQueryData([
      'notes',
      { page, search: committedSearch, tag: activeTag },
    ]);
    const old = Array.isArray(prev) ? prev : (prev?.content ?? []);
    const updated = updateFn(old);
    queryClient.setQueryData(
      ['notes', { page, search: committedSearch, tag: activeTag }],
      Array.isArray(prev) ? updated : { ...prev, content: updated }
    );
    return { prev };
  };
  ...
  const createMutation = useMutation({
    ...
    onMutate: n =>
      optimistic(old => [
        { ...n, id: `temp-${Date.now()}`, updatedAt: new Date().toISOString() },
        ...old,
      ]),
  ...
  const updateMutation = useMutation({
    ...
    onMutate: ({ id, data: d }) =>
      optimistic(old =>
        old.map(n =>
          n.id === id ? { ...n, ...d, updatedAt: new Date().toISOString() } : n
        )
      ),
      ...
      <header className='pb-6 border-b border-hairline flex items-end justify-between gap-6'>
        <div className='space-y-3'>
          ...
          <h1 className='display-serif text-[40px] md:text-[56px] text-foreground'>
            Field{' '}
            <span className='italic text-[hsl(var(--accent))]'>Notes</span>
          </h1>
          <p className='text-[13px] text-muted-foreground font-serif italic max-w-md'>
            {totalElements} entr{totalElements !== 1 ? 'ies' : 'y'} —
            observations, thesis, errata, ideas.
          </p>
        </div>
          ...
          <div>
            {pinned.length > 0 && (
              <div className='flex items-baseline gap-3 mb-4'>
                ...
                <span className='index-num tnum'>
                  [{String(unpinned.length).padStart(2, '0')}]
                </span>
              </div>
            )}
            <div className={gridClass}>
