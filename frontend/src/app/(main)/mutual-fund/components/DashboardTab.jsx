...
function PerformanceList({ title, data = [], isWorst: _isWorst = false }) {
  if (!data || data.length === 0) return null;
  return (
    <div className='ed-card p-5 relative overflow-hidden'>
      <h3 className='font-serif italic text-lg mb-4'>{title}</h3>
    ...
    return (
      <section className='ed-card p-12 text-center max-w-md mx-auto'>
        ...
        <p className='text-[12px] text-muted-foreground'>
          Start by adding a mutual fund scheme.
        </p>
      </section>
    );
  }
    ...
    <div className='space-y-6'>
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <AllocationCard
          ...
          data={summaryData.categoryAllocation}
        />
        <AllocationCard
          title='Platform Allocation'
          data={summaryData.platformAllocation}
        />
        <AllocationCard
          title='Bank Allocation'
          data={summaryData.bankAllocation}
        />
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        <PerformanceList
          ...
          data={summaryData.topPerformingFunds}
        />
        <PerformanceList
          title='Needs Attention'
          data={summaryData.worstPerformingFunds}
          isWorst={true}
        />
      </div>
    </div>
