...
export default function CompleteProfilePage() {
  ...
  return (
    <AuthPageShell
      ...
      maxWidth='md'
      asideQuote={
        '"Your identity is the key to a secure and personal financial ledger."'
      }
    >
            ...
            <AuthFormField label='Phone (+91)' id='phoneNumber'>
              <div className='relative'>
                <div className='absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 text-muted-foreground'>
                  <Phone size={14} />
                  <span className='text-[11px] font-mono border-l border-hairline pl-1.5'>
                    +91
                  </span>
                </div>
                <input
                ...
                <div className='mt-2.5 space-y-1.5'>
                  <div className='flex gap-1'>
                    {[1, 2, 3, 4].map(seg => (
                      <div
                        key={seg}
                        className={`h-[3px] flex-1 ${seg <= strength.segments ? STRENGTH_COLORS[strength.level] : 'bg-hairline'}`}
                      />
                    ))}
                  </div>
