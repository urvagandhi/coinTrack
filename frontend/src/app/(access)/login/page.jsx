...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    const wasRemembered =
      localStorage.getItem('cointrack_remember_me') === 'true';
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (/^\d{10}$/.test(credentials.usernameOrEmail)) {
        ...
        /^[\d\s\-\+\(\)]+$/.test(credentials.usernameOrEmail) &&
        /\d/.test(credentials.usernameOrEmail)
      ) {
        credentials.usernameOrEmail = credentials.usernameOrEmail.replace(
          /[^0-9+]/g,
          ''
        );
      }
  ...
  return (
    <AuthPageShell
      ...
      asideQuote={
        '"A trusted account is the foundation of every honest ledger."'
      }
    >
      {showTotpInput ? (
        <form onSubmit={handleTotpSubmit} className='space-y-6'>
          ...
          <AuthSubmitButton
            isLoading={isLoading}
            disabled={
              isRecoveryMode ? totpCode.length !== 8 : totpCode.length !== 6
            }
          >
        ...
        <>
          <form onSubmit={handleSubmit} className='space-y-5'>
            ...
            <div className='relative flex items-center py-2'>
              ...
              <span className='flex-shrink-0 mx-4 text-muted-foreground text-[11px] uppercase tracking-[0.2em]'>
                Or
              </span>
              <div className='flex-grow border-t border-hairline'></div>
            </div>
