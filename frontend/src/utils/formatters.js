...
export const formatters = {
  ...
  currencyCompact: (amount, _currency = 'INR') => {
    if (!amount) return '₹0';
  ...
  percentage: (value, options = {}) => {
    ...
    try {
      ...
      return new Intl.NumberFormat('en-IN', formatOptions).format(
        decimalValue || 0
      );
    } catch (error) {
  ...
  pnlPercentage: (value, showSign = true) => {
    ...
    const formatted = formatters.percentage(
      Math.abs(value) > 1 ? Math.abs(value) / 100 : Math.abs(value)
    );
    const sign = showSign ? (value >= 0 ? '+' : '-') : '';
...
export const colors = {
  ...
  getChangeBgColor: (value, theme = 'light') => {
    if (value > 0) {
      return theme === 'dark'
        ? 'rgba(34, 197, 94, 0.1)'
        : 'rgba(34, 197, 94, 0.1)'; // Green bg
    } else if (value < 0) {
      return theme === 'dark'
        ? 'rgba(239, 68, 68, 0.1)'
        : 'rgba(239, 68, 68, 0.1)'; // Red bg
    } else {
      return theme === 'dark'
        ? 'rgba(156, 163, 175, 0.1)'
        : 'rgba(156, 163, 175, 0.1)'; // Gray bg
    }
  },
};
