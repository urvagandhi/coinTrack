 ...
 *
 * <p>Handles all input types encountered across Indian brokers: Double/Float: new
 * BigDecimal(Double.toString(value)) — NEVER new BigDecimal(doubleValue) to avoid IEEE 754
 * precision loss (critical for penny stocks on Upstox). String: Parses after stripping whitespace.
 * Handles "-0.00" → ZERO, "" → ZERO. Integer/Long: BigDecimal.valueOf(value)
 *
 * <p>Angel One returns all numeric fields as Strings ("1234.50", "-0.00", ""). Upstox returns
 * prices as floats. Zerodha returns prices as doubles.
 *
 * <p>Stateless utility — no Spring dependency needed.
 */
public final class PriceNormalizer {
  ...
  public static BigDecimal toBigDecimal(Object value, String fieldName, Broker broker) {
    ...
    try {
      return switch (value) {
        case Double d ->
            new BigDecimal(Double.toString(d)).setScale(DEFAULT_SCALE, RoundingMode.HALF_UP);

        case Float f ->
            new BigDecimal(Float.toString(f)).setScale(DEFAULT_SCALE, RoundingMode.HALF_UP);

        case String s -> parseString(s, fieldName, broker);

        case Integer i -> BigDecimal.valueOf(i).setScale(DEFAULT_SCALE, RoundingMode.HALF_UP);

        case Long l -> BigDecimal.valueOf(l).setScale(DEFAULT_SCALE, RoundingMode.HALF_UP);

        case BigDecimal bd -> bd.setScale(DEFAULT_SCALE, RoundingMode.HALF_UP);

        default -> {
          log.warn(
              "Unexpected type {} for field='{}' from broker={}, attempting toString parse",
      ...
      throw new BrokerException(
          String.format(
              "Failed to parse field '%s' value '%s' from broker %s: %s",
  ...
  /**
   * Parses a String value to BigDecimal. Handles Angel One quirks: - "-0.00" → ZERO - "" or blank →
   * ZERO with warning - Non-numeric → throws BrokerException
   */
  private static BigDecimal parseString(String s, String fieldName, Broker broker) {
    ...
    try {
      ...
      throw new BrokerException(
          String.format(
              "Non-numeric string '%s' for field '%s' from broker %s", trimmed, fieldName, broker),
          broker);
    }
  }

  /**
   * Parses a quantity value to BigDecimal with scale=0 (whole units). Angel One returns quantity as
   * String ("10"), Zerodha as int, Upstox as int.
   */
  public static BigDecimal toQuantity(Object value, String fieldName, Broker broker) {
    if (value == null) {
      log.warn(
          "Null quantity for field='{}' from broker={}, defaulting to ZERO", fieldName, broker);
