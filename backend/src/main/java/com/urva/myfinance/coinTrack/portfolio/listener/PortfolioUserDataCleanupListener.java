...
/**
 * Removes all portfolio cache/canonical data for a deleted user.
 * market_prices is deliberately kept — it is deployment-wide shared data, not user-owned.
 */
@Component
public class PortfolioUserDataCleanupListener {

    private static final Logger logger = LoggerFactory.getLogger(PortfolioUserDataCleanupListener.class);
    ...
    public PortfolioUserDataCleanupListener(CanonicalHoldingRepository holdingRepository,
    ...
    public void onUserDeleted(UserDeletedEvent event) {
        ...
        try {
            ...
            logger.error("Cascade cleanup failed for portfolio data, user {}: {}", userId, e.getMessage());
        }
    }
}
