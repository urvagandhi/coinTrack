...
/**
 * Savings scheme calculations (PPF, EPF, FD, RD, NSC, SSY, SCSS, MIS, NPS, APY). Stateless,
 * thread-safe.
 */
@Component
public class SavingsMath {
  ...
  /**
   * Calculate PPF balance after N years with compound interest. Interest = balance × rate,
   * compounded annually.
   */
  ...
  /** Calculate FD maturity with compound interest. Formula: A = P × (1 + r/n)^(n×t) */
  public BigDecimal fdMaturity(
      BigDecimal principal, BigDecimal ratePercent, int tenureDays, int compoundingFrequency) {
    BigDecimal years =
        BigDecimal.valueOf(tenureDays).divide(BigDecimal.valueOf(365), SCALE, ROUNDING);
    BigDecimal rateDecimal = ratePercent.divide(HUNDRED, SCALE, ROUNDING);
    BigDecimal periodicRate =
        rateDecimal.divide(BigDecimal.valueOf(compoundingFrequency), SCALE, ROUNDING);
  ...
  public BigDecimal scssQuarterlyInterest(BigDecimal principal, BigDecimal ratePercent) {
    BigDecimal quarterlyRate =
        ratePercent.divide(HUNDRED, SCALE, ROUNDING).divide(BigDecimal.valueOf(4), SCALE, ROUNDING);
  ...
  /**
   * Calculate APY approximate monthly contribution. Simplified — actual APY uses government lookup
   * tables.
   */
  public BigDecimal apyMonthlyContribution(BigDecimal desiredPension, int currentAge) {
    return BigDecimal.valueOf(desiredPension.doubleValue() * (1 + (currentAge - 18) * 0.1));
  }
}
