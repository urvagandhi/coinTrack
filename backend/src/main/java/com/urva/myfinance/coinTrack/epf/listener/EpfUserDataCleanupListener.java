...
/**
 * Removes EPF ledger transactions for a deleted user.
 * EPF settings live embedded in the user document — removed with the user itself.
 */
@Component
public class EpfUserDataCleanupListener {
    ...
    public void onUserDeleted(UserDeletedEvent event) {
        try {
            ...
            logger.error("Cascade cleanup failed for EPF transactions, user {}: {}", event.userId(), e.getMessage());
        }
    }
}
