 ...
 *
 * <p>Zerodha quirks handled: - average_price == 0.0 for CDSL transfers → dataConfidence = LOW -
 * ISIN null for newly listed/illiquid stocks → fallback to tradingsymbol - day_change /
 * day_change_percentage provided (unlike Angel One)
 */
@Component
public class ZerodhaHoldingMapper {
  ...
  public CanonicalHolding toCanonical(
      ZerodhaHoldingRaw raw, String userId, String brokerAccountId) {
    ...
    if (isin == null || isin.isBlank()) {
      log.warn(
          "Zerodha holding missing ISIN for tradingsymbol={}, using tradingsymbol as fallback",
    ...
    BigDecimal t1Quantity =
        BigDecimal.valueOf(raw.getT1_quantity() != null ? raw.getT1_quantity() : 0);
    ...
    BigDecimal dayChange =
        raw.getDay_change() != null
            ? PriceNormalizer.toBigDecimal(raw.getDay_change(), "day_change", Broker.ZERODHA)
            : null;

    // 13. Day change percentage
    BigDecimal dayChangePct =
        raw.getDay_change_percentage() != null
            ? PriceNormalizer.toBigDecimal(
                raw.getDay_change_percentage(), "day_change_percentage", Broker.ZERODHA)
            : null;
