...
/**
 * Facade over focused math classes. Keeps existing static API unchanged so service impls don't need
 * modification.
 *
 * <p>Delegates to: SipMath, LoanMath, InvestmentMath, XirrCalculator, SavingsMath, TaxMath,
 * MathUtil.
 */
public final class FinancialMath {
  ...
  public static BigDecimal sipFutureValue(
      BigDecimal monthlyInvestment, BigDecimal annualRate, int years) {
    return SIP.sipFutureValue(monthlyInvestment, annualRate, years);
  }

  public static BigDecimal stepUpSipFutureValue(
      BigDecimal initialMonthlyInvestment,
      BigDecimal annualRate,
      int years,
      BigDecimal stepUpPercent) {
    return SIP.stepUpSipFutureValue(initialMonthlyInvestment, annualRate, years, stepUpPercent);
  }

  public static BigDecimal stepUpSipTotalInvestment(
      BigDecimal initialMonthlyInvestment, int years, BigDecimal stepUpPercent) {
    return SIP.stepUpSipTotalInvestment(initialMonthlyInvestment, years, stepUpPercent);
  }

  // ── Lumpsum / CAGR / Inflation ──────────────────────────────

  public static BigDecimal lumpsumFutureValue(
  ...
  public static BigDecimal inflationAdjustedFuture(
      BigDecimal presentValue, BigDecimal inflationRate, int years) {
    return INVESTMENT.inflationAdjustedFuture(presentValue, inflationRate, years);
  }

  public static BigDecimal inflationAdjustedPresentValue(
      BigDecimal futureValue, BigDecimal inflationRate, int years) {
  ...
  public static BigDecimal stockAverage(List<StockPurchase> purchases) {
    ...
    List<InvestmentMath.StockPurchase> converted =
        purchases.stream()
  ...
  public static BigDecimal compoundInterest(
      BigDecimal principal, BigDecimal annualRate, int years, int compoundingFrequency) {
  ...
  public record XirrResult(
      BigDecimal rate, boolean success, String errorCode, String errorMessage) {
  ...
  public static XirrResult xirr(List<CashFlow> cashFlows) {
    ...
    List<XirrCalculator.CashFlow> converted =
        cashFlows.stream().map(cf -> new XirrCalculator.CashFlow(cf.date(), cf.amount())).toList();
