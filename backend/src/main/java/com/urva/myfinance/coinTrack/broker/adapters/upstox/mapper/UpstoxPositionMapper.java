 ...
 *
 * <p>Upstox quirks: - Prices are Float — converted via PriceNormalizer - quantity can be negative
 * for sell positions - overnightQuantity is a separate field (Upstox-specific) - multiplier
 * provided by Upstox for F&O instruments
 */
@Component
public class UpstoxPositionMapper {
  ...
  public CanonicalPosition toCanonical(
      UpstoxPositionRaw raw, String userId, String brokerAccountId) {
    // Symbol normalization
    String symbol =
        SymbolNormalizer.normalize(raw.getTradingSymbol(), Broker.UPSTOX, raw.getExchange());
    ...
    BigDecimal overnightQty =
        BigDecimal.valueOf(raw.getOvernightQuantity() != null ? raw.getOvernightQuantity() : 0);

    // Average buy price (Float → PriceNormalizer)
    BigDecimal avgBuyPrice =
        PriceNormalizer.toBigDecimal(raw.getAveragePrice(), "average_price", Broker.UPSTOX);

    // Average sell price (nullable, Float → PriceNormalizer)
    BigDecimal avgSellPrice =
        raw.getSellPrice() != null
    ...
    BigDecimal lastPrice =
        PriceNormalizer.toBigDecimal(raw.getLastPrice(), "last_price", Broker.UPSTOX);

    // Realized P&L (Float → PriceNormalizer)
    BigDecimal realizedPnL =
        PriceNormalizer.toBigDecimal(raw.getRealised(), "realised", Broker.UPSTOX);

    // Unrealized P&L (Float → PriceNormalizer)
    BigDecimal unrealizedPnL =
        PriceNormalizer.toBigDecimal(raw.getUnrealised(), "unrealised", Broker.UPSTOX);
