...
public class GoldSilverExcelExporter {
  ...
  public static ResponseEntity<byte[]> export(List<GoldSilverResponseDTO> allInvestments) {
    try (Workbook workbook = new XSSFWorkbook();
      ...
      // Tab 2: Gold
      List<GoldSilverResponseDTO> gold =
          allInvestments.stream()
              .filter(f -> f.getMetalType() == MetalType.GOLD)
      ...
      // Tab 3: Silver
      List<GoldSilverResponseDTO> silver =
          allInvestments.stream()
              .filter(f -> f.getMetalType() == MetalType.SILVER)
              .collect(Collectors.toList());
      if (!silver.isEmpty())
        createSheet(
            workbook,
      ...
      // Tab 4: Active
      List<GoldSilverResponseDTO> active =
          allInvestments.stream()
              .filter(f -> f.getStatus() == GsStatus.ACTIVE)
              .collect(Collectors.toList());
      if (!active.isEmpty())
        createSheet(
            workbook,
      ...
      // Tab 5: Due
      List<GoldSilverResponseDTO> due =
          allInvestments.stream()
              .filter(f -> f.getStatus() == GsStatus.DUE)
              .collect(Collectors.toList());
      if (!due.isEmpty())
        createSheet(
            workbook,
      ...
      // Tab 6: Matured
      List<GoldSilverResponseDTO> matured =
          allInvestments.stream()
              .filter(f -> f.getStatus() == GsStatus.MATURED)
              .collect(Collectors.toList());
      if (!matured.isEmpty())
        createSheet(
            workbook,
      ...
      httpHeaders.set(
          HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"gold_silver_investments.xlsx\"");
  ...
  private static void createSheet(
      Workbook workbook,
      String sheetName,
      List<GoldSilverResponseDTO> data,
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      createNumericCell(
          row, 0, dto.getItemNo() != null ? dto.getItemNo().doubleValue() : null, rightAlignStyle);
      createCell(
          row, 1, dto.getPurchaseDate() != null ? dto.getPurchaseDate().toString() : "", dataStyle);
      ...
      createCell(
          row, 6, dto.getRateSource() != null ? dto.getRateSource().name() : "LIVE", dataStyle);
      createNumericCell(
          row,
          7,
          dto.getNetWeight() != null ? dto.getNetWeight().doubleValue() : null,
          weightStyle);
      createNumericCell(
          row,
          8,
          dto.getRatePerGram() != null ? dto.getRatePerGram().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          9,
          dto.getMetalAmount() != null ? dto.getMetalAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          10,
          dto.getMakingChargePercent() != null
              ? dto.getMakingChargePercent().doubleValue() / 100.0
              : null,
          percentStyle);
      createNumericCell(
          row,
          11,
          dto.getMakingChargeAmount() != null ? dto.getMakingChargeAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          12,
          dto.getStoneOtherCharges() != null ? dto.getStoneOtherCharges().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          13,
          dto.getTotalAmount() != null ? dto.getTotalAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          14,
          dto.getGstPercent() != null ? dto.getGstPercent().doubleValue() / 100.0 : null,
          percentStyle);
      createNumericCell(
          row,
          15,
          dto.getGstAmount() != null ? dto.getGstAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          16,
          dto.getNetAmount() != null ? dto.getNetAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          17,
          dto.getCurrentMarketRate() != null ? dto.getCurrentMarketRate().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          18,
          dto.getCurrentValue() != null ? dto.getCurrentValue().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          19,
          dto.getProfitLoss() != null ? dto.getProfitLoss().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          20,
          dto.getReturnPercent() != null ? dto.getReturnPercent().doubleValue() / 100.0 : null,
          percentStyle);
      createCell(row, 21, dto.getStatus() != null ? dto.getStatus().name() : "", dataStyle);
      createCell(
          row,
          22,
          dto.getMaturityDate() != null ? dto.getMaturityDate().toString() : "-",
          dataStyle);
    ...
    if (!data.isEmpty()) {
      ...
      double totalWeight =
          data.stream()
              .mapToDouble(d -> d.getNetWeight() != null ? d.getNetWeight().doubleValue() : 0.0)
              .sum();
      double totalMetalAmt =
          data.stream()
              .mapToDouble(d -> d.getMetalAmount() != null ? d.getMetalAmount().doubleValue() : 0.0)
              .sum();
      double totalMakingAmt =
          data.stream()
              .mapToDouble(
                  d ->
                      d.getMakingChargeAmount() != null
                          ? d.getMakingChargeAmount().doubleValue()
                          : 0.0)
              .sum();
      double totalOtherAmt =
          data.stream()
              .mapToDouble(
                  d ->
                      d.getStoneOtherCharges() != null
                          ? d.getStoneOtherCharges().doubleValue()
                          : 0.0)
              .sum();
      double totalAmt =
          data.stream()
              .mapToDouble(d -> d.getTotalAmount() != null ? d.getTotalAmount().doubleValue() : 0.0)
              .sum();
      double totalGstAmt =
          data.stream()
              .mapToDouble(d -> d.getGstAmount() != null ? d.getGstAmount().doubleValue() : 0.0)
              .sum();
      double totalNetAmt =
          data.stream()
              .mapToDouble(d -> d.getNetAmount() != null ? d.getNetAmount().doubleValue() : 0.0)
              .sum();
      ...
      double totalPl =
          data.stream()
              .mapToDouble(d -> d.getProfitLoss() != null ? d.getProfitLoss().doubleValue() : 0.0)
              .sum();
