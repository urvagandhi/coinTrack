...
/**
 * Default implementation of {@link NotificationService}. Delegates to {@link EmailService} when
 * available.
 *
 * <p>{@code EmailService} is injected optionally — if the email module is not configured,
 * notifications degrade gracefully to log warnings.
 */
@Service
public class NotificationServiceImpl implements NotificationService {
  ...
  public void sendSecurityAlertWithIP(User user, String event, String ipAddress) {
    if (emailService != null && user.getEmail() != null) {
      ...
      logger.info(
          "Security alert sent: event={}, userId={}, ip={}", event, user.getId(), ipAddress);
    } else {
      logger.warn("Cannot send security alert: email service not configured or user has no email");
    }
  }
  ...
  public void sendWelcomeNotification(User user) {
    if (emailService != null && user.getEmail() != null) {
      ...
      logger.warn(
          "Cannot send welcome notification: email service not configured or user has no email");
    }
  }
