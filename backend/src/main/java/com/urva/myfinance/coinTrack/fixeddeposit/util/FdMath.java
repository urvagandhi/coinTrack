...
public final class FdMath {
    ...
    /**
     * Per-FD TDS input used by {@link #computeBankLevelTds}. Represents one FD's contribution
     * to its bank's aggregate TDS computation for a financial year.
     */
    ...
    /**
     * Bank-level TDS result for a single FD, enriched with the bank's aggregate context so a
     * caller can explain WHY a threshold was crossed (the threshold is per-bank, not per-FD).
     */
     ...
     *
     * The ₹50,000 / ₹1,00,000 threshold applies to the TOTAL interest a person earns from
     * ALL FDs at the SAME bank (payer) in a financial year — not to each FD individually.
     * This method groups the FDs of one bank, checks the threshold against the summed gross
     * interest, and (when TDS is triggered) distributes the deduction back to each FD
     * proportionally to its own gross interest.
     *
     * Semantics (per household-tracker behaviour):
     * <ul>
     *   <li><b>15G/15H exemption is bank-level</b> — a declaration covers all deposits at the
     *       payer. If ANY FD at the bank is flagged exempt, the whole bank group is exempt.</li>
     *   <li><b>Senior threshold</b> — used only when every non-exempt FD at the bank is flagged
     *       senior (a homogeneous group); otherwise the regular threshold applies.</li>
     *   <li><b>Proportional split</b> is computed AFTER the exemption, so exempted FDs
     *       contribute their interest to the group total for threshold-crossing but receive
     *       no TDS share.</li>
     * </ul>
     */
    public static List<BankTdsResult> computeBankLevelTds(List<FdTdsInput> fds) {
        ...
        boolean allNonExemptSenior = fds.stream()
                .filter(fd -> !fd.form15g15hSubmitted())
                .allMatch(FdTdsInput::isSeniorCitizen);
        ...
        BigDecimal bankTaxable = anyExempt
                ? BigDecimal.ZERO
                : bankGross.subtract(threshold);
        ...
        for (int i = 0; i < fds.size(); i++) {
            ...
            if (gross.compareTo(BigDecimal.ZERO) <= 0) {
                results.add(buildBankTdsResult(gross, threshold, bankGross, bankTaxable,
                        bankTdsDeducted, BigDecimal.ZERO, BigDecimal.ZERO));
                continue;
            }
        ...
        if (lastAllocatableIndex >= 0) {
            ...
            BigDecimal tdsDeducted = last.tdsDeducted().add(delta).max(BigDecimal.ZERO).setScale(2, ROUNDING);
            BigDecimal net = last.netInterest().subtract(delta).max(BigDecimal.ZERO).setScale(2, ROUNDING);
            results.set(lastAllocatableIndex, new BankTdsResult(
                    last.grossInterest(), last.tdsThreshold(), last.bankTaxableInterest(),
                    last.tdsRate(), tdsDeducted, net,
                    last.bankTotalGrossInterest(), last.bankTaxableInterest(), last.bankTotalTdsDeducted()));
        }
    ...
    private static BankTdsResult buildBankTdsResult(
            BigDecimal gross, BigDecimal threshold, BigDecimal bankGross,
            BigDecimal bankTaxable, BigDecimal bankTdsDeducted,
            BigDecimal share, BigDecimal tdsRate) {
    ...
    public static MaturityResult computeMaturity(
        ...
        if (fdType == FdType.NON_CUMULATIVE) {
            ...
            totalInterest = computeCumulativeInterest(principal, effectiveRatePercent, totalDays, compoundingFreq);
        }
     ...
     *
     * <p>TDS (194A) is a per-FY obligation: the interest contributed to a bank-and-holder group
     * in FY X is only the interest ACCRUED within that FY's window, not the FD's lifetime
     * interest. For an FD spanning multiple financial years, using lifetime interest would
     * wrongly attribute it all to whichever FY is queried and push it over the exemption
     * threshold every year.
     *
     * <p>Accrual basis:
     * <ul>
     *   <li><b>Cumulative</b> — interest is credited on accrual. The accrued value tracks the
     *       compounding curve, so window accrual = value(fyEnd) − value(fyStart), each value
     *       computed by the standard compounding engine up to that date.</li>
     *   <li><b>Non-cumulative</b> — interest is paid out (cash basis) as simple interest on
     *       principal; window accrual = simple interest on the FDs active days within the FY.</li>
     * </ul>
     *
     * <p>The window is clamped to the FD's actual active span (issue → maturity, or issue →
     * premature-withdrawal date), so no interest is accrued before issue, after maturity, or
     * after withdrawal.
     *
     * @param principal          issue amount
     * @param ratePercent        contracted annual rate (% p.a.)
     * @param issueDate          FD issue date
     * @param endDate            FD end date (maturity) — during premature withdrawal, the
     *                           realized/withdrawal date should be passed here instead
    ...
    public static BigDecimal computeFyAccruedInterest(
        ...
        if (principal == null || ratePercent == null || issueDate == null || endDate == null
        ...
        BigDecimal valueAtStart = computeAccruedValue(principal, ratePercent, issueDate, activeStart, compoundingFreq);
        BigDecimal valueAtEnd = computeAccruedValue(principal, ratePercent, issueDate, activeEnd, compoundingFreq);
    ...
    private static BigDecimal computeAccruedValue(
        ...
        if (brokenDays > 0) {
            ...
            BigDecimal brokenInterest = amountAfterFullPeriods.multiply(dailyRate).multiply(BigDecimal.valueOf(brokenDays));
    ...
    private static BigDecimal computeSimpleInterest(BigDecimal principal, BigDecimal ratePercent, long days) {
        BigDecimal rateDecimal = ratePercent.divide(HUNDRED, SCALE, ROUNDING);
        BigDecimal dailyRate = rateDecimal.divide(BigDecimal.valueOf(DAYS_IN_YEAR), SCALE, ROUNDING);
    ...
    public static PrematureWithdrawalResult computePrematureWithdrawal(
        ...
        if (withdrawalDate.isBefore(issueDate) || withdrawalDate.isAfter(maturityDate)) {
            throw new IllegalArgumentException("Withdrawal date must be between issue date and maturity date");
        }
        ...
        if (actualTenorDays < 7) {
            MaturityResult zeroResult = computeMaturity(
                principal, contractedRate, issueDate, withdrawalDate,
                fdType, compoundingFreq, isSeniorCitizen, InterestPayoutFrequency.AT_MATURITY
            );
        ...
        MaturityResult fullMaturity = computeMaturity(
            principal, contractedRate, issueDate, maturityDate,
            fdType, compoundingFreq, isSeniorCitizen, InterestPayoutFrequency.AT_MATURITY
        );

        MaturityResult actualMaturity = computeMaturity(
            principal, contractedRate, issueDate, withdrawalDate,
            fdType, compoundingFreq, isSeniorCitizen, InterestPayoutFrequency.AT_MATURITY
        );
        ...
        BigDecimal realizedMaturity = computeWithPenalty(
    ...
    private static BigDecimal computeWithPenalty(
        ...
        if (fdType == FdType.NON_CUMULATIVE) {
            return principal.add(computeNonCumulativeInterest(principal, effectiveRatePercent, actualTenorDays));
        } else {
            if (actualTenorDays < SIMPLE_INTEREST_THRESHOLD_DAYS) {
                return principal.add(computeSimpleInterest(principal, effectiveRatePercent, actualTenorDays));
            }
            BigDecimal interest = computeCumulativeInterest(principal, effectiveRatePercent, actualTenorDays, compoundingFreq);
    ...
    public static BigDecimal computeInterestRateFromMaturity(
        ...
        for (int i = 0; i < 60; i++) {
            ...
            MaturityResult result = computeMaturity(
                principal, mid, issueDate, maturityDate,
                fdType, compoundingFreq, isSeniorCitizen, InterestPayoutFrequency.AT_MATURITY
            );
