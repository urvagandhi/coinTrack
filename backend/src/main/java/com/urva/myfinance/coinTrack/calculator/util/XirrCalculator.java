...
/**
 * XIRR (Extended Internal Rate of Return) calculator. Newton-Raphson primary, bisection fallback.
...
public class XirrCalculator {
  ...
  public XirrResult xirr(List<CashFlow> cashFlows) {
    ...
    boolean hasPositive =
        cashFlows.stream().anyMatch(cf -> cf.amount().compareTo(BigDecimal.ZERO) > 0);
    boolean hasNegative =
        cashFlows.stream().anyMatch(cf -> cf.amount().compareTo(BigDecimal.ZERO) < 0);
    if (!hasPositive || !hasNegative) {
      return XirrResult.failure(
          "INVALID_INPUT", "Cash flows must have both positive and negative values");
    }

    List<CashFlow> sortedFlows =
        cashFlows.stream().sorted((a, b) -> a.date().compareTo(b.date())).toList();
    ...
    return XirrResult.failure(
        "XIRR_NON_CONVERGENCE", "XIRR calculation did not converge. Try different cash flows.");
  }
  ...
  private Double bisection(List<CashFlow> flows, LocalDate startDate, double low, double high) {
