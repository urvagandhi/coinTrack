 ...
 *
 * <p>PURPOSE: Users registered BEFORE email verification was implemented don't have emailVerified
 * set. This would block them from using features that require verified email. This migration marks
 * all existing users as verified.
 *
 * <p>USAGE: 1. Add 'migration' to spring.profiles.active in application.properties OR run with
 * -Dspring.profiles.active=migration 2. Start the application once 3. Remove 'migration' profile
 * after completion
 *
 * <p>Example: mvn spring-boot:run -Dspring.profiles.active=migration
 *
 * <p>SAFETY: - Only runs with 'migration' profile (not in normal operation) - Only updates users
 * where emailVerified is null or false - Logs all changes for audit trail
 */
@Component
@Profile("migration")
public class EmailVerificationMigration implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    Update update =
        new Update().set("emailVerified", true).set("emailVerifiedAt", LocalDateTime.now());
