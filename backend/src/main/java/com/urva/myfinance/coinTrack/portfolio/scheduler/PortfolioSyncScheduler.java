package com.urva.myfinance.coinTrack.portfolio.scheduler;
...
public class PortfolioSyncScheduler {
  ...
  /**
   * CRON 1: MARKET HOURS SYNC Runs every 5 minutes during market hours (09:00 - 15:59 approx, logic
   * tightened in service). The Cron expression is slightly wider (9-15) than strict market hours
   * (9:15-15:30), but syncAllActiveAccounts() calls syncSafetyService.isMarketOpen() internally to
   * enforce exact times.
   */
  ...
  /**
   * CRON 2: OFF-HOURS SYNC Runs every 15 minutes. Skips if market is OPEN. Only syncs if data is
      ...
      do {
        ...
        for (BrokerAccount account : accountPage.getContent()) {
          try {
            ...
            logger.error(
                "Error in Off-Hours sync for account {}: {}", account.getId(), e.getMessage());
          }
        }
        page++;
  ...
  private boolean shouldSyncOffHours(BrokerAccount account) {
    ...
    long minutesSinceLastSync =
        Duration.between(lastSync, LocalDateTime.now(INDIA_ZONE)).toMinutes();
