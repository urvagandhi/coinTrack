...
public class BrokerConnectServiceImpl implements BrokerConnectService {
  ...
  private final com.urva.myfinance.coinTrack.portfolio.sync.PortfolioSyncService
      portfolioSyncService;
  private final EncryptionUtil encryptionUtil;

  @Autowired
  public BrokerConnectServiceImpl(
  ...
  public String getLoginUrl(Broker broker) {
    ...
    if (broker == Broker.ANGELONE) {
      ...
      throw new BrokerException(
          "AngelOne does not use a login URL. POST /api/brokers/angelone/connect.", broker);
    }
  ...
  public void connectAngelOne(String userId) {
    BrokerAccount account =
        accountRepository.findByUserIdAndBroker(userId, Broker.ANGELONE).stream()
            .findFirst()
            .orElseThrow(
                () ->
                    new BrokerException(
    ...
    try {
      ...
      AngelOneCredentials creds =
          new AngelOneCredentials(
      ...
      account.setAngelOneTokenExpiresAt(
          LocalDateTime.ofInstant(session.expiresAt(), java.time.ZoneId.of("Asia/Kolkata")));
      ...
      throw new BrokerException(
          "AngelOne login failed: " + cause.getMessage(), Broker.ANGELONE, cause);
    } catch (Exception e) {
  ...
  public void handleCallback(String userId, Broker broker, String requestToken) {
    ...
    Optional<BrokerAccount> existingOpt =
        accountRepository.findByUserIdAndBroker(userId, broker).stream().findFirst();
    BrokerAccount account;
    ...
    if (broker == Broker.ZERODHA) {
      ...
      if (apiKey == null || encryptedSecret == null) {
        throw new BrokerException(
            "Missing Zerodha API credentials. Please disconnect and reconnect.", broker);
      }

      java.util.Map<String, Object> tokenData =
          zerodhaLiveDataService.exchangeToken(requestToken, apiKey, encryptedSecret);
      ...
      if (apiKey == null || encryptedSecret == null || redirectUri == null) {
        throw new BrokerException(
            "Missing Upstox API credentials. Please save apiKey/secret/redirectUri before connecting.",
            broker);
      }

      try {
        UpstoxCredentials creds =
            new UpstoxCredentials(apiKey, encryptedSecret, requestToken, redirectUri);
        ...
        Throwable cause = ce.getCause() != null ? ce.getCause() : ce;
        throw new BrokerException(
            "Upstox token exchange failed: " + cause.getMessage(), broker, cause);
      } catch (Exception e) {
