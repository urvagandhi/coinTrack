 ...
 *
 * <p>Priority chain: 1. MongoDB cache (15-sec TTL during market hours, 5-min off-hours) 2. Zerodha
 * LTP API: GET /quote/ltp?i=NSE:RELIANCE&i=BSE:SBIN (max 200 per call) 3. Canonical holding's
 * stored currentPrice (marked stale)
 *
 * <p>Changed: Replaced random/hardcoded stub with real Zerodha LTP API calls. Never throws — always
...
public class MarketDataServiceImpl implements MarketDataService {
  ...
  public Map<String, MarketPrice> getPrices(List<String> symbols) {
    ...
    Map<String, MarketPrice> result =
        new HashMap<>(
            cached.stream()
                .filter(
                    mp ->
                        Duration.between(mp.getUpdatedAt(), LocalDateTime.now()).getSeconds()
                            < getCacheTtlSeconds())
                .collect(
                    Collectors.toMap(MarketPrice::getSymbol, Function.identity(), (a, b) -> a)));

    // 2. Identify missing symbols
    List<String> missing = symbols.stream().filter(s -> !result.containsKey(s)).distinct().toList();
    ...
    List<String> stillMissing = missing.stream().filter(s -> !result.containsKey(s)).toList();
  ...
  /**
   * Batch fetch live prices from Zerodha LTP API. Uses the first active Zerodha account found (any
  ...
  private Map<String, MarketPrice> fetchFromZerodhaLtp(List<String> symbols) {
    ...
    BrokerAccount active =
        zerodhaAccounts.stream()
            .filter(
                a -> a.getIsActive() && !a.isTokenExpired() && a.getZerodhaAccessToken() != null)
            .filter(
                a -> {
    ...
    if (active == null) {
      ...
      log.debug(
          "No Zerodha account eligible for LTP (none connected, or all lack market-data permission). Using canonical prices.");
      return result;
    }
    ...
    for (int i = 0; i < symbols.size(); i += LTP_BATCH_SIZE) {
      ...
      try {
        Map<String, MarketPrice> batchResult =
            callZerodhaLtpApi(batch, apiKey, accessToken, active);
  ...
  /**
   * GET https://api.kite.trade/quote/ltp?i=NSE:RELIANCE&i=BSE:SBIN Response: { "status": "success",
   * "data": { "NSE:RELIANCE": { "instrument_token": 738561, "last_price": 2680.30 } } }
   */
  private Map<String, MarketPrice> callZerodhaLtpApi(
      List<String> symbols, String apiKey, String accessToken, BrokerAccount active) {
    Map<String, MarketPrice> result = new HashMap<>();

    // Build query params: ?i=NSE:RELIANCE&i=BSE:SBIN
    String queryParams = symbols.stream().map(s -> "i=" + s).collect(Collectors.joining("&"));
    ...
    try {
                ...
                if (lastPrice > 0) {
                  result.put(
                      ...
                      MarketPrice.builder()
                          .symbol(symbol)
                          .currentPrice(
                              new BigDecimal(Double.toString(lastPrice))
                                  .setScale(2, RoundingMode.HALF_UP))
                          .previousClose(
                              null) // LTP endpoint doesn't provide close — will be filled from
                          // canonical
      ...
      if (e.getStatusCode().value() == 429) {
        log.warn("Zerodha LTP 429 rate limited — account={} user={}", acctId, acctUser);
          ...
          && "TokenException".equals(errorType)
          && active != null) {
        // Real auth failure (expired/revoked session). Signal reconnect.
        log.warn(
            "Zerodha LTP TokenException — flagging account={} user={} apiKey={} for reconnect.",
            acctId,
            acctUser,
            apiKeyPreview);
        try {
          ...
          active.setExpiryReason(
              com.urva.myfinance.coinTrack.broker.model.ExpiryReason.TOKEN_INVALID);
          brokerAccountRepository.save(active);
        } catch (Exception saveEx) {
          log.warn("Failed to flag account {} for reconnect: {}", acctId, saveEx.getMessage());
        }
      } else if (e.getStatusCode().value() == 403
          && "PermissionException".equals(errorType)
          && active != null) {
        ...
        boolean firstTime =
            noMarketDataPermission.putIfAbsent(
                    active.getId(), System.currentTimeMillis() + PERMISSION_DENIED_TTL_MS)
                == null;
        if (firstTime) {
          log.warn(
              "Zerodha LTP PermissionException — account={} user={} apiKey={} lacks market-data scope. "
        ...
        if (active.getExpiryReason() != null
            && active.getExpiryReason()
                != com.urva.myfinance.coinTrack.broker.model.ExpiryReason.NONE
        ...
        log.warn(
            "Zerodha LTP API error account={}: status={} type={} body={}",
            acctId,
            e.getStatusCode(),
            errorType,
            body);
      ...
      log.warn(
          "Zerodha LTP API call failed account={}: {}",
          active != null ? active.getId() : "n/a",
          e.getMessage());
    }
  ...
  private Map<String, MarketPrice> fallbackToCanonical(List<String> symbols) {
    ...
    List<String> stillMissing = symbols.stream().filter(s -> !result.containsKey(s)).toList();
