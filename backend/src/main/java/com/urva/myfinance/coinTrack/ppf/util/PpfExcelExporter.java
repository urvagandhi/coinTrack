...
public class PpfExcelExporter {
  ...
  public static ResponseEntity<byte[]> export(
      List<PpfTransactionResponseDTO> transactions,
      String userName,
    ...
    try (Workbook workbook = new XSSFWorkbook();
      ...
      for (int i = 0; i < transactions.size(); i++) {
        ...
        createCell(
            row,
            3,
            dto.getParticularType() != null ? dto.getParticularType().name() : "",
            dataStyle);
        createNumericCell(
            row,
            4,
            dto.getDebitAmount() != null ? dto.getDebitAmount().doubleValue() : null,
            currencyStyle);
        createNumericCell(
            row,
            5,
            dto.getCreditAmount() != null ? dto.getCreditAmount().doubleValue() : null,
            currencyStyle);
        createNumericCell(
            row,
            6,
            dto.getBalance() != null ? dto.getBalance().doubleValue() : null,
            currencyStyle);
      ...
      if (!transactions.isEmpty()) {
        ...
        totalCurrencyStyle.setDataFormat(
            workbook.createDataFormat().getFormat("[$₹-en-IN]#,##0.00"));
        ...
        double totalDebit =
            transactions.stream()
                .mapToDouble(
                    d -> d.getDebitAmount() != null ? d.getDebitAmount().doubleValue() : 0.0)
                .sum();
        double totalCredit =
            transactions.stream()
                .mapToDouble(
                    d -> d.getCreditAmount() != null ? d.getCreditAmount().doubleValue() : 0.0)
                .sum();
      ...
      httpHeaders.set(
          HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"ppf_transactions.xlsx\"");
  ...
  private static int renderHeaderMetadata(
      Sheet sheet,
      Workbook workbook,
      String userName,
      PpfSettingsResponseDTO settings,
