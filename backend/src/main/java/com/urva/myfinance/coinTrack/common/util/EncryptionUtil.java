 ...
 *
 * <p>Encrypted format: [IV (12 bytes)] [Ciphertext] [Auth Tag (16 bytes)]
 *
 * <p>Security guarantees: - 12-byte (96-bit) IV — the recommended length for GCM - Random IV
 * generated per encryption via {@link SecureRandom} - 128-bit authentication tag — prevents
 * tampering - Key must be exactly 32 bytes (validated by {@link EncryptionConfig} at startup) - No
 * plaintext or key material is ever logged
 */
@Component
public class EncryptionUtil {
  ...
  /**
   * Encrypt plaintext using a caller-supplied key. Useful for per-tenant or per-entity encryption
   * where the key differs from the application-wide default.
   *
  ...
  /**
   * Decrypt if encrypted, otherwise return the plain text as-is. Useful for backward compatibility
   * when migrating from plain-text to encrypted storage.
   */
  public String decryptSafe(String data) {
    ...
    try {
      ...
      org.slf4j.LoggerFactory.getLogger(EncryptionUtil.class)
          .warn(
              "decryptSafe: ciphertext-shaped input failed to decrypt (key mismatch?): {}",
              e.getMessage());
      return data;
    }
  }
  ...
  /**
   * Derive a 32-byte AES key from a key override string. Accepts either: - 32-character raw key
   * (used as UTF-8 bytes directly) - 64-character hex string (decoded to 32 bytes)
   */
