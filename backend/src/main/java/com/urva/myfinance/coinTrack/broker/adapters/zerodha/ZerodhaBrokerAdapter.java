 ...
 *
 * <p>Hexagonal Architecture: this is an "adapter" that translates between the Zerodha-specific wire
 * protocol and the canonical domain models.
 *
 * <p>All fetch methods return CompletableFuture for parallel execution. HTTP responses are
 * validated: 4xx/5xx throw typed exceptions.
 */
@Component
public class ZerodhaBrokerAdapter implements BrokerAdapter {
  ...
  public CompletableFuture<BrokerSession> authenticate(BrokerCredentials credentials) {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          try {
            String apiSecret =
                encryptionUtil.decrypt(zCreds.apiKey()); // apiKey here is the encrypted secret
            ...
            String responseBody =
                webClient
                    ...
                    .body(
                        BodyInserters.fromFormData("api_key", zCreds.apiKey())
            ...
            throw new BrokerAuthException(
                "Zerodha authentication failed", Broker.ZERODHA, e); // unreachable
          } catch (Exception e) {
            throw new BrokerAuthException(
                "Zerodha authentication failed: " + e.getMessage(), Broker.ZERODHA, e);
          }
        });
  }
  ...
  public CompletableFuture<List<CanonicalHolding>> fetchHoldings(BrokerSession session) {
    return CompletableFuture.supplyAsync(
        () -> {
          List<ZerodhaHoldingRaw> rawList =
              fetchListFromKite(
              ...
              .filter(
                  raw -> {
                    if (raw.getQuantity() != null && raw.getQuantity() < 0) {
                      log.warn(
                          "Skipping holding with negative quantity: {}", raw.getTradingsymbol());
                      return false;
                    }
                    return true;
                  })
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          List<Map<String, Object>> rawList =
              (List)
                  fetchListFromKite(session, KITE_BASE + "/mf/holdings", "MF Holdings", Map.class);
          ...
          return rawList.stream()
              ...
              .map(
                  dto ->
                      mfMapper.toCanonicalMfHolding(dto, session.accountId(), session.accountId()))
              .toList();
        });
  }
  ...
  public CompletableFuture<List<CanonicalMfOrder>> fetchMfOrders(BrokerSession session) {
    return CompletableFuture.supplyAsync(
        () -> {
          @SuppressWarnings({"unchecked", "rawtypes"})
          List<Map<String, Object>> rawList =
              (List) fetchListFromKite(session, KITE_BASE + "/mf/orders", "MF Orders", Map.class);
          ...
          return rawList.stream()
              ...
              .map(
                  dto -> mfMapper.toCanonicalMfOrder(dto, session.accountId(), session.accountId()))
              .toList();
        });
  }
  ...
  private <T> List<T> fetchListFromKite(
    ...
    try {
      ...
      throw new BrokerApiDownException(
          "Zerodha API unreachable fetching " + logTag, Broker.ZERODHA, e);
  ...
  private void handleWebClientError(WebClientResponseException e, String context) {
    ...
    throw new BrokerApiDownException(
        "Zerodha client error " + status.value() + " for " + context, Broker.ZERODHA, e);
  }
