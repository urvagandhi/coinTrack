...
public class TradingCalculatorServiceImpl
    implements com.urva.myfinance.coinTrack.calculator.service.TradingCalculatorService {
  ...
  @Autowired
  public TradingCalculatorServiceImpl(
      CalculatorConfigLoader configLoader, StatutoryChargesConfig statutoryConfig) {
  ...
  private BigDecimal getBrokerage(String broker, String type) {
    ...
    Double value =
        configLoader.getValue(assumptions, "brokerage." + broker.toLowerCase() + "." + type);
    return value != null ? BigDecimal.valueOf(value) : BigDecimal.ZERO;
  }

  /** Calculate brokerage and all charges. */
  public CalculatorResponse<BrokerageResponse> calculateBrokerage(
      BrokerageRequest request, boolean debug) {
    ...
    BigDecimal gstRate =
        statutoryConfig.getGst() != null
            ? statutoryConfig.getGst().getDefaultRate()
            : new BigDecimal("18");
    BigDecimal sebiChargesRate =
        statutoryConfig.getEffectiveRate(statutoryConfig.getSebiCharges(), now);
    ...
    if (statutoryConfig.getTransactionCharges() != null) {
      var exchangeCharges =
          statutoryConfig.getTransactionCharges().get(request.exchange().toLowerCase());
    ...
    switch (txnType) {
      case "DELIVERY" -> {
        brokerage =
            buyValue
                .add(sellValue)
                .multiply(getBrokerage(broker, "equityDelivery"))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
        stt =
            buyValue
                .add(sellValue)
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStt().getTrading().get("delivery"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
        stampDuty =
            buyValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStampDuty().getTrading().get("delivery"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
      }
      case "INTRADAY" -> {
        BigDecimal pBrokerage =
            buyValue
                .add(sellValue)
        ...
        stt =
            sellValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStt().getTrading().get("intraday"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
        stampDuty =
            buyValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStampDuty().getTrading().get("intraday"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
      }
      case "FUTURES" -> {
        ...
        stt =
            sellValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStt().getTrading().get("futuresSell"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
        stampDuty =
            buyValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStampDuty().getTrading().get("futures"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
      }
      case "OPTIONS" -> {
        brokerage = BigDecimal.valueOf(40);
        stt =
            sellValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStt().getTrading().get("optionsSell"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
        stampDuty =
            buyValue
                .multiply(
                    statutoryConfig.getEffectiveRate(
                        statutoryConfig.getStampDuty().getTrading().get("options"), now))
                .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
      }
      default -> {
    ...
    BigDecimal transactionCharges =
        turnover.multiply(transChargeRate).divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
    BigDecimal sebiCharges =
        turnover.multiply(sebiChargesRate).divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
    BigDecimal gst =
        brokerage
    ...
    BigDecimal totalCharges =
        brokerage.add(stt).add(transactionCharges).add(gst).add(sebiCharges).add(stampDuty);
    BigDecimal netProfit = grossProfit.subtract(totalCharges);
    BigDecimal breakeven =
        buyValue.add(totalCharges).divide(request.quantity(), 2, RoundingMode.HALF_EVEN);

    BrokerageResponse result =
        new BrokerageResponse(
            buyValue,
            sellValue,
            grossProfit,
            brokerage,
    ...
    return CalculatorResponse.success(
        CalculatorMetadata.of("brokerage", CATEGORY, List.of("Exchange: " + request.exchange())),
        result,
        null);
  }
  ...
  public CalculatorResponse<MarginResponse> calculateMargin(MarginRequest request, boolean debug) {
    ...
    Map<String, Object> marginConf =
        (Map<String, Object>) configLoader.getDefaultAssumptions().get("margin");
    Double leverage =
        request.leverage() != null
            ? request.leverage().doubleValue()
            : configLoader.getValue(
                marginConf, "defaultLeverage." + request.segmentType().toLowerCase());
    ...
    BigDecimal requiredMargin =
        tradeValue.divide(BigDecimal.valueOf(leverage), 2, RoundingMode.HALF_EVEN);
    ...
    BigDecimal marginPercent =
        BigDecimal.valueOf(100.0 / leverage).setScale(2, RoundingMode.HALF_EVEN);
    BigDecimal exposure = tradeValue;

    MarginResponse result =
        new MarginResponse(
            tradeValue,
            requiredMargin,
            marginPercent,
