...
/** SIP (Systematic Investment Plan) calculations. Stateless, thread-safe. */
@Component
public class SipMath {
  ...
  /** Calculate SIP Future Value. Formula: FV = P × [((1 + r)^n − 1) / r] × (1 + r) */
  public BigDecimal sipFutureValue(BigDecimal monthlyInvestment, BigDecimal annualRate, int years) {
    ...
    BigDecimal monthlyRate =
        annualRate.divide(HUNDRED, SCALE, ROUNDING).divide(TWELVE, SCALE, ROUNDING);
  ...
  /** Calculate Step-Up SIP Future Value. SIP amount increases by stepUpPercent each year. */
  public BigDecimal stepUpSipFutureValue(
    ...
    for (int year = 1; year <= years; year++) {
      ...
      for (int month = 1; month <= 12; month++) {
        ...
        BigDecimal fvOfThisContribution =
            currentSIP.multiply(MathUtil.pow(BigDecimal.ONE.add(monthlyRate), monthsRemaining));
  ...
  public BigDecimal stepUpSipTotalInvestment(
