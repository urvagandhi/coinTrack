 ...
 *
 * <p>Key Upstox quirks: - No MF API — MF capabilities are not declared and not implemented. -
 ...
 * Rate limit: 1000 req/min shared — X-RateLimit-Remaining is tracked for early warning. - All
 * prices are Float; numeric conversion goes through PriceNormalizer. - sell_quantity is a negative
 * Integer; mappers use the explicit `side` field.
 */
@Component
public class UpstoxBrokerAdapter implements BrokerAdapter {
  ...
  public CompletableFuture<BrokerSession> authenticate(BrokerCredentials credentials) {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          if (uCreds.apiKey() == null
              || uCreds.encryptedApiSecret() == null
              || uCreds.authorizationCode() == null
              || uCreds.redirectUri() == null) {
          ...
          try {
            ...
            String responseBody =
                webClient
                    ...
                    .body(
                        BodyInserters.fromFormData("code", uCreds.authorizationCode())
            ...
            if (tokenNode == null || tokenNode.isNull()) {
              throw new BrokerAuthException("Upstox response missing access_token", Broker.UPSTOX);
            }
            String accessToken = tokenNode.asText();
            String upstoxUserId =
                root.has("user_id") && !root.get("user_id").isNull()
                    ? root.get("user_id").asText()
                    : "upstox-user";

            return new BrokerSession(
                upstoxUserId, Broker.UPSTOX, accessToken, computeExpiry(), Map.of());
          } catch (BrokerAuthException e) {
            throw e;
          } catch (WebClientResponseException e) {
            handleWebClientError(e, "authenticate");
            throw new BrokerAuthException(
                "Upstox authentication failed", Broker.UPSTOX, e); // unreachable
          } catch (Exception e) {
            throw new BrokerAuthException(
  ...
  /**
   * Upstox tokens expire at ~3:30 AM IST the next day. If we're already past 3:30 AM today, the
   * next expiry is 3:30 AM tomorrow; otherwise it's 3:30 AM today.
   */
  ...
  public CompletableFuture<List<CanonicalPosition>> fetchPositions(BrokerSession session) {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          List<UpstoxPositionRaw> rawList =
              fetchListFromUpstox(
                  session.accessToken(),
   ...
   *
   * @throws BrokerException with a "session expired" message when the stored token is
   *     missing/expired.
   */
  public List<OrderDTO> fetchOrders(BrokerAccount account) {
    String accessToken = requireLiveToken(account);
    List<UpstoxOrderRaw> rawList =
        fetchListFromUpstox(
            accessToken, UPSTOX_BASE + "/order/retrieve-all", "orders", UpstoxOrderRaw.class);
  ...
  /**
   * Fetches funds/margins for the Upstox account in the Kite-shaped {@link FundsDTO} format used by
   * the /api/portfolio/funds endpoint.
   */
  public FundsDTO fetchFundsAsKite(BrokerAccount account) {
    String accessToken = requireLiveToken(account);
    UpstoxFundsRaw raw =
        fetchObjectFromUpstox(
            accessToken, UPSTOX_BASE + "/user/get-funds-and-margin", "funds", UpstoxFundsRaw.class);
  ...
  private <T> List<T> fetchListFromUpstox(
      String accessToken, String url, String logTag, Class<T> itemType) {
    try {
      ...
      throw new BrokerApiDownException(
          "Upstox API unreachable fetching " + logTag, Broker.UPSTOX, e);
  ...
  private <T> T fetchObjectFromUpstox(
      String accessToken, String url, String logTag, Class<T> responseType) {
    try {
      ResponseEntity<String> response =
          webClient
              .get()
  ...
  private void handleWebClientError(WebClientResponseException e, String logTag) {
    ...
    throw new BrokerApiDownException(
        "Upstox client error " + status.value() + " for " + logTag, Broker.UPSTOX, e);
  }
}
