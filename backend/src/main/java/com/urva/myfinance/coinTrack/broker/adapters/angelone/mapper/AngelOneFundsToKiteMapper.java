...
/**
 * Maps Angel One /user/v1/getRMS response to the Kite-flavored FundsDTO for the
 * /api/portfolio/funds endpoint.
 *
 * <p>Angel One returns a flat structure (availablecash, utilisedamount, net, …). Kite's wire shape
 * nests these under equity → available{cash, collateral, …} and equity → utilised{debits, ...}. We
 * project Angel One's flat fields into the equity segment only (commodity segment left null since
 * AngelOne RMS does not split equity vs commodity in the same response).
 */
@Component
public class AngelOneFundsToKiteMapper {
  ...
  public FundsDTO toKite(AngelOneFundsRaw raw) {
    ...
    BigDecimal availableCash =
        PriceNormalizer.toBigDecimal(raw.getAvailablecash(), "availablecash", Broker.ANGELONE);
    BigDecimal usedMargin =
        PriceNormalizer.toBigDecimal(raw.getUtilisedamount(), "utilisedamount", Broker.ANGELONE);
    BigDecimal net =
        raw.getNet() != null && !raw.getNet().isBlank()
    ...
    av.setCollateral(
        raw.getCollateral() != null && !raw.getCollateral().isBlank()
