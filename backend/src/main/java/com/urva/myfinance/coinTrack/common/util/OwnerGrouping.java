 ...
 * module-specific variants below) rather than inlining {@code a + "|" + b}, so the grouping rule can
 * never drift between, for example, the FD TDS summary, the FD Excel export, and the MF cross-check
 * aggregation.
 *
 * <p>Key rule: {@code groupKey(placeOrPlatform, holder) = placeOrPlatform + "|" + HolderName.normalize(holder)}.
 * Blank place/platform and blank holder collapse to the display fallback {@code "Unknown"} so a
 * missing value never silently becomes a bucket of its own.
 */
