...
public class GoldSilverServiceImpl implements GoldSilverService {
  ...
  public GoldSilverResponseDTO addInvestment(GoldSilverRequestDTO requestDTO, String userId) {
    ...
    RateSource rateSource =
        requestDTO.getRateSource() != null ? requestDTO.getRateSource() : RateSource.LIVE;

    GoldSilverInvestment investment =
  ...
  @Override
  public GoldSilverResponseDTO updateInvestment(
      String id, GoldSilverRequestDTO requestDTO, String userId) {
  ...
  @Override
  public GoldSilverResponseDTO updateRateMode(
    ...
    if (requestDTO.getRateSource() == RateSource.MANUAL) {
      if (requestDTO.getManualRate() != null
          && requestDTO.getManualRate().compareTo(BigDecimal.ZERO) > 0) {
        investment.setCurrentMarketRate(
            requestDTO.getManualRate().setScale(2, RoundingMode.HALF_UP));
      }
  ...
  public void updateMarketRate(MarketRateUpdateRequestDTO requestDTO, String userId) {
    Criteria criteria =
        Criteria.where("userId")
            .is(userId)
            .and("metalType")
            .is(requestDTO.getMetalType())
            .and("rateSource")
            .is(RateSource.MANUAL);
  ...
  public GoldSilverSummaryDTO getSummary(String userId) {
    ...
    if (sumNetAmount.compareTo(BigDecimal.ZERO) > 0) {
      overallReturn =
          totalPL
              .divide(sumNetAmount, 4, RoundingMode.HALF_UP)
  ...
  public List<GoldSilverResponseDTO> getAllForExport(
    ...
    return mongoTemplate.find(query, GoldSilverInvestment.class).stream()
  ...
  private void resolvePurityAndRates(
      GoldSilverInvestment investment, GoldSilverRequestDTO requestDTO) {
    ...
    if (optionId != null && !optionId.trim().isEmpty()) {
      Optional<PurityOption> opt =
          defaultPurityOptions.stream().filter(p -> p.getId().equals(optionId)).findFirst();
      if (opt.isPresent()) {
  ...
  private void recomputeLiveRateForRecord(GoldSilverInvestment investment) {
    Optional<MetalRateSnapshot> snapshotOpt =
        snapshotRepository.findFirstByMetalTypeOrderByFetchedAtDesc(investment.getMetalType());
    BigDecimal factor =
        investment.getPurityFactor() != null ? investment.getPurityFactor() : BigDecimal.ONE;
    ...
    if (snapshotOpt.isPresent() && snapshotOpt.get().getEffectiveBaseRate() != null) {
      ...
      BigDecimal computedRate =
          effectiveBaseRate.multiply(factor).setScale(2, RoundingMode.HALF_UP);
  ...
  private GoldSilverInvestment findAndVerifyOwnership(String id, String userId) {
    return repository
        ...
        .orElseThrow(
            () -> new DomainException("Investment not found or access denied", "NOT_FOUND", 404));
  }
  ...
  private GoldSilverResponseDTO toResponseDTO(GoldSilverInvestment inv) {
    ...
    Integer daysToMaturity =
        inv.getMaturityDate() != null
            ? (int) ChronoUnit.DAYS.between(today, inv.getMaturityDate())
            : null;
    ...
    Optional<MetalRateSnapshot> snapshotOpt =
        snapshotRepository.findFirstByMetalTypeOrderByFetchedAtDesc(inv.getMetalType());
    ...
    if (status != null) {
      if (status == GsStatus.ACTIVE) {
        criteria.andOperator(
            new Criteria()
                .orOperator(
                    Criteria.where("status").is(status), Criteria.where("status").exists(false)));
      } else {
