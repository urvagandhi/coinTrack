...
public class FinancialYearUtil {
  ...
  /**
   * Resolves a financial year string like "2025-26" into its start and end dates. Start date is
   * April 1st of the first year. End date is March 31st of the following year.
   */
  public static LocalDate[] resolveFinancialYear(String financialYear) {
    ...
    if (!matcher.matches()) {
      throw new IllegalArgumentException(
          "Invalid financial year format. Expected format: YYYY-YY (e.g. 2025-26)");
    }
    ...
    if (endYearShort != (startYear + 1) % 100) {
      throw new IllegalArgumentException(
          "Invalid financial year range. End year must be one year after start year.");
    }
