...
public class LoanCalculatorServiceImpl implements LoanCalculatorService {
  ...
  public CalculatorResponse<EmiResponse> calculateEmi(EmiRequest request, boolean debug) {
    ...
    if (debug) {
      ...
      DebugInfo debugInfo =
          DebugInfo.of(monthlyRate, request.months(), "E = P × r × (1+r)^n / ((1+r)^n − 1)");
  ...
  @Override
  public CalculatorResponse<SimpleInterestResponse> calculateSimpleInterest(
      SimpleInterestRequest request, boolean debug) {
    BigDecimal totalInterest =
        FinancialMath.simpleInterest(request.principal(), request.annualRate(), request.years());
    BigDecimal maturityAmount = request.principal().add(totalInterest);

    SimpleInterestResponse result =
        new SimpleInterestResponse(request.principal(), maturityAmount, totalInterest);
    CalculatorMetadata metadata =
        CalculatorMetadata.of(
            "simple-interest", CATEGORY, List.of("Simple calculation", "No compounding"));

    return CalculatorResponse.success(metadata, result, null);
  }

  @Override
  public CalculatorResponse<CompoundInterestResponse> calculateCompoundInterest(
    ...
    BigDecimal rate =
        request.annualRate().divide(BigDecimal.valueOf(100), 10, RoundingMode.HALF_EVEN);
    int n = request.compoundingFrequency();
    double effectiveRate = Math.pow(1 + rate.doubleValue() / n, n) - 1;
    BigDecimal effectiveAnnualRate =
        BigDecimal.valueOf(effectiveRate * 100).setScale(2, RoundingMode.HALF_EVEN);

    CompoundInterestResponse result =
        new CompoundInterestResponse(
            request.principal(),
            maturityAmount,
            totalInterest,
            effectiveAnnualRate,
    ...
    CalculatorMetadata metadata =
        CalculatorMetadata.of(
            "compound-interest", CATEGORY, List.of("Interest compounded " + compoundingDesc));
  ...
  @Override
  public CalculatorResponse<FlatVsReducingResponse> compareFlatVsReducing(
      EmiRequest request, boolean debug) {
    int years = request.months() / 12;

    // Reducing balance EMI (Standard)
    BigDecimal reducingEmi =
        FinancialMath.emi(request.principal(), request.annualRate(), request.months());
    ...
    // Flat rate EMI
    BigDecimal flatEmi =
        FinancialMath.flatRateEmi(
            request.principal(), request.annualRate(), years == 0 ? 1 : years);
    ...
    FlatVsReducingResponse result =
        new FlatVsReducingResponse(
            request.principal(),
            reducingEmi,
            reducingInterest,
            reducingTotal,
  ...
  private List<YearlyBreakdown> generateEmiBreakdown(EmiRequest request, BigDecimal emi) {
    ...
    BigDecimal monthlyRate =
        request.annualRate().divide(BigDecimal.valueOf(1200), 10, RoundingMode.HALF_EVEN);
    ...
    for (int year = 1; year <= (request.months() + 11) / 12; year++) {
      ...
      breakdown.add(
          new YearlyBreakdown(
              year,
              yearlyPrincipal.setScale(2, RoundingMode.HALF_EVEN),
              yearlyInterest.setScale(2, RoundingMode.HALF_EVEN),
  ...
  private List<YearlyBreakdown> generateCompoundInterestBreakdown(CompoundInterestRequest request) {
    ...
    BigDecimal ratePerPeriod =
        rate.divide(BigDecimal.valueOf(request.compoundingFrequency()), 10, RoundingMode.HALF_EVEN);
