...
/**
 * Migration runner that executes programmatically on application startup. Automatically cleans up
 * old or misnamed indexes on email, googleId, phoneNumber, and username, and ensures unique sparse
 * indexes exist with exact standard names matching Spring Data conventions.
 *
 * <p>Runs only once - tracked in schema_migrations collection.
 */
@Component
@Profile("!prod")
public class IndexMigration implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    try {
      ...
      if (!mongoTemplate.collectionExists(User.class)) {
        logger.info(
            "Users collection does not exist yet. Indexes will be auto-created by Spring Data on first insert.");
        logger.info("========================================");
      ...
      // 1. Drop existing single-field indexes on target fields if they have non-standard names or
      // non-sparse options
      List<Document> indexInfos =
          mongoTemplate.getDb().getCollection(collectionName).listIndexes().into(new ArrayList<>());
      for (Document indexInfo : indexInfos) {
        ...
        if (key != null) {
          for (String field : new String[] {"email", "googleId", "phoneNumber", "username"}) {
            // Only touch single-field indexes — never drop compound indexes that merely contain
            // these fields
            if (key.size() == 1 && key.containsKey(field)) {
              ...
              if (!field.equals(name) || !isUnique || !isSparse) {
                logger.info(
                    "Found legacy/incompatible index on {}: {}. Dropping it...", field, name);
                try {
      ...
      migrationService.recordMigration(
          MIGRATION_ID, "1.0.0", "User unique sparse index migration", indexesCreated);

    } catch (Exception e) {
      logger.error("Error during index migration: {}", e.getMessage());
      migrationService.recordMigrationFailure(
          MIGRATION_ID, "1.0.0", "User unique sparse index migration", e.getMessage());
      throw e;
    }
  ...
  private int ensureIndex(
      IndexOperations indexOps, Set<String> existingIndexNames, String fieldName) {
    ...
    try {
      logger.info(
          "Ensuring unique sparse index on {} exists with name '{}'...", fieldName, fieldName);
      indexOps.createIndex(
          new Index().on(fieldName, Sort.Direction.ASC).named(fieldName).unique().sparse());
