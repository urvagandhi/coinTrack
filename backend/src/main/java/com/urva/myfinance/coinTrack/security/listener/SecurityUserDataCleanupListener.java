...
/**
 * Removes blacklisted-token rows for a deleted user. Refresh tokens are revoked by the user module
 * itself before this event fires; the TTL index would eventually clean these rows anyway, but
 * immediate removal avoids any window where orphaned rows linger.
 */
@Component
public class SecurityUserDataCleanupListener {

  private static final Logger logger =
      LoggerFactory.getLogger(SecurityUserDataCleanupListener.class);
  ...
  public void onUserDeleted(UserDeletedEvent event) {
    try {
      ...
      logger.error(
          "Cascade cleanup failed for invalidated tokens, user {}: {}",
          event.userId(),
          e.getMessage());
    }
  }
