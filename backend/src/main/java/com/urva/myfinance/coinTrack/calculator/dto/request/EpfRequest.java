...
public record EpfRequest(
    ...
    @DecimalMin(value = "0", message = "Contribution cannot be negative")
        BigDecimal employeeContributionPercent,
  ...
  public EpfRequest {
    if (employeeContributionPercent == null) employeeContributionPercent = new BigDecimal("12");
    if (employerContributionPercent == null) employerContributionPercent = new BigDecimal("3.67");
    if (currentEpfBalance == null) currentEpfBalance = BigDecimal.ZERO;
    if (annualSalaryIncrease == null) annualSalaryIncrease = BigDecimal.ZERO;
  }
}
