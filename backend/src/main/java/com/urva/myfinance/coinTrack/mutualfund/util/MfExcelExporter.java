...
public class MfExcelExporter {
  ...
  public static ResponseEntity<byte[]> export(
    ...
    try (Workbook workbook = new XSSFWorkbook();
      ...
      httpHeaders.set(
          HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"Mutual_Funds_Ledger.xlsx\"");
  ...
  private static int renderSummaryHeader(
      Sheet sheet, Workbook workbook, OverallSummaryDto summary, int colCount) {
    if (summary == null) {
      return 0;
    }
    ...
    String[] sumHeaders = {
      "Total Invested",
      "Current Invested (Ledger)",
      "Total Redeemed",
      "Overall Valuation P&L",
      "Active SIPs Count"
    };
    ...
    double totInvested =
        summary.getTotalInvested() != null ? summary.getTotalInvested().doubleValue() : 0.0;
    double currentInvest =
        summary.getCurrentInvestment() != null ? summary.getCurrentInvestment().doubleValue() : 0.0;
    double totRedeemed =
        summary.getTotalRedeemed() != null ? summary.getTotalRedeemed().doubleValue() : 0.0;
  ...
  // ── Tab 1: MF Investment ──────────────────────────────────────────────────
  private static void createMfInvestmentSheet(
      Workbook workbook,
      String sheetName,
      List<SchemeSummaryDto> data,
      Map<String, MfScheme> schemeMap,
      OverallSummaryDto summary,
      List<SipContribution> sips,
    ...
    sortedData.sort(
        Comparator.comparing(
                (SchemeSummaryDto dto) -> dto.getPlatform() != null ? dto.getPlatform() : "")
            .thenComparing(
                (SchemeSummaryDto dto) -> {
                  Set<FundStatus> statuses =
                      dto.getStatuses() != null ? dto.getStatuses() : new HashSet<>();
                  if (statuses.contains(FundStatus.SIP)) return 1;
                  if (statuses.contains(FundStatus.LUMPSUM)
                      && !statuses.contains(FundStatus.FULLY_REDEEMED)) return 2;
                  return 3;
                })
    ...
    if (sips != null && !sips.isEmpty()) {
      for (SipContribution sip : sips) {
        ...
        sipMatrix
            .computeIfAbsent(sip.getSchemeId(), k -> new HashMap<>())
    ...
    if (!dynamicMonths.isEmpty()) {
      ...
      sheet.addMergedRegion(
          new CellRangeAddress(startRow, startRow, staticHeaders.length, totalCols - 1));
    }
    ...
    for (int i = 0; i < sortedData.size(); i++) {
      ...
      createCell(
          row,
          3,
          dto.getBank() != null ? dto.getBank() : (scheme != null ? scheme.getBank() : "-"),
          currentRowStyle);
      ...
      createNumericCell(
          row,
          7,
          dto.getTotalUnit() != null ? dto.getTotalUnit().doubleValue() : 0.0,
          currentUnitStyle);
      createNumericCell(
          row,
          8,
          dto.getTotalInvestment() != null ? dto.getTotalInvestment().doubleValue() : 0.0,
          currentCurStyle);
      createNumericCell(
          row,
          9,
          dto.getCurrentInvestment() != null ? dto.getCurrentInvestment().doubleValue() : 0.0,
          currentCurStyle);
      createNumericCell(
          row,
          10,
          dto.getTotalTradedValue() != null ? dto.getTotalTradedValue().doubleValue() : 0.0,
          currentCurStyle);
      createNumericCell(
          row,
          11,
          dto.getLumpsumInvestment() != null ? dto.getLumpsumInvestment().doubleValue() : 0.0,
          currentCurStyle);
      createNumericCell(
          row,
          12,
          dto.getSipInvestment() != null ? dto.getSipInvestment().doubleValue() : 0.0,
          currentCurStyle);

      Map<YearMonth, BigDecimal> schemeSips =
          sipMatrix.getOrDefault(dto.getSchemeId(), Collections.emptyMap());
  ...
  // ── Tab 2: Investment & Valuation ──────────────────────────────────────────
  private static void createValuationSheet(
      Workbook workbook,
      String sheetName,
      List<ValuationSnapshot> data,
      CellStyle headerStyle,
      CellStyle dataStyle,
      CellStyle currencyStyle,
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      createNumericCell(
          row,
          3,
          snap.getInvestmentValue() != null ? snap.getInvestmentValue().doubleValue() : 0.0,
          currencyStyle);
      createNumericCell(
          row,
          4,
          snap.getCurrentValue() != null ? snap.getCurrentValue().doubleValue() : 0.0,
          currencyStyle);
      createNumericCell(
          row,
          5,
          snap.getPeriodPL() != null ? snap.getPeriodPL().doubleValue() : 0.0,
          currencyStyle);
      createNumericCell(
          row,
          6,
    ...
    if (!data.isEmpty()) {
      ...
      double totInvested =
          data.stream()
              .mapToDouble(
                  d -> d.getInvestmentValue() != null ? d.getInvestmentValue().doubleValue() : 0.0)
              .sum();
      double totCurrent =
          data.stream()
              .mapToDouble(
                  d -> d.getCurrentValue() != null ? d.getCurrentValue().doubleValue() : 0.0)
              .sum();
      double totPL =
          data.stream()
              .mapToDouble(d -> d.getPeriodPL() != null ? d.getPeriodPL().doubleValue() : 0.0)
              .sum();
  ...
  // ── Tab 3: Lumpsum Investment ──────────────────────────────────────────────
  private static void createLumpsumSheet(
      Workbook workbook,
      String sheetName,
      List<LumpsumTransaction> data,
      Map<String, MfScheme> schemeMap,
      CellStyle headerStyle,
      CellStyle dataStyle,
      CellStyle currencyStyle,
    ...
    for (LumpsumTransaction tx : sortedData) {
      String fy =
          tx.getInvestmentDate() != null ? getFinancialYear(tx.getInvestmentDate()) : "Unknown";
    ...
    for (Map.Entry<String, List<LumpsumTransaction>> entry : fyMap.entrySet()) {
      ...
      sheet.addMergedRegion(
          new CellRangeAddress(fyRow.getRowNum(), fyRow.getRowNum(), 0, headers.length - 1));
      ...
      for (LumpsumTransaction tx : txs) {
        ...
        double amt =
            tx.getLumpsumInvestment() != null ? tx.getLumpsumInvestment().doubleValue() : 0.0;
        createNumericCell(row, 5, amt, currencyStyle);
        fyTotLumpsum += amt;

        createNumericCell(
            row, 6, tx.getNavPrice() != null ? tx.getNavPrice().doubleValue() : 0.0, currencyStyle);
  ...
  // ── Tab 4: Redemption Investment ───────────────────────────────────────────
  private static void createRedemptionSheet(
      Workbook workbook,
      String sheetName,
      List<RedemptionTransaction> data,
      Map<String, MfScheme> schemeMap,
      CellStyle headerStyle,
      CellStyle dataStyle,
      CellStyle currencyStyle,
    ...
    for (RedemptionTransaction tx : sortedData) {
      String fy =
          tx.getRedemptionDate() != null ? getFinancialYear(tx.getRedemptionDate()) : "Unknown";
      fyMap.computeIfAbsent(fy, k -> new java.util.ArrayList<>()).add(tx);
    }
    ...
    for (Map.Entry<String, List<RedemptionTransaction>> entry : fyMap.entrySet()) {
      ...
      for (RedemptionTransaction tx : txs) {
        ...
        createNumericCell(
            row, 5, tx.getTotalUnit() != null ? tx.getTotalUnit().doubleValue() : 0.0, unitStyle);
        createNumericCell(
            row,
            6,
            tx.getRedemptionUnit() != null ? tx.getRedemptionUnit().doubleValue() : 0.0,
            unitStyle);
        createNumericCell(
            row,
            7,
            tx.getBalanceUnit() != null ? tx.getBalanceUnit().doubleValue() : 0.0,
            unitStyle);

        createNumericCell(
            row,
            8,
            tx.getTotalInvestment() != null ? tx.getTotalInvestment().doubleValue() : 0.0,
            currencyStyle);
        createNumericCell(
            row,
            9,
            tx.getTradeInvestmentValue() != null ? tx.getTradeInvestmentValue().doubleValue() : 0.0,
            currencyStyle);
        createNumericCell(
            row,
            10,
            tx.getBalanceInvestment() != null ? tx.getBalanceInvestment().doubleValue() : 0.0,
            currencyStyle);

        createNumericCell(
            row,
            11,
            tx.getRedemptionNav() != null ? tx.getRedemptionNav().doubleValue() : 0.0,
            dataStyle); // NAV

        double redVal =
            tx.getNetRedemptionValue() != null
                ? tx.getNetRedemptionValue().doubleValue()
                : (tx.getRedemptionValue() != null ? tx.getRedemptionValue().doubleValue() : 0.0);

        // NOTE: Kept for reference. Using multiline strings breaks Excel's ability to sum rows
        // natively.
        /*
  ...
  // ── Tab 5: SIP Details ────────────────────────────────────────────────────
  private static void createSipSheet(
      Workbook workbook,
      String sheetName,
      List<SipContribution> data,
      Map<String, MfScheme> schemeMap,
      CellStyle headerStyle,
      CellStyle dataStyle,
      CellStyle currencyStyle,
      CellStyle boldStyle) {
    Sheet sheet = workbook.createSheet(sheetName);
    String[] headers = {"Holder Name", "Scheme Name", "Contribution Month", "Amount", "Remarks"};
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      createNumericCell(
          row, 3, tx.getAmount() != null ? tx.getAmount().doubleValue() : 0.0, currencyStyle);
  ...
  private static String getFinancialYear(LocalDate date) {
    if (date == null) return "Unknown FY";
  ...
  private static String formatDate(LocalDate date) {
    ...
    return String.format(
        "%02d.%02d.%d", date.getDayOfMonth(), date.getMonthValue(), date.getYear());
  }
