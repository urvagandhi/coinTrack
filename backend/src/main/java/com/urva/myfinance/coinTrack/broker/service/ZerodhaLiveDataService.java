...
/**
 * Provides live data fetching from Zerodha Kite API for pass-through endpoints. This is NOT a
 * BrokerAdapter — it returns Kite DTOs directly for the REST API contract.
 *
 * <p>This service replaces ZerodhaBrokerService for live API pass-through use cases. It lives in
 * broker/service/ (not broker/adapters/) so portfolio/ can import it. Uses WebClient for all HTTP
 * calls.
 */
@Service
public class ZerodhaLiveDataService {
  ...
  private static final String KITE_LOGIN_URL =
  ...
  @Autowired
  public ZerodhaLiveDataService(
      WebClient.Builder brokerWebClientBuilder, EncryptionUtil encryptionUtil) {
   ...
   *
   * <p>IDEMPOTENT: Same request_token always returns same access_token.
   *
  ...
  public Map<String, Object> exchangeToken(
      String requestToken, String apiKey, String encryptedApiSecret) {
  ...
  /**
   * Extracts the token expiry time for a Zerodha account. Zerodha tokens expire at ~6:00 AM IST
   * daily.
   *
  ...
  /**
   * Fetches funds/margins from Zerodha Kite API. Uses manual parsing to inject raw data into
   * equity/commodity segments.
   *
   * @param account the broker account with valid token
    ...
    try {
      ...
      if (dataNode != null) {
        if (fundsDTO.getEquity() != null && dataNode.has("equity")) {
          Map<String, Object> equityRaw =
              objectMapper.convertValue(dataNode.get("equity"), Map.class);
          fundsDTO.getEquity().setRaw(equityRaw);
        }
        if (fundsDTO.getCommodity() != null && dataNode.has("commodity")) {
          Map<String, Object> commodityRaw =
              objectMapper.convertValue(dataNode.get("commodity"), Map.class);
  ...
  /**
   * Fetches mutual fund holdings from Zerodha Kite API. Uses manual mapping from raw maps to
   * MutualFundDTO.
   *
   * @param account the broker account with valid token
  ...
  public List<MutualFundDTO> fetchMfHoldings(BrokerAccount account) {
    ...
    List<Map<String, Object>> rawList =
        (List)
            fetchListFromKite(
                account, "https://api.kite.trade/mf/holdings", "MF Holdings", Map.class);
  ...
  /**
   * Fetches mutual fund orders from Zerodha Kite API. Uses manual mapping from raw maps to
   * MutualFundOrderDTO.
   *
   * @param account the broker account with valid token
  ...
  public List<MutualFundOrderDTO> fetchMfOrders(BrokerAccount account) {
    ...
    List<Map<String, Object>> rawList =
        (List)
            fetchListFromKite(account, "https://api.kite.trade/mf/orders", "MF Orders", Map.class);
  ...
  public UserProfileDTO fetchProfile(BrokerAccount account) {
    Map<String, Object> rawMap =
        fetchObjectFromKite(account, "https://api.kite.trade/user/profile", "profile", Map.class);
  ...
  /**
   * Fetches mutual fund SIPs from Zerodha Kite API. Uses manual mapping from raw maps to MfSipDTO.
   *
   * @param account the broker account with valid token
  ...
  public List<MfSipDTO> fetchMfSips(BrokerAccount account) {
    @SuppressWarnings({"unchecked", "rawtypes"})
    List<Map<String, Object>> rawList =
        (List) fetchListFromKite(account, "https://api.kite.trade/mf/sips", "MF SIPs", Map.class);
  ...
  /**
   * Fetches mutual fund instruments from Zerodha Kite API. Note: This endpoint returns CSV (not
   * JSON!).
   *
   * @param account the broker account with valid token
  ...
  public List<MfInstrumentDTO> fetchMfInstruments(BrokerAccount account) {
    ...
    try {
      ...
      for (int i = 1; i < lines.length; i++) {
        ...
        dto.setRedemptionQuantityMultiplier(
            safeBigDecimal(str.apply("redemption_quantity_multiplier")));
   ...
   */
  private <T> List<T> fetchListFromKite(
      BrokerAccount account, String url, String logTag, Class<T> itemType) {
    if (!account.hasValidToken()) {
      throw new BrokerException("Invalid token", Broker.ZERODHA);
    }
   ...
   */
  private <T> T fetchObjectFromKite(
      BrokerAccount account, String url, String logTag, Class<T> responseType) {
    if (!account.hasValidToken()) {
      throw new BrokerException("Invalid token", Broker.ZERODHA);
    }
  ...
  /**
   * Builds Kite API authentication headers. Authorization format: "token {apiKey}:{accessToken}"
   *
   * @param headers the HTTP headers to populate
   * @param account the broker account containing API key and access token
   */
  private void buildKiteHeaders(
      org.springframework.http.HttpHeaders headers, BrokerAccount account) {
