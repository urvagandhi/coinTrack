...
/**
 * Maps Upstox /order/retrieve-all rows to the shared Kite-flavored OrderDTO wire format. Used by
 * the orders/trades/funds pass-through path.
 *
 * <p>Upstox quirks: - Prices are Float — routed through PriceNormalizer to preserve precision. -
 * Exchange "NSE_EQ"/"BSE_EQ" is normalized to "NSE"/"BSE" so the UI doesn't render the segment
 * suffix. - Timestamps arrive as "yyyy-MM-dd HH:mm:ss" strings; parsed leniently — null on parse
 * failure.
 */
@Component
public class UpstoxOrderMapper {
  ...
  public OrderDTO toKite(UpstoxOrderRaw raw) {
    ...
    dto.setTriggerPrice(
        PriceNormalizer.toBigDecimal(raw.getTriggerPrice(), "trigger_price", Broker.UPSTOX));
