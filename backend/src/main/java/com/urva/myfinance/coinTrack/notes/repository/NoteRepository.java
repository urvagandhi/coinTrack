...
public interface NoteRepository extends MongoRepository<Note, String> {
  ...
  /**
   * Case-insensitive substring search over title and content, scoped to one user. Implemented with
   * $regex (NOT $text) so partial words match; callers MUST pass the term through Pattern.quote —
   * the raw value is interpolated into the regex.
   */
  @Query(
