...
/**
 * Maps Upstox /user/get-funds-and-margin response to the Kite-flavored FundsDTO for the
 * /api/portfolio/funds endpoint.
 *
 * <p>Upstox returns {equity:{...}, commodity:{...}} with flat fields (available_margin,
 * used_margin, …). Kite's wire shape nests them under available{cash, collateral, …} and
 * utilised{debits, span, exposure, …}. We project Upstox flat fields into the closest Kite slots
 * and set net = available + used.
 */
@Component
public class UpstoxFundsToKiteMapper {
  ...
  private FundsDTO.SegmentFundsDTO mapSegment(UpstoxFundsRaw.EquityFunds segment) {
    ...
    BigDecimal used =
        PriceNormalizer.toBigDecimal(segment.getUsedMargin(), "used_margin", Broker.UPSTOX);
    ...
    ut.setDebits(used);
    ut.setSpan(
        segment.getSpanMargin() != null
            ? PriceNormalizer.toBigDecimal(segment.getSpanMargin(), "span_margin", Broker.UPSTOX)
            : null);
    ut.setExposure(
        segment.getExposureMargin() != null
            ? PriceNormalizer.toBigDecimal(
                segment.getExposureMargin(), "exposure_margin", Broker.UPSTOX)
            : null);
    out.setUtilised(ut);
