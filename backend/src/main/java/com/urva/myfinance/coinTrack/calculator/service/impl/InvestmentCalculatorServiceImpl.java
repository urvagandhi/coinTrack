...
/**
 * Implementation of Investment Calculator Service. Uses FinancialMath utility for all calculations.
 */
@Service
public class InvestmentCalculatorServiceImpl implements InvestmentCalculatorService {

  private static final Logger logger =
      LoggerFactory.getLogger(InvestmentCalculatorServiceImpl.class);
  ...
  public CalculatorResponse<SipResponse> calculateSip(SipRequest request, boolean debug) {
    logger.debug(
        "Calculating SIP: monthly={}, rate={}%, years={}",
    ...
    BigDecimal totalInvestment =
        request.monthlyInvestment().multiply(BigDecimal.valueOf(request.years() * 12));
    BigDecimal totalGains = futureValue.subtract(totalInvestment);
    BigDecimal absoluteReturns =
        totalGains
    ...
    SipResponse result =
        new SipResponse(totalInvestment, futureValue, totalGains, absoluteReturns, cagr);
    ...
    if (debug) {
      ...
      DebugInfo debugInfo =
          DebugInfo.of(
              monthlyRate, request.years() * 12, "FV = P × [((1 + r)^n − 1) / r] × (1 + r)");
  ...
  @Override
  public CalculatorResponse<SipResponse> calculateStepUpSip(
      StepUpSipRequest request, boolean debug) {
    logger.debug(
        "Calculating Step-Up SIP: initial={}, step-up={}%, rate={}%, years={}",
    ...
    BigDecimal futureValue =
        FinancialMath.stepUpSipFutureValue(
            request.monthlyInvestment(),
            request.expectedReturn(),
            request.years(),
            request.stepUpPercent());
    ...
    BigDecimal totalInvestment =
        FinancialMath.stepUpSipTotalInvestment(
            request.monthlyInvestment(), request.years(), request.stepUpPercent());
    ...
    if (debug) {
      ...
      DebugInfo debugInfo =
          DebugInfo.of(
              monthlyRate, request.years() * 12, "Sum of year-wise SIP with annual step-up");
      return CalculatorResponse.successWithDebug(metadata, result, breakdown, debugInfo);
    }
  ...
  @Override
  public CalculatorResponse<LumpsumResponse> calculateLumpsum(
      LumpsumRequest request, boolean debug) {
    logger.debug(
        "Calculating Lumpsum: principal={}, rate={}%, years={}",
        request.principal(), request.expectedReturn(), request.years());

    BigDecimal futureValue =
        FinancialMath.lumpsumFutureValue(
            request.principal(), request.expectedReturn(), request.years());
    ...
    LumpsumResponse result =
        new LumpsumResponse(request.principal(), futureValue, totalGains, absoluteReturns);

    CalculatorMetadata metadata =
        CalculatorMetadata.of(
            "lumpsum",
            CATEGORY,
            List.of("Returns are compounded annually", "No exit load or taxes considered"));
    ...
    if (debug) {
      DebugInfo debugInfo =
          DebugInfo.of(
              request.expectedReturn().divide(BigDecimal.valueOf(100), 10, RoundingMode.HALF_EVEN),
              request.years(),
              "FV = P × (1 + r)^n");
      return CalculatorResponse.successWithDebug(metadata, result, breakdown, debugInfo);
    }
  ...
  public CalculatorResponse<CagrResponse> calculateCagr(CagrRequest request, boolean debug) {
    logger.debug(
        "Calculating CAGR: initial={}, final={}, years={}",
        request.initialValue(),
        request.finalValue(),
        request.years());

    BigDecimal cagr =
        FinancialMath.cagr(request.initialValue(), request.finalValue(), request.years());

    CagrResponse result =
        new CagrResponse(request.initialValue(), request.finalValue(), request.years(), cagr);
    ...
    CalculatorMetadata metadata =
        CalculatorMetadata.of(
            ...
            CATEGORY,
            List.of(
                "CAGR represents the mean annual growth rate", "Assumes reinvestment of returns"));
  ...
  public CalculatorResponse<XirrResponse> calculateXirr(XirrRequest request, boolean debug) {
    ...
    List<FinancialMath.CashFlow> flows =
  ...
  @Override
  public CalculatorResponse<StockAverageResponse> calculateStockAverage(
      StockAverageRequest request, boolean debug) {
    logger.debug(
        "Calculating Stock Average: existing={}/{}, new={}/{}",
        request.existingQuantity(),
    ...
    StockAverageResponse result =
        new StockAverageResponse(
            totalQuantity,
  ...
  @Override
  public CalculatorResponse<InflationResponse> calculateInflation(
      InflationRequest request, boolean debug) {
    logger.debug(
        "Calculating Inflation: value={}, rate={}%, years={}",
        request.presentValue(), request.inflationRate(), request.years());

    BigDecimal futureValue =
        FinancialMath.inflationAdjustedFuture(
            request.presentValue(), request.inflationRate(), request.years());

    BigDecimal purchasingPowerLoss = futureValue.subtract(request.presentValue());

    InflationResponse result =
        new InflationResponse(
    ...
    CalculatorMetadata metadata =
        CalculatorMetadata.of(
            ...
            CATEGORY,
            List.of(
                "Calculates future value based on annual inflation rate", "FV = PV × (1 + r)^n"));

    return CalculatorResponse.success(metadata, result, null);
  }

  @Override
  public CalculatorResponse<SipResponse> calculateMutualFundReturns(
      SipRequest request, boolean debug) {
