 ...
 *
 * <p>Upstox quirks: - Prices are Float — converted via PriceNormalizer (Float.toString() to avoid
 * precision loss) - exchange: "NSE_EQ" → NSE, "BSE_EQ" → BSE via ExchangeNormalizer - ISIN also
 * extractable from instrument_token format "NSE_EQ|INE002A01018" - dayChange and
 * dayChangePercentage are provided (unlike Angel One)
 */
@Component
public class UpstoxHoldingMapper {
  ...
  public CanonicalHolding toCanonical(UpstoxHoldingRaw raw, String userId, String brokerAccountId) {
    ...
    if (isin == null || isin.isBlank()) {
      ...
      if (isin == null || isin.isBlank()) {
        log.warn(
            "Upstox holding missing ISIN for tradingSymbol={}, instrumentToken={}",
            raw.getTradingSymbol(),
            raw.getInstrumentToken());
    ...
    BigDecimal dayChangePct =
        raw.getDayChangePercentage() != null
            ? PriceNormalizer.toBigDecimal(
                raw.getDayChangePercentage(), "day_change_percentage", Broker.UPSTOX)
            : null;
