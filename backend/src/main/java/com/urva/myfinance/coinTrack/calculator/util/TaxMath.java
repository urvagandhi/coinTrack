...
/** Tax calculations (Income Tax, HRA, GST, TDS, Gratuity). Stateless, thread-safe. */
@Component
public class TaxMath {
  ...
  /**
   * Calculate HRA exemption. Exemption = min(actualHRA, percentOfSalary, rentMinus10Pct)
   *
   * @param isMetro true for metro (50%), false for non-metro (40%)
   */
  public BigDecimal hraExemption(
      BigDecimal salary, BigDecimal actualHra, BigDecimal rentPaid, boolean isMetro) {
  ...
  /** Calculate Gratuity. Formula: (Last salary × 15 × Years) / 26 */
