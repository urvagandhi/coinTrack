...
public class EpfBalanceRecalculationService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(EpfBalanceRecalculationService.class);
  ...
  /**
   * Recalculates the dual running balances (EPF and EPS) for a given user. Must be called within a
   * MongoDB transaction context or be annotated with @Transactional.
   */
  @Transactional
  public void recalculateLedger(String userId) {
    ...
    for (EpfTransaction txn : sortedTransactions) {
      BigDecimal empContr =
          txn.getEmployeeContribution() != null ? txn.getEmployeeContribution() : BigDecimal.ZERO;
      BigDecimal emprEpfContr =
      ...
      BigDecimal emprEpsContr =
          txn.getEmployerEpsContribution() != null
              ? txn.getEmployerEpsContribution()
              : BigDecimal.ZERO;

      runningEpfBalance =
          runningEpfBalance.add(empContr).add(emprEpfContr).add(vpf).subtract(withdrawal);
      ...
      if (runningEpfBalance.compareTo(BigDecimal.ZERO) < 0) {
        throw new InsufficientEpfBalanceException(
            "Transaction on "
                ...
                + " results in a negative EPF balance: "
                + runningEpfBalance);
      }
      if (runningEpsBalance.compareTo(BigDecimal.ZERO) < 0) {
        throw new InsufficientEpfBalanceException(
            "Transaction on "
                + txn.getTransactionDate()
                + " results in a negative EPS balance: "
                + runningEpsBalance);
      }

      boolean epfChanged =
          txn.getEpfBalance() == null || txn.getEpfBalance().compareTo(runningEpfBalance) != 0;
      boolean epsChanged =
          txn.getEpsBalance() == null || txn.getEpsBalance().compareTo(runningEpsBalance) != 0;
