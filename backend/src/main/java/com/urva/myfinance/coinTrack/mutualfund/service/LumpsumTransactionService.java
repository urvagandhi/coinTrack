...
public class LumpsumTransactionService {
  ...
  @Autowired private LumpsumTransactionRepository repository;
  ...
  public List<LumpsumTransaction> getTransactionsByDateRange(
  ...
  public LumpsumTransaction createTransaction(String userId, LumpsumTransaction transaction) {
    ...
    LocalDate applicableDate =
        settlementDateCalculator.calculateApplicableDate(
            transaction.getInvestmentDate(), transaction.getIsAfterCutoff());
    ...
    schemeRepository
        ...
        .ifPresent(
            scheme -> {
              ...
              if (transaction.getDebitedBank() == null
                  || transaction.getDebitedBank().trim().isEmpty()) {
                    ...
                    if (transaction.getLumpsumInvestment() != null) {
                      BigDecimal netInvestment =
                          transaction
                              .getLumpsumInvestment()
                              .subtract(
                                  transaction.getStampDuty() != null
                                      ? transaction.getStampDuty()
                                      : BigDecimal.ZERO);
                      BigDecimal units =
    ...
    if (saved.getStatus() == TransactionStatus.COMPLETED) {
      ...
      redemptionTransactionService.recalculateRedemptionsAfterDate(
          userId, saved.getSchemeId(), saved.getInvestmentDate());
    }
  ...
  @org.springframework.transaction.annotation.Transactional
  public LumpsumTransaction updateTransaction(
      String userId, String id, LumpsumTransaction transaction) {
    ...
    if (!oldSchemeId.equals(saved.getSchemeId())) {
      ...
      if (saved.getStatus() == TransactionStatus.COMPLETED) {
        LocalDate earliestDate =
            oldDate.isBefore(saved.getInvestmentDate()) ? oldDate : saved.getInvestmentDate();
        redemptionTransactionService.recalculateRedemptionsAfterDate(
  ...
  public void deleteTransaction(String userId, String id) {
    ...
    if (existing.getStatus() == TransactionStatus.COMPLETED) {
      redemptionTransactionService.recalculateRedemptionsAfterDate(
          userId, existing.getSchemeId(), existing.getInvestmentDate());
    }
    transactionSequenceService.reorderLumpsumTransactions(userId);
  }
}
