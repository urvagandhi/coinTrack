 ...
 *
 * <p>Zerodha MF quirks: - ISIN must start with "INF" for mutual funds — logged as warning if not -
 * Quantity and prices are already BigDecimal in the raw DTO - Order dates come as Strings — parsed
 * via DateNormalizer
 */
@Component
public class ZerodhaMfMapper {
  ...
  public CanonicalMfHolding toCanonicalMfHolding(
      ZerodhaMfHoldingRaw raw, String userId, String brokerAccountId) {
    // ISIN validation
    String isin = raw.getIsin();
    if (isin != null && !isin.startsWith("INF")) {
      log.warn(
          "Zerodha MF holding ISIN='{}' does not start with 'INF' for fund={}",
          isin,
          raw.getFund());
    }
    ...
    BigDecimal units =
        raw.getQuantity() != null
            ? new BigDecimal(Double.toString(raw.getQuantity())).setScale(4, RoundingMode.HALF_UP)
            : BigDecimal.ZERO;

    // Average NAV (purchase price — double)
    BigDecimal avgNav =
        raw.getAveragePrice() != null
            ? new BigDecimal(Double.toString(raw.getAveragePrice()))
                .setScale(4, RoundingMode.HALF_UP)
            : BigDecimal.ZERO;

    // Current NAV (last_price — double)
    BigDecimal currentNav =
        raw.getCurrentPrice() != null
            ? new BigDecimal(Double.toString(raw.getCurrentPrice()))
                .setScale(4, RoundingMode.HALF_UP)
            : BigDecimal.ZERO;
    ...
    BigDecimal unrealizedPnL =
        currentValue.subtract(investedValue).setScale(2, RoundingMode.HALF_UP);
  ...
  public CanonicalMfOrder toCanonicalMfOrder(
      ZerodhaMfOrderRaw raw, String userId, String brokerAccountId) {
    ...
    BigDecimal executedQuantity =
        raw.getExecutedQuantity() != null
            ? raw.getExecutedQuantity().setScale(4, RoundingMode.HALF_UP)
            : BigDecimal.ZERO;

    // Executed NAV
    BigDecimal executedNav =
        raw.getExecutedNav() != null
