...
public class PlanningCalculatorServiceImpl
    implements com.urva.myfinance.coinTrack.calculator.service.PlanningCalculatorService {
  ...
  public CalculatorResponse<RetirementResponse> calculateRetirement(
      RetirementRequest request, boolean debug) {
    int retireAge =
        request.retirementAge() != null
            ? request.retirementAge()
            : getAssumption("defaultRetirementAge");
    int lifeExp =
        request.lifeExpectancy() != null
            ? request.lifeExpectancy()
            : getAssumption("defaultLifeExpectancy");
    BigDecimal inflation =
        request.expectedInflation() != null
            ? request.expectedInflation()
            : BigDecimal.valueOf((Double) getAssumption("defaultInflation"));
    BigDecimal preReturn =
        request.preRetirementReturn() != null
            ? request.preRetirementReturn()
            : BigDecimal.valueOf((Double) getAssumption("defaultPreRetirementReturn"));
    BigDecimal postReturn =
        request.postRetirementReturn() != null
            ? request.postRetirementReturn()
    ...
    BigDecimal monthlyExpenseAtRetirement =
        FinancialMath.inflationAdjustedFuture(
            request.currentMonthlyExpense(), inflation, yearsToRetirement);

    BigDecimal realReturnRate = calculateRealReturn(postReturn, inflation);
    BigDecimal annualExpenseAtRetirement =
        monthlyExpenseAtRetirement.multiply(BigDecimal.valueOf(12));
    BigDecimal corpusRequired =
        calculatePresentValueOfAnnuity(
            annualExpenseAtRetirement, realReturnRate, yearsInRetirement);

    // Add longevity buffer
    Integer bufferPercent = getAssumption("longevityBuffer");
    BigDecimal longevityBuffer =
        corpusRequired
            .multiply(BigDecimal.valueOf(bufferPercent))
            .divide(HUNDRED, 2, RoundingMode.HALF_EVEN);
    corpusRequired = corpusRequired.add(longevityBuffer);

    BigDecimal currentSavingsFV =
        request.currentSavings().compareTo(BigDecimal.ZERO) > 0
            ? FinancialMath.lumpsumFutureValue(
                request.currentSavings(), preReturn, yearsToRetirement)
            : BigDecimal.ZERO;
    ...
    List<YearlyProjection> projection =
        generateProjection(
            request,
            yearsToRetirement,
            yearsInRetirement,
    ...
    return CalculatorResponse.success(
        CalculatorMetadata.of("retirement", CATEGORY, List.of("Inflation: " + inflation + "%")),
        result,
        null);
  }
  ...
  private BigDecimal calculatePresentValueOfAnnuity(
      BigDecimal annualPayment, BigDecimal rate, int years) {
  ...
  private BigDecimal calculateRequiredSip(
      BigDecimal targetCorpus, BigDecimal annualRate, int years) {
    double r = annualRate.doubleValue() / 1200;
    int n = years * 12;
    if (r == 0) return targetCorpus.divide(BigDecimal.valueOf(n), 2, RoundingMode.HALF_EVEN);
    double factor = (Math.pow(1 + r, n) - 1) / r * (1 + r);
    return targetCorpus.divide(BigDecimal.valueOf(factor), 2, RoundingMode.HALF_EVEN);
  }

  private List<YearlyProjection> generateProjection(
