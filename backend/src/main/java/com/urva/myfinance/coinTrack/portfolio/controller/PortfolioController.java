...
/**
 * REST controller for portfolio operations. Uses Principal from SecurityContext for user
 * identification (not getUserByToken).
 */
@RestController
@RequestMapping("/api/portfolio")
@Tag(
    name = "Portfolio",
    description = "Portfolio summary, holdings, positions, funds, and mutual funds")
public class PortfolioController {
  ...
  /**
   * Get portfolio summary for authenticated user. Uses SecurityContext Principal instead of manual
   * token parsing.
   */
  ...
  /**
   * Get consolidated holdings (from all brokers). Filtered from Summary service to ensure
   * consistency.
   */
  ...
  public ResponseEntity<?> getHoldings(Principal principal) {
    ...
    List<com.urva.myfinance.coinTrack.portfolio.dto.SummaryHoldingDTO> holdings =
        summary.getHoldingsList().stream()
  ...
  /**
   * Get net positions for authenticated user. Renamed from /net-positions to /positions to match
   * canonical API.
   */
  ...
  public ResponseEntity<?> getMutualFunds(Principal principal) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(portfolioSummaryService.getMutualFunds(user.getId())));
  }
  ...
  public ResponseEntity<?> getMfOrders(Principal principal) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(portfolioSummaryService.getMfOrders(user.getId())));
  }
  ...
  public ResponseEntity<?> getMfTimeline(Principal principal) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(portfolioSummaryService.getMfTimeline(user.getId())));
  }
  ...
  public ResponseEntity<?> getMfInstruments(Principal principal) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(portfolioSummaryService.getMfInstruments(user.getId())));
  }
}
