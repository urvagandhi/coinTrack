 ...
 *
 * <p>Angel One quirks handled: - averageprice, ltp, close, profitandloss are Double (not String) -
 * quantity, t1quantity are Integer (not String) - profitandloss is COMBINED realized + unrealized
 * (cannot split) - dayChange/dayChangePct NOT provided by Angel One - isin null for SME stocks →
 * fallback to symboltoken - close used as currentPrice (more stable than ltp intraday)
 */
@Component
public class AngelOneHoldingMapper {
  ...
  public CanonicalHolding toCanonical(
      AngelOneHoldingRaw raw, String userId, String brokerAccountId) {
    ...
    if (isin == null || isin.isBlank()) {
      log.warn(
          "Angel One holding missing ISIN for tradingsymbol={}, using symboltoken={} as fallback",
      ...
      log.warn(
          "Angel One holding ISIN='{}' does not start with 'INE' for symbol={}",
          isin,
          raw.getTradingsymbol());
      dataConfidence = DataConfidence.LOW;
    }
    ...
    String symbol =
        SymbolNormalizer.normalize(raw.getTradingsymbol(), Broker.ANGELONE, raw.getExchange());
    ...
    BigDecimal t1Quantity =
        BigDecimal.valueOf(raw.getT1quantity() != null ? raw.getT1quantity() : 0);

    // Average buy price (Double from API)
    BigDecimal avgBuyPrice =
        PriceNormalizer.toBigDecimal(raw.getAverageprice(), "averageprice", Broker.ANGELONE);
    ...
    BigDecimal unrealizedPnL =
        PriceNormalizer.toBigDecimal(raw.getProfitandloss(), "profitandloss", Broker.ANGELONE);
    if (log.isDebugEnabled()) {
      log.debug(
          "Angel One 'profitandloss' for symbol={} is combined realized+unrealized: {}",
          symbol,
          unrealizedPnL);
    }
