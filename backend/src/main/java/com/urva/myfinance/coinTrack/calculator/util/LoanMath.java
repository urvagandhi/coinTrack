...
/** Loan and interest calculations (EMI, CI, SI). Stateless, thread-safe. */
@Component
public class LoanMath {
  ...
  /** Calculate EMI using reducing balance method. Formula: EMI = P × r × (1+r)^n / ((1+r)^n − 1) */
  ...
  /**
   * Calculate EMI using flat rate method (for comparison). Formula: EMI = (P + P × R × T) / (T ×
   * 12)
   */
  ...
  /** Calculate Compound Interest. Formula: A = P × (1 + r/n)^(n×t) */
  public BigDecimal compoundInterest(
  ...
  /** Calculate Simple Interest. Formula: SI = P × R × T / 100 */
