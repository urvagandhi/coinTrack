...
public class FdV130Migration implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            ...
            totalModified += backfill("compoundingFrequency", CompoundingFrequency.QUARTERLY.toString(), "compoundingFrequency default");
            ...
            totalModified += backfillBoolean("isPrematurelyWithdrawn", false, "isPrematurelyWithdrawn default");

            // 10. server validation fields
            totalModified += backfillNull("serverComputedMaturityAmount", "serverComputedMaturityAmount default");
            totalModified += backfillBoolean("maturityAmountOverridden", false, "maturityAmountOverridden default");
            ...
            migrationService.recordMigration(MIGRATION_ID, "1.3.0", "FD v1.3.0 industry standards backfill", totalModified + legacyStatusCount);
            
            logger.info("FD v1.3.0 migration complete. Modified {} documents (field backfills) + {} legacy status documents", 
                    totalModified, legacyStatusCount);
        } catch (Exception e) {
            logger.error("FD v1.3.0 migration failed: {}", e.getMessage());
            migrationService.recordMigrationFailure(MIGRATION_ID, "1.3.0", "FD v1.3.0 industry standards backfill", e.getMessage());
            throw e;
        }
    }
    ...
    private int migrateLegacyStatus() {
        ...
        int count = (int) mongoTemplate.updateMulti(query, update, "fixed_deposits").getModifiedCount();
        if (count > 0) logger.info("Legacy status migration: updated {} WITHDRAWN -> PREMATURELY_WITHDRAWN", count);
        return count;
    }
}