 ...
 *
 * <p>Active in ALL profiles. Without BREVO_API_KEY, isConfigured() is false and sends are skipped
 * with a warning (relevant in dev); if the key is missing in prod, startup logs ERROR and emails
 * fail-safe.
 *
 * <p>Retry: 3 attempts with exponential backoff (1s, 2s, 4s) for 5xx/network errors. Non-retryable:
 * 400 (bad payload), 401 (bad API key).
 */
@Service
public class BrevoEmailService implements EmailSender {
  ...
  /**
   * Builds the WebClient from the shared {@link WebClientConfig} builder (10 s connect timeout, 15
   * s response timeout, 2 MB in-memory codec limit) instead of a bare {@code WebClient.create()}.
   */
  ...
  /**
   * Send an email using Brevo Transactional Email API. Fail-safe: returns boolean, never throws.
   */
    ...
    Map<String, Object> body =
        Map.of(
            ...
            Map.of(
                ...
                "name", config.getSenderName()),
            "to",
            List.of(Map.of("email", to, "name", toName != null ? toName : to)),
            "subject",
            subject,
            "htmlContent",
            htmlContent);
    ...
    try {
      ...
      if (cause instanceof WebClientResponseException wcre) {
        ...
        if (status == 401) {
          ...
          log.error(
              "Failed to send email via Brevo after retries: to={}, subject={}, status={}, error={}",
              to,
              subject,
              status,
              wcre.getMessage());
        ...
        log.error(
            "Failed to send email via Brevo: to={}, subject={}, error={}",
            to,
            subject,
            cause.getMessage());
      }
  ...
  /**
   * Determine if an error is retryable. Retry: 5xx server errors, network/IO errors. Don't retry:
   * 400 (bad request), 401 (bad API key).
   */
