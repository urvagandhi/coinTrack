...
/**
 * Enriches canonical holdings with live market data → SummaryHoldingDTO. Stateless — all state
 * passed as method params.
 *
 * <p>P&L rules: - Trust broker P&L when dataConfidence == HIGH and value != null - Fall back to
 * local computation when LOW or missing - dayChange: prefer market data, fall back to canonical,
 * then null
 */
@Component
public class HoldingEnricher {

  public List<SummaryHoldingDTO> enrich(
      List<CanonicalHolding> holdings, Map<String, MarketPrice> priceMap) {
    return holdings.stream().map(h -> enrichSingle(h, priceMap.get(h.getSymbol()))).toList();
  }
  ...
  private SummaryHoldingDTO enrichSingle(CanonicalHolding h, MarketPrice marketPrice) {
    ...
    if (marketPrice != null
        && marketPrice.getCurrentPrice() != null
      ...
      previousClose =
          h.getCurrentPrice()
              .subtract(
                  h.getDayChange()
                      .divide(
                          qty.compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ONE : qty,
                          4,
    ...
    if (h.getDataConfidence() == DataConfidence.HIGH
        && h.getUnrealizedPnL() != null
    ...
    } else if (currentPrice.compareTo(BigDecimal.ZERO) > 0
    ...
    return SummaryHoldingDTO.builder()
        ...
        .raw(
            Map.of(
                ...
                "data_confidence",
                    h.getDataConfidence() != null ? h.getDataConfidence().name() : "HIGH",
