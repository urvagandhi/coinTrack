...
/**
 * Purges one-time email tokens (verification / reset / MFA-recovery links)
 * for a deleted user. The collection carries a TTL index, but immediate
 * removal closes any window where a still-valid magic link could be issued
...
public class EmailUserDataCleanupListener {
    ...
    public void onUserDeleted(UserDeletedEvent event) {
        try {
            ...
            logger.error("Cascade cleanup failed for email tokens, user {}: {}", event.userId(), e.getMessage());
        }
    }
}
