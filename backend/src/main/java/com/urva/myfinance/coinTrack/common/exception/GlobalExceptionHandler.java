 ...
 *
 * <p>Rules: - 4xx errors: logged at WARN level (message only, no stack trace) - 5xx errors: logged
 * at ERROR level (full stack trace) - Stack traces are NEVER exposed in any response - All
 * responses use {@link ApiErrorResponse} for consistency
 */
...
public class GlobalExceptionHandler {
  ...
  /**
   * Handles authentication failures (401). More specific than DomainException, so Spring resolves
   * it first.
   */
  ...
  /**
   * Bean Validation errors from {@code @Valid} on request bodies. Returns field-level error details
   * so the frontend can highlight individual fields.
   */
  ...
  /**
   * Missing-entity lookups (e.g. NoteService findById) map to 404 instead of falling through to the
   * 500 catch-all.
   */
  ...
  /**
   * No handler matched the request path. Requires {@code
  ...
  private ResponseEntity<ApiErrorResponse> buildResponse(
    ...
    ApiErrorResponse error =
        new ApiErrorResponse(status.value(), errorCode, message, request.getRequestURI());
