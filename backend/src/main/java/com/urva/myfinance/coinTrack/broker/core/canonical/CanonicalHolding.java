 ...
 *
 * <p>Compound unique index on (userId, brokerAccountId, isin) ensures one entry per stock per
 * broker account. ISIN is the primary dedup key across brokers.
 *
 * <p>Indian broker quirks handled by mappers before data reaches this model: - Zerodha:
 * average_price can be 0.0 for CDSL transfers → dataConfidence = LOW - Angel One: all numeric
 * fields arrive as Strings → parsed by PriceNormalizer - Upstox: prices are floats → converted via
 * Float.toString() to avoid precision loss
 */
@Document(collection = "canonical_holdings")
@CompoundIndexes({
  @CompoundIndex(
      name = "idx_holding_unique",
      def = "{'userId': 1, 'brokerAccountId': 1, 'isin': 1}",
      unique = true),
...
public class CanonicalHolding {
  ...
  /**
   * Primary dedup key. Starts with "INE" for equity. If null, fallback to symbol +
   * dataConfidence=LOW.
   */
  private String isin;
  ...
  @Builder.Default private BigDecimal t1Quantity = BigDecimal.ZERO;
  ...
  @Indexed private Instant lastSyncedAt;
