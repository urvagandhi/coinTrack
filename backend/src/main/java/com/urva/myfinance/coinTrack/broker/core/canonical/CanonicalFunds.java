 ...
 *
 * <p>Zerodha: equity segment → available.cash, utilised.debits. Commodity segment may be null.
 * Angel One: net (String), availablecash (String), utilisedamount (String). All strings. Upstox:
 * used_margin, available_margin inside fund.equity sub-object.
 */
@Document(collection = "canonical_funds")
@CompoundIndexes({
  @CompoundIndex(
      name = "idx_funds_unique",
      def = "{'userId': 1, 'brokerAccountId': 1}",
      unique = true)
})
@Builder
@Getter
