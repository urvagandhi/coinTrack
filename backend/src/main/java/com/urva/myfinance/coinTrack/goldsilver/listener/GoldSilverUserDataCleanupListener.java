...
/**
 * Removes gold/silver investments for a deleted user.
 * Metal rate settings live embedded in the user document; rate snapshots are
 * deployment-wide shared data — both survive the deletion.
 */
@Component
public class GoldSilverUserDataCleanupListener {

    private static final Logger logger = LoggerFactory.getLogger(GoldSilverUserDataCleanupListener.class);
    ...
    public void onUserDeleted(UserDeletedEvent event) {
        try {
            ...
            logger.error("Cascade cleanup failed for gold/silver investments, user {}: {}",
