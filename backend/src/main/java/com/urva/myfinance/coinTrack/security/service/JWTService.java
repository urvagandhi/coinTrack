 ...
 *
 * <p>Changed: - generateToken now takes User, adds userId + email claims - Added extractUserId /
 * extractEmail - Added refresh token generation + rotation - Added TokenPair record
 */
@Service
public class JWTService {
  ...
  /**
   * Generates a cryptographically secure refresh token. Only the SHA-256 hash is stored in MongoDB
   * — the plaintext is returned to the caller once and never persisted on the server.
   */
  ...
  /**
   * Validates and rotates a refresh token (one-time use). If a revoked token is presented, all
   * tokens for that user are revoked (token reuse = potential breach).
   */
  public TokenPair validateAndRotateRefreshToken(
      String rawToken, User user, String deviceInfo, String ipAddress) {
    String tokenHash = HashUtil.sha256(rawToken);
    RefreshToken stored =
    ...
    if (stored.isRevoked()) {
      logger.warn(
          "Refresh token reuse detected for userId={}. Revoking all tokens.", stored.getUserId());
  ...
  /**
   * Parse and verify a token, returning the Claims object. Useful when multiple fields need to be
   * extracted from the same token.
   */
