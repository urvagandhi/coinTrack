 ...
 *
 * <p>Token Flow: 1. createToken() - Create JWT + DB record 2. validateToken() - Verify JWT
 * signature AND check DB record 3. markUsed() - Mark token as used (single-use) 4.
 * invalidateAllForUser() - Invalidate all tokens on credential change
 *
 * <p>SECURITY NOTE: JWT validation alone is NOT sufficient. The token MUST exist in DB with
 * used=false to be considered valid. This prevents replay attacks even if an attacker has a valid
 * JWT.
 */
@Service
...
public class EmailTokenService {
   ...
   * @param purpose Token purpose (EMAIL_VERIFY, PASSWORD_RESET, EMAIL_CHANGE_VERIFY)
  ...
  public String createToken(
      User user, String purpose, String newEmail, HttpServletRequest request) {
    // Generate unique token ID
    String tokenId = UUID.randomUUID().toString();

    // Calculate expiry
    LocalDateTime expiresAt =
        LocalDateTime.now().plusMinutes(emailConfig.getMagicLinkExpiryMinutes());
    ...
    logger.info(
        "Created email token: purpose={}, userId={}, tokenId={}", purpose, user.getId(), tokenId);
   ...
   *
   * <p>IMPORTANT: This validates BOTH the JWT signature AND the DB record. Even if the JWT is
   * valid, the token is rejected if: - It doesn't exist in DB - It has been used - The purpose
   * doesn't match
   *
  ...
  public EmailToken validateToken(String token, String expectedPurpose) {
    try {
      ...
      if (!expectedPurpose.equals(dbToken.getPurpose())) {
        logger.warn(
            "DB token purpose mismatch: expected={}, actual={}",
            expectedPurpose,
            dbToken.getPurpose());
        throw new InvalidEmailTokenException("Invalid token purpose");
      }
  ...
  /**
   * Mark a token as used. Must be called after successful verification to prevent reuse.
   *
  ...
  /**
   * Invalidate all tokens for a user. Called on sensitive account changes (password, email, TOTP
   * reset, username).
   *
