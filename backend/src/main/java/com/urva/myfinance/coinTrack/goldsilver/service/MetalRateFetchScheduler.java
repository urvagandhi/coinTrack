 ...
 *
 * <p>Runs daily at 10:00 AM (configurable via {@code goldapi.cron.morning}). Before each fetch,
 * performs:
 *
 * <ol>
 *   <li><strong>Quota guard:</strong> Checks monthly API usage against the 100 req/month limit
 *   <li><strong>Health check:</strong> Verifies GoldAPI service availability via {@code
 *       /api/status}
 * </ol>
 *
...
public class MetalRateFetchScheduler {
  ...
  @Autowired
  public MetalRateFetchScheduler(
      LiveMetalRateService liveMetalRateService, GoldApiUsageService goldApiUsageService) {
  ...
  public void fetchLatestMetalRates() {
    ...
    if (!goldApiUsageService.isWithinQuota()) {
      logger.warn(
          "⚠️ Scheduled metal rate fetch SKIPPED — GoldAPI monthly quota limit reached. "
    ...
    if (!goldApiUsageService.isApiHealthy()) {
      logger.warn(
          "⚠️ Scheduled metal rate fetch SKIPPED — GoldAPI health check failed. "
    ...
    try {
      ...
      logger.info(
          "Scheduled metal rate fetch completed successfully. "
              + "Estimated remaining API requests this month: {}",
          remaining >= 0 ? remaining : "unknown");
    } catch (Exception e) {
