...
public class EpfExcelExporter {
  ...
  public static ResponseEntity<byte[]> export(
      List<EpfTransactionResponseDTO> transactions,
  ...
  public static ResponseEntity<byte[]> export(
    ...
    try (Workbook workbook = new XSSFWorkbook();
      ...
      if (!transactions.isEmpty()) {
        List<EpfTransactionResponseDTO> sortedAll =
            transactions.stream()
        ...
        if (!sortedAll.isEmpty()) {
          ...
          double firstTxnEmp =
              (firstTxn.getEmployeeContribution() != null
                      ? firstTxn.getEmployeeContribution().doubleValue()
                      : 0.0)
                  + (firstTxn.getVpfAmount() != null ? firstTxn.getVpfAmount().doubleValue() : 0.0);
          double firstTxnEmpr =
              firstTxn.getEmployerEpfContribution() != null
                  ? firstTxn.getEmployerEpfContribution().doubleValue()
                  : 0.0;
          double firstTxnWithdrawal =
              firstTxn.getWithdrawalAmount() != null
                  ? firstTxn.getWithdrawalAmount().doubleValue()
                  : 0.0;
          double initialBalance =
              (firstTxn.getEpfBalance() != null ? firstTxn.getEpfBalance().doubleValue() : 0.0)
                  - (firstTxnEmp + firstTxnEmpr - firstTxnWithdrawal);

          double currentEmpClosing = Math.round((initialBalance / 2.0) * 100.0) / 100.0;
          double currentEmprClosing =
              Math.round((initialBalance - currentEmpClosing) * 100.0) / 100.0;

          List<PrecalcMonthMap> monthsMapping =
              List.of(
          ...
          for (int y = minYear; y <= maxYear; y++) {
            ...
            List<EpfTransactionResponseDTO> yearTxns =
                sortedAll.stream()
                    .filter(
                        t ->
            ...
            for (EpfTransactionResponseDTO txn : yearTxns) {
              if (txn.getRemarks() != null
                  && txn.getRemarks().startsWith("Annual Interest Credit")) {
                continue;
              }
              ...
              group.employeeContribution +=
                  (txn.getEmployeeContribution() != null
                      ? txn.getEmployeeContribution().doubleValue()
                      : 0.0);
              group.employerEpfContribution +=
                  (txn.getEmployerEpfContribution() != null
                      ? txn.getEmployerEpfContribution().doubleValue()
                      : 0.0);
              group.vpfAmount +=
                  (txn.getVpfAmount() != null ? txn.getVpfAmount().doubleValue() : 0.0);
              group.withdrawalAmount +=
                  (txn.getWithdrawalAmount() != null
                      ? txn.getWithdrawalAmount().doubleValue()
                      : 0.0);
            }

            double yearEmpInterest =
                Math.round((currentEmpClosing * 12.0 * yearRate) / 1200.0 * 100.0) / 100.0;
            double yearEmprInterest =
                Math.round((currentEmprClosing * 12.0 * yearRate) / 1200.0 * 100.0) / 100.0;
            ...
            for (PrecalcMonthMap m : monthsMapping) {
              ...
              double empShare =
                  grp != null
                      ...
                      : 0.0;
              double emprShare =
                  grp != null ? (grp.employerEpfContribution - (withdrawalAmt / 2.0)) : 0.0;

              yearEmpContrib += empShare;
              yearEmprContrib += emprShare;

              yearEmpInterest +=
                  Math.round((empShare * m.monthsConsider * yearRate) / 1200.0 * 100.0) / 100.0;
              yearEmprInterest +=
                  Math.round((emprShare * m.monthsConsider * yearRate) / 1200.0 * 100.0) / 100.0;
            }

            currentEmpClosing =
                Math.round((currentEmpClosing + yearEmpContrib + yearEmpInterest) * 100.0) / 100.0;
            currentEmprClosing =
                Math.round((currentEmprClosing + yearEmprContrib + yearEmprInterest) * 100.0)
                    / 100.0;
          }
        }
      }
      ...
      Map<String, List<EpfTransactionResponseDTO>> fyMap =
          transactions.stream()
              .collect(Collectors.groupingBy(txn -> getFinancialYear(txn.getTransactionDate())));

      // Sort FYs in descending order
      List<String> sortedFys =
          fyMap.keySet().stream().sorted(Comparator.reverseOrder()).collect(Collectors.toList());
      ...
      httpHeaders.set(
          HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"epf_transactions.xlsx\"");
  ...
  private static int renderSummaryHeader(
      Sheet sheet, Workbook workbook, EpfSummaryDTO summary, int colCount) {
    if (summary == null) {
      return 0;
    }
    ...
    double epfBal =
        summary.getCurrentEpfBalance() != null ? summary.getCurrentEpfBalance().doubleValue() : 0.0;
    double epsBal =
        summary.getCurrentEpsBalance() != null ? summary.getCurrentEpsBalance().doubleValue() : 0.0;
    double empShare =
        (summary.getTotalEmployeeContribution() != null
                ? summary.getTotalEmployeeContribution().doubleValue()
                : 0.0)
            + (summary.getTotalVpfContributed() != null
                ? summary.getTotalVpfContributed().doubleValue()
                : 0.0);
    double emprEpf =
        summary.getTotalEmployerEpfContribution() != null
            ? summary.getTotalEmployerEpfContribution().doubleValue()
            : 0.0;
    double emprEps =
        summary.getTotalEmployerEpsContribution() != null
    ...
    double[] sumValues = {epfBal, epsBal, empShare, emprEpf, emprEps, accruedInterest};
  ...
  private static void createFySheet(
      Workbook workbook,
      String fy,
      List<EpfTransactionResponseDTO> fyList,
      List<EpfInterestRateConfig.InterestRate> rates,
      Map<String, YearBalanceSummary> yearBalances,
    ...
    for (int j = 0; j < headers.length; j++) createCell(opRow, j, "-", openingRowStyle);
    ...
    createNumericCell(
        opRow, 5, openingEmprShare > 0 ? openingEmprShare : null, openingCurrencyStyle);
    ...
    for (EpfTransactionResponseDTO txn : fyList) {
      ...
      group.employerEpfContribution +=
          (txn.getEmployerEpfContribution() != null
              ? txn.getEmployerEpfContribution().doubleValue()
              : 0.0);
      group.employerEpsContribution +=
          (txn.getEmployerEpsContribution() != null
              ? txn.getEmployerEpsContribution().doubleValue()
              : 0.0);
      group.vpfAmount += (txn.getVpfAmount() != null ? txn.getVpfAmount().doubleValue() : 0.0);
      group.withdrawalAmount +=
          (txn.getWithdrawalAmount() != null ? txn.getWithdrawalAmount().doubleValue() : 0.0);
    ...
    for (MonthDef md : monthDefs) {
      ...
      if (grouped != null) {
                ...
                .map(
                    t -> {
                      ...
                      if (isW) {
                        ...
                        if ("AUTO_SALARY".equals(modeLabel)) m = "Auto Salary";
                        else if ("MANUAL_OVERRIDE".equals(modeLabel)) m = "Manual Entry";
                        else if ("INTEREST".equals(modeLabel)) m = "Interest";
                      }

                      if (t.getTransactionDate() != null) {
                        m +=
                            " ("
                                + String.format("%02d", t.getTransactionDate().getDayOfMonth())
                                + ")";
                      }
        ...
        double empShareVal =
            grouped.employeeContribution + grouped.vpfAmount - (grouped.withdrawalAmount / 2.0);
        ...
        if (grouped.withdrawalAmount > 0.0) {
          if (!empShareStr.isEmpty()) empShareStr += "\n";
          empShareStr +=
              "-" + formatCurrency(java.math.BigDecimal.valueOf(grouped.withdrawalAmount / 2.0));
        }

        String emprEpfStr = "";
        if (grouped.employerEpfContribution > 0.0) {
          emprEpfStr +=
              formatCurrency(java.math.BigDecimal.valueOf(grouped.employerEpfContribution));
        }
        if (grouped.withdrawalAmount > 0.0) {
          if (!emprEpfStr.isEmpty()) emprEpfStr += "\n";
          emprEpfStr +=
              "-" + formatCurrency(java.math.BigDecimal.valueOf(grouped.withdrawalAmount / 2.0));
        }
        ...
        int maxLines =
            Math.max(linesDate, Math.max(linesMode, Math.max(linesEmpShare, linesEmprEpf)));
        ...
        if (grouped.withdrawalAmount > 0.0) {
          ...
          createNumericCell(
              row,
              4,
              grouped.employeeContribution > 0 ? grouped.employeeContribution : null,
              currencyStyle);
          createNumericCell(
              row,
              5,
        ...
        createNumericCell(
            row,
            6,
            grouped.employerEpsContribution > 0 ? grouped.employerEpsContribution : null,
            currencyStyle);
    ...
    for (int j = 9; j < headers.length; j++) createCell(r5, j, "", epfBalLabelStyle);
  ...
  private static void createSheet(
      Workbook workbook,
      String sheetName,
      List<EpfTransactionResponseDTO> data,
      EpfSummaryDTO summary,
      CellStyle headerStyle,
      CellStyle dataStyle,
      CellStyle rightAlignStyle,
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      createCell(
          row,
          1,
          dto.getTransactionDate() != null ? dto.getTransactionDate().toString() : "",
          dataStyle);

      String modeStr = dto.getMode() != null ? dto.getMode().name() : "";
      if ("AUTO_SALARY".equals(modeStr)) modeStr = "Auto Salary";
      else if ("MANUAL_OVERRIDE".equals(modeStr)) modeStr = "Manual Entry";
      else if ("INTEREST".equals(modeStr)) modeStr = "Interest";
      createCell(row, 2, modeStr, dataStyle);

      createNumericCell(
          row, 3, dto.getBasicDA() != null ? dto.getBasicDA().doubleValue() : null, currencyStyle);
      createNumericCell(
          row,
          4,
          dto.getEmployeeContribution() != null
              ? dto.getEmployeeContribution().doubleValue()
              : null,
          currencyStyle);
      createNumericCell(
          row,
          5,
          dto.getEmployerEpfContribution() != null
              ? dto.getEmployerEpfContribution().doubleValue()
              : null,
          currencyStyle);
      createNumericCell(
          row,
          6,
          dto.getEmployerEpsContribution() != null
              ? dto.getEmployerEpsContribution().doubleValue()
              : null,
          currencyStyle);
      createNumericCell(
          row,
          7,
          dto.getVpfAmount() != null ? dto.getVpfAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          8,
          dto.getWithdrawalAmount() != null ? dto.getWithdrawalAmount().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          9,
          dto.getEpfBalance() != null ? dto.getEpfBalance().doubleValue() : null,
          currencyStyle);
      createNumericCell(
          row,
          10,
          dto.getEpsBalance() != null ? dto.getEpsBalance().doubleValue() : null,
          currencyStyle);
    ...
    if (!"All Transactions".equals(sheetName) && !data.isEmpty()) {
      ...
      double sumEmp =
          data.stream()
              .mapToDouble(
                  d ->
                      d.getEmployeeContribution() != null
                          ? d.getEmployeeContribution().doubleValue()
                          : 0)
              .sum();
      double sumEmpr =
          data.stream()
              .mapToDouble(
                  d ->
                      d.getEmployerEpfContribution() != null
                          ? d.getEmployerEpfContribution().doubleValue()
                          : 0)
              .sum();
      double sumEps =
          data.stream()
              .mapToDouble(
                  d ->
                      d.getEmployerEpsContribution() != null
                          ? d.getEmployerEpsContribution().doubleValue()
                          : 0)
              .sum();
      double sumVpf =
          data.stream()
              .mapToDouble(d -> d.getVpfAmount() != null ? d.getVpfAmount().doubleValue() : 0)
              .sum();
      ...
      createNumericCell(
          r4,
          9,
          last.getEpfBalance() != null ? last.getEpfBalance().doubleValue() : 0,
