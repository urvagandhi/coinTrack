 ...
 *
 * <p>Auth model: direct credential login (no OAuth redirect). The backend stores the user's
 * clientCode + encrypted password + encrypted TOTP seed, generates a live OTP via {@link
 * DefaultCodeGenerator} on /connect, and exchanges them for a jwtToken.
 *
 * <p>Token lifecycle: jwtToken valid ~24 hours, with a refreshToken that allows silent renewal —
 * {@link #refreshTokenIfNeeded} runs at the top of every portfolio fetch and refreshes when the
 * remaining lifetime is under 30 minutes.
 *
 * <p>CRITICAL: Angel One returns HTTP 200 for ALL responses including errors. Must check response
 * body "errorcode" field: "0" / empty + status=true = success "AB1010" = auth expired →
 * BrokerAuthException "RR" = rate limited → BrokerRateLimitException
 *
 * <p>Profile API: rate limit 1 req/sec — kept thin; we do not poll profile.
 */
@Component
public class AngelOneBrokerAdapter implements BrokerAdapter {
  ...
  private static final String LOGIN_URL =
      SMART_API_BASE + "/rest/auth/angelbroking/user/v1/loginByPassword";
  private static final String REFRESH_URL =
      SMART_API_BASE + "/rest/auth/angelbroking/jwt/v1/generateTokens";
  private static final String LOGOUT_URL =
      SMART_API_BASE + "/rest/secure/angelbroking/user/v1/logout";

  private static final String HOLDINGS_URL =
      SMART_API_BASE + "/rest/secure/angelbroking/portfolio/v1/getHolding";
  ...
  private static final String ORDERS_URL =
      SMART_API_BASE + "/rest/secure/angelbroking/order/v1/getOrderBook";
  private static final String TRADES_URL =
      SMART_API_BASE + "/rest/secure/angelbroking/order/v1/getTradeBook";
  private static final String FUNDS_URL =
  ...
  private static final Set<BrokerCapability> CAPABILITIES =
      EnumSet.of(
  ...
  public CompletableFuture<BrokerSession> authenticate(BrokerCredentials credentials) {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          if (aCreds.apiKey() == null
              || aCreds.clientCode() == null
              || aCreds.encryptedPassword() == null
              || aCreds.totp() == null) {
          ...
          try {
            ...
            String refreshToken =
                data.has("refreshToken") ? data.get("refreshToken").asText() : null;
            ...
            return new BrokerSession(
                aCreds.clientCode(), Broker.ANGELONE, jwtToken, expiresAt, metadata);
          ...
          try {
            ...
            String responseBody =
                webClient
                    ...
                    .headers(
                        h ->
                            applySmartApiHeaders(
                                h,
                                session.metadata().getOrDefault("apiKey", ""),
                                session.accessToken()))
                    .bodyValue(body)
                    .retrieve()
                    .bodyToMono(String.class)
            ...
            String newRefresh =
                data.has("refreshToken") ? data.get("refreshToken").asText() : refreshToken;
            String newFeed =
                data.has("feedToken")
                    ? data.get("feedToken").asText()
            ...
            return new BrokerSession(
                session.accountId(), Broker.ANGELONE, newToken, expiresAt, metadata);
          } catch (BrokerAuthException | BrokerRateLimitException | BrokerApiDownException e) {
            throw e;
          } catch (Exception e) {
            throw new BrokerAuthException(
                "AngelOne refresh failed: " + e.getMessage(), Broker.ANGELONE, e);
          }
        });
  }
  ...
  /**
   * Generates a live 6-digit TOTP code from a Base32-encoded TOTP seed using the same library the
   * app uses for its own 2FA.
   */
  ...
  public CompletableFuture<List<CanonicalHolding>> fetchHoldings(BrokerSession session) {
    return CompletableFuture.supplyAsync(
        () -> {
          List<AngelOneHoldingRaw> rawList =
              fetchListFromAngelOne(session, HOLDINGS_URL, "holdings", AngelOneHoldingRaw.class);
  ...
  public CompletableFuture<List<CanonicalPosition>> fetchPositions(BrokerSession session) {
    return CompletableFuture.supplyAsync(
        () -> {
          List<AngelOnePositionRaw> rawList =
              fetchListFromAngelOne(session, POSITIONS_URL, "positions", AngelOnePositionRaw.class);
          return rawList.stream()
  ...
  public CompletableFuture<CanonicalFunds> fetchFunds(BrokerSession session) {
    return CompletableFuture.supplyAsync(
        () -> {
          AngelOneFundsRaw raw =
              fetchObjectFromAngelOne(session, FUNDS_URL, "funds", AngelOneFundsRaw.class);
  ...
  public List<OrderDTO> fetchOrders(BrokerAccount account) {
    ...
    List<AngelOneOrderRaw> rawList =
        fetchListFromAngelOne(session, ORDERS_URL, "orders", AngelOneOrderRaw.class);
  ...
  public List<TradeDTO> fetchTrades(BrokerAccount account) {
    BrokerAccount fresh = refreshTokenIfNeeded(account);
    BrokerSession session = toSession(fresh);
    List<AngelOneTradeRaw> rawList =
        fetchListFromAngelOne(session, TRADES_URL, "trades", AngelOneTradeRaw.class);
  ...
  /**
   * Calls SmartAPI /user/v1/logout and clears tokens locally. Safe to call when no token is present
   * — only the local clear runs.
   */
  ...
  /**
   * Ensures the BrokerAccount has a live JWT before a portfolio fetch. Refresh path: when remaining
   * lifetime is under {@link #REFRESH_AHEAD}, call /generateTokens and persist the new tokens. If
   * refresh fails, throw the canonical session-expired BrokerException so callers map to "Please
   * reconnect."
   */
  public BrokerAccount refreshTokenIfNeeded(BrokerAccount account) {
    ...
    if (now.isAfter(expiresAt.minusMinutes(REFRESH_AHEAD.toMinutes()))) {
      ...
      try {
        ...
        account.setAngelOneTokenExpiresAt(
            LocalDateTime.ofInstant(refreshed.expiresAt(), ZoneId.of("Asia/Kolkata")));
  ...
  /**
   * Builds an in-memory BrokerSession from the persisted account fields. Decrypts the stored JWT —
   * if decryption fails, treats it as session-expired.
   */
  private BrokerSession toSession(BrokerAccount account) {
    try {
      ...
      Map<String, String> metadata = new HashMap<>();
      metadata.put(
          "apiKey", account.getAngelOneApiKey() != null ? account.getAngelOneApiKey() : "");
      if (account.getAngelOneRefreshToken() != null) {
  ...
  private <T> List<T> fetchListFromAngelOne(
      BrokerSession session, String url, String logTag, Class<T> itemType) {
    try {
      String responseBody =
          webClient
              .get()
              ...
              .headers(
                  h ->
                      applySmartApiHeaders(
                          h, session.metadata().getOrDefault("apiKey", ""), session.accessToken()))
              .retrieve()
              .bodyToMono(String.class)
              .block();
  ...
  private <T> T fetchObjectFromAngelOne(
      BrokerSession session, String url, String logTag, Class<T> responseType) {
    try {
      String responseBody =
          webClient
              .get()
  ...
  /**
   * Angel One CRITICAL validation: HTTP 200 for ALL responses. "0" / empty errorcode + status=true
   * = success "AB1010" = auth expired "RR" = rate limited
   */
  private void validateAngelOneResponse(JsonNode root, String context) {
    if (root == null) {
      throw new BrokerApiDownException(
          "Null response from AngelOne for " + context, Broker.ANGELONE);
    }
    ...
    String message =
        root.has("message") ? root.get("message").asText("Unknown error") : "Unknown error";

    if ("AB1010".equals(errorCode)) {
      throw new BrokerAuthException(
          "AngelOne session expired: " + message, Broker.ANGELONE, errorCode);
    }
    ...
    throw new BrokerApiDownException(
        "AngelOne error [" + errorCode + "]: " + message + " (context: " + context + ")",
        Broker.ANGELONE);
  }
}
