...
public class EpfContributionCalculationService {
  ...
  public CalculationResult calculate(
    ...
    if (useActualSalaryForEps) {
      epsContribution =
          basicDA.multiply(new BigDecimal("0.0833")).setScale(2, RoundingMode.HALF_UP);
    } else {
    ...
    BigDecimal totalEmployerContribution =
        basicDA.multiply(new BigDecimal("0.12")).setScale(2, RoundingMode.HALF_UP);
    ...
    BigDecimal employeeContributionRate =
        employeeContributionRatePercent.divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
    BigDecimal employeeContribution =
        basicDA.multiply(employeeContributionRate).setScale(2, RoundingMode.HALF_UP);

    return new CalculationResult(
        employeeContribution, employerEpfContribution, epsContribution, vpf);
  }
}
