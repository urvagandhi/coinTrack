...
public class EpfTransactionServiceImpl implements EpfTransactionService {
  ...
  public EpfSettingsResponseDTO updateSettings(EpfSettingsRequestDTO requestDTO, String userId) {
    ...
    EpfSettingsEmbed embed =
        EpfSettingsEmbed.builder()
  ...
  @Override
  @Transactional
  public EpfTransactionResponseDTO createTransaction(
      EpfTransactionRequestDTO requestDTO, String userId) {
    ...
    if (requestDTO.getMode() == ContributionMode.AUTO_SALARY) {
      ...
      if (empContr == null
          && emprEpfContr == null
          && emprEpsContr == null
          && requestDTO.getWithdrawalAmount() == null) {
        throw new ValidationException(
            "mode", "Contribution or withdrawal fields must be provided in MANUAL_OVERRIDE mode");
      }
    }
  ...
  public Page<EpfTransactionResponseDTO> getTransactions(
    ...
    List<EpfTransactionResponseDTO> dtos = transactions.stream().map(this::toResponseDTO).toList();
  ...
  @Override
  @Transactional
  public EpfTransactionResponseDTO updateTransaction(
      String id, EpfTransactionRequestDTO requestDTO, String userId) {
  ...
  public EpfSummaryDTO getSummary(String userId) {
    List<EpfTransaction> list =
        epfTransactionRepository.findByUserId(
            userId,
    ...
    for (EpfTransaction txn : list) {
      if (txn.getRemarks() != null && txn.getRemarks().startsWith("Annual Interest Credit")) {
        if (txn.getEmployeeContribution() != null) {
          interestCreditedLifetimeEpf =
              interestCreditedLifetimeEpf.add(txn.getEmployeeContribution());
        }
        if (txn.getEmployerEpfContribution() != null) {
          interestCreditedLifetimeEpf =
              interestCreditedLifetimeEpf.add(txn.getEmployerEpfContribution());
        }
        if (txn.getEmployerEpsContribution() != null) {
          interestCreditedLifetimeEps =
              interestCreditedLifetimeEps.add(txn.getEmployerEpsContribution());
        }
      } else {
        if (txn.getEmployeeContribution() != null) {
        ...
        if (txn.getEmployerEpfContribution() != null) {
          totalEmployerEpfContribution =
              totalEmployerEpfContribution.add(txn.getEmployerEpfContribution());
        }
        if (txn.getEmployerEpsContribution() != null) {
          totalEmployerEpsContribution =
              totalEmployerEpsContribution.add(txn.getEmployerEpsContribution());
        }
    ...
    BigDecimal currentEpfBalance =
        lastTxn.getEpfBalance() != null ? lastTxn.getEpfBalance() : BigDecimal.ZERO;
    BigDecimal currentEpsBalance =
        lastTxn.getEpsBalance() != null ? lastTxn.getEpsBalance() : BigDecimal.ZERO;
    ...
    try {
      EpfInterestAccrualService.EpfInterestAccrualResult accrualResult =
          interestAccrualService.calculateAccruedInterest(userId, currentFy);
      accruedEpf = accrualResult.getAccruedEpfInterest();
      accruedEps = accrualResult.getAccruedEpsInterest();
    } catch (Exception ex) {
      logger.warn(
          "Could not calculate live interest accrual for FY {}: {}", currentFy, ex.getMessage());
    }
    ...
    for (EpfTransaction txn : list) {
      if (!txn.getTransactionDate().isBefore(currentFyDates[0])
          && !txn.getTransactionDate().isAfter(currentFyDates[1])) {
        if (txn.getRemarks() == null || !txn.getRemarks().startsWith("Annual Interest Credit")) {
          if (txn.getEmployeeContribution() != null) {
  ...
  public List<EpfTransactionResponseDTO> getAllForExport(
    ...
    List<EpfTransaction> transactions = mongoTemplate.find(query, EpfTransaction.class);
    return transactions.stream().map(this::toResponseDTO).toList();
  }

  // ── Helper methods ──────────────────────────────────────────────────

  private EpfTransaction findAndVerifyOwnership(String id, String userId) {
    return epfTransactionRepository
        .findByIdAndUserId(id, userId)
        .orElseThrow(
            () ->
                new DomainException(
                    "EPF transaction not found or access denied", "NOT_FOUND", 404));
  }
  ...
  private Query buildDynamicQuery(
      String userId, String dateFrom, String dateTo, String financialYear, ContributionMode mode) {
    ...
    if (financialYear != null && !financialYear.trim().isEmpty()) {
      ...
      if (dateFrom != null && dateTo != null) {
        criteria
            .and("transactionDate")
            .gte(LocalDate.parse(dateFrom, DateTimeFormatter.ISO_DATE))
