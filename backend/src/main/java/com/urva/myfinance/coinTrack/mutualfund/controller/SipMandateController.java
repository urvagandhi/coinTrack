...
public class SipMandateController {
  ...
  @Autowired private SipMandateService service;
  ...
  @GetMapping("/{id}")
  public ResponseEntity<ApiResponse<SipMandate>> getMandate(
  ...
  public ResponseEntity<ApiResponse<List<SipMandate>>> getMandatesByStatus(
      @AuthenticationPrincipal UserPrincipal userDetails, @PathVariable String status) {
    List<SipMandate> mandates = service.getMandatesByStatus(userDetails.getUserId(), status);
    return ResponseEntity.ok(
        ApiResponse.success(mandates, "Fetched mandates by status successfully"));
  }

  @PatchMapping("/{id}/stop")
  public ResponseEntity<ApiResponse<SipMandate>> stopMandate(
      @AuthenticationPrincipal UserPrincipal userDetails,
  ...
  public ResponseEntity<ApiResponse<SipMandate>> restartMandate(
      @AuthenticationPrincipal UserPrincipal userDetails,
      @PathVariable String id,
      @RequestBody java.util.Map<String, String> payload) {
    String date = payload.get("date");
  ...
  public ResponseEntity<ApiResponse<SipMandate>> createMandate(
      @AuthenticationPrincipal UserPrincipal userDetails, @RequestBody SipMandate mandate) {
  ...
  public ResponseEntity<ApiResponse<Integer>> backfillAllMandates(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(
            backfilledCount,
            "Successfully backfilled " + backfilledCount + " missing SIP contributions."));
  }
}
