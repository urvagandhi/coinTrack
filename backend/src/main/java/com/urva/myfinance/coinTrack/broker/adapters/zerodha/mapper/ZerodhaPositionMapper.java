 ...
 *
 * <p>Zerodha quirks: - instrument_type: null/empty/"EQ" for equity, "FUT" for futures, "CE"/"PE"
 * for options - quantity can be negative (short positions) - realizedPnL is not directly available
 * from positions API — set to ZERO - lot size not included in position response — multiplier
 * defaults to 1
 */
@Component
public class ZerodhaPositionMapper {
  ...
  public CanonicalPosition toCanonical(
      ZerodhaPositionRaw raw, String userId, String brokerAccountId) {
    // Symbol normalization
    String symbol =
        SymbolNormalizer.normalize(raw.getTradingsymbol(), Broker.ZERODHA, raw.getExchange());
    ...
    PositionType positionType =
        quantity.compareTo(BigDecimal.ZERO) < 0 ? PositionType.SHORT : PositionType.LONG;

    // Overnight quantity (int from API)
    BigDecimal overnightQty =
        BigDecimal.valueOf(raw.getOvernight_quantity() != null ? raw.getOvernight_quantity() : 0);
    ...
    if (raw.getAverage_price() != null && raw.getAverage_price() != 0.0) {
      avgBuyPrice =
          PriceNormalizer.toBigDecimal(raw.getAverage_price(), "average_price", Broker.ZERODHA);
    } else {
    ...
    BigDecimal avgSellPrice =
        raw.getSell_price() != null
            ? PriceNormalizer.toBigDecimal(raw.getSell_price(), "sell_price", Broker.ZERODHA)
            : null;

    // Last price
    BigDecimal lastPrice =
        PriceNormalizer.toBigDecimal(raw.getLast_price(), "last_price", Broker.ZERODHA);

    // Realized P&L from realised field (Zerodha positions API provides this)
    BigDecimal realizedPnL =
        PriceNormalizer.toBigDecimal(raw.getRealised(), "realised", Broker.ZERODHA);

    // Unrealized P&L from unrealised field
    BigDecimal unrealizedPnL =
        PriceNormalizer.toBigDecimal(raw.getUnrealised(), "unrealised", Broker.ZERODHA);
    ...
    return CanonicalPosition.builder()
        ...
        .totalPnL(totalPnL)
        .multiplier(
            raw.getMultiplier() != null && raw.getMultiplier() > 0 ? raw.getMultiplier() : 1)
