...
public class ValuationSnapshotController {
  ...
  @Autowired private ValuationSnapshotService service;
  ...
  public ResponseEntity<ApiResponse<List<ValuationSnapshot>>> getSnapshots(
    ...
    List<ValuationSnapshot> snapshots =
        service.getSnapshots(userDetails.getUserId(), holderName, platform);
    return ResponseEntity.ok(
        ApiResponse.success(snapshots, "Fetched valuation snapshots successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<ValuationSnapshot>> getSnapshot(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(snapshot, "Fetched valuation snapshot successfully"));
  }

  @PostMapping
  public ResponseEntity<ApiResponse<ValuationSnapshot>> createSnapshot(
      @AuthenticationPrincipal UserPrincipal userDetails, @RequestBody ValuationSnapshot snapshot) {
    ValuationSnapshot created = service.createSnapshot(userDetails.getUserId(), snapshot);
    return ResponseEntity.ok(
        ApiResponse.success(created, "Valuation snapshot created successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<ValuationSnapshot>> updateSnapshot(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(updated, "Valuation snapshot updated successfully"));
  }
