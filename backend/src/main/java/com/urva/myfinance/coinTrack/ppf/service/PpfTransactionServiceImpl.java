...
public class PpfTransactionServiceImpl implements PpfTransactionService {
  ...
  @Override
  @Transactional
  public PpfTransactionResponseDTO createTransaction(
      PpfTransactionRequestDTO requestDTO, String userId) {
    ...
    PpfTransaction transaction =
        PpfTransaction.builder()
  ...
  public Page<PpfTransactionResponseDTO> getTransactions(
    ...
    List<PpfTransactionResponseDTO> dtos = transactions.stream().map(this::toResponseDTO).toList();
  ...
  @Override
  @Transactional
  public PpfTransactionResponseDTO updateTransaction(
      String id, PpfTransactionRequestDTO requestDTO, String userId) {
  ...
  public List<String> getFinancialYears(String userId) {
    ...
    PpfTransaction earliest =
        mongoTemplate.findOne(
            base.with(Sort.by(Sort.Direction.ASC, "transactionDate")).limit(1),
            PpfTransaction.class);
  ...
  public List<PpfTransactionResponseDTO> getAllForExport(
      String userId, String dateFrom, String dateTo, String financialYear, String particulars) {
    Query query = buildDynamicQuery(userId, dateFrom, dateTo, financialYear, particulars);
  ...
  private void validateRequestDTO(PpfTransactionRequestDTO requestDTO) {
    ...
    boolean hasCredit =
        requestDTO.getCreditAmount() != null
            && requestDTO.getCreditAmount().compareTo(BigDecimal.ZERO) > 0;
    boolean hasDebit =
        requestDTO.getDebitAmount() != null
            && requestDTO.getDebitAmount().compareTo(BigDecimal.ZERO) > 0;

    if (hasCredit && hasDebit) {
      throw new ValidationException(
          "amount", "Exactly one of debitAmount or creditAmount must be present, not both");
    }
    if (!hasCredit && !hasDebit) {
      throw new ValidationException(
          "amount", "Exactly one of debitAmount or creditAmount must be greater than 0");
    }

    if (requestDTO.getCreditAmount() != null
        && requestDTO.getCreditAmount().compareTo(BigDecimal.ZERO) <= 0) {
      throw new ValidationException("creditAmount", "Amount must be greater than 0");
    }
    if (requestDTO.getDebitAmount() != null
        && requestDTO.getDebitAmount().compareTo(BigDecimal.ZERO) <= 0) {
  ...
  private PpfTransaction findAndVerifyOwnership(String id, String userId) {
    return ppfTransactionRepository
        .findByIdAndUserId(id, userId)
        .orElseThrow(
            () ->
                new DomainException(
                    "PPF transaction not found or access denied", "NOT_FOUND", 404));
  }
  ...
  public PpfSettingsResponseDTO updateSettings(PpfSettingsRequestDTO requestDTO, String userId) {
    ...
    PpfSettingsEmbed embed =
        (user.getPpfSettings() != null)
            ? user.getPpfSettings()
  ...
  private PpfSettingsResponseDTO toSettingsDTO(PpfSettingsEmbed embed, String userId) {
    if (embed == null) {
      return PpfSettingsResponseDTO.builder().userId(userId).configured(false).build();
    }
