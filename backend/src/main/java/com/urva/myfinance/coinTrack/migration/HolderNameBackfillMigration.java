...
public class HolderNameBackfillMigration implements ApplicationRunner {
    ...
    @Autowired
    public HolderNameBackfillMigration(MongoTemplate mongoTemplate, MigrationService migrationService) {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            ...
            migrationService.recordMigration(MIGRATION_ID, "1.0.0", "Canonical holderName backfill (FD + MF)",
                    totalModified);
            logger.info("Holder-name backfill migration complete. Normalized {} documents across {} collections.",
                    totalModified, TARGET_COLLECTIONS.size());
        } catch (Exception e) {
            logger.error("Holder-name backfill migration failed: {}", e.getMessage());
            migrationService.recordMigrationFailure(MIGRATION_ID, "1.0.0",
                    "Canonical holderName backfill (FD + MF)", e.getMessage());
            throw e;
        }
    }
    ...
    private int backfillCollection(String collection) {
        List<String> idsToFix = mongoTemplate.find(
                        new Query(Criteria.where("holderName").exists(true)),
