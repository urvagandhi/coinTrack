...
public class MfSchemeController {
  ...
  @Autowired private MfSchemeService service;

  @Autowired private MfSchemeAggregationService aggregationService;
    ...
    if (!includeRedeemed) {
      schemes =
          schemes.stream()
              .filter(
                  s -> {
                    java.util.Set<FundStatus> derivedStatuses =
                        aggregationService.calculateSummary(userId, s.getId()).getStatuses();
                    return derivedStatuses == null
                        || !derivedStatuses.contains(FundStatus.FULLY_REDEEMED);
                  })
  ...
  public ResponseEntity<ApiResponse<List<MfScheme>>> searchSchemes(
      @AuthenticationPrincipal UserPrincipal userDetails, @RequestParam String query) {
  ...
  public ResponseEntity<ApiResponse<List<MfScheme>>> getSchemesByCategory(
      @AuthenticationPrincipal UserPrincipal userDetails, @PathVariable String category) {
    List<MfScheme> schemes = service.getSchemesByCategory(userDetails.getUserId(), category);
    return ResponseEntity.ok(
        ApiResponse.success(schemes, "Schemes by category fetched successfully"));
  }

  @GetMapping("/platform/{platform}")
  public ResponseEntity<ApiResponse<List<MfScheme>>> getSchemesByPlatform(
      @AuthenticationPrincipal UserPrincipal userDetails, @PathVariable String platform) {
    List<MfScheme> schemes = service.getSchemesByPlatform(userDetails.getUserId(), platform);
    return ResponseEntity.ok(
        ApiResponse.success(schemes, "Schemes by platform fetched successfully"));
  }

  @GetMapping("/bank/{bank}")
  public ResponseEntity<ApiResponse<List<MfScheme>>> getSchemesByBank(
      @AuthenticationPrincipal UserPrincipal userDetails, @PathVariable String bank) {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<ApiResponse<MfScheme>> getScheme(
      @AuthenticationPrincipal UserPrincipal userDetails, @PathVariable String id) {
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getNavForDate(
    ...
    if (scheme.getAmfiCode() == null || scheme.getAmfiCode().trim().isEmpty()) {
      ...
      response.put(
          "applicableDate", settlementDateCalculator.calculateApplicableDate(date, isAfterCutoff));
    ...
    LocalDate applicableDate =
        settlementDateCalculator.calculateApplicableDate(date, isAfterCutoff);
  ...
  @PostMapping
  public ResponseEntity<ApiResponse<MfScheme>> createScheme(
      @AuthenticationPrincipal UserPrincipal userDetails, @RequestBody MfScheme scheme) {
  ...
  @PutMapping("/{id}")
  public ResponseEntity<ApiResponse<MfScheme>> updateScheme(
      @AuthenticationPrincipal UserPrincipal userDetails,
      @PathVariable String id,
