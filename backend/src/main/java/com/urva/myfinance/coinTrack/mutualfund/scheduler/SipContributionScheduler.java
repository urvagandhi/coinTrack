...
public class SipContributionScheduler {
  ...
  @Autowired
  public SipContributionScheduler(
  ...
  /**
   * Runs every day at 1:00 AM. Checks all active SIP mandates and creates a new SipContribution if
   * today is the scheduled deduction day.
   */
  ...
  public void generateMonthlySipContributions() {
    ...
    List<SipMandate> activeMandates =
        sipMandateRepository.findAll().stream()
    ...
    for (SipMandate mandate : activeMandates) {
      ...
      if (today.getDayOfMonth() == executionDay) {
        ...
        boolean exists =
            sipContributionRepository.existsBySipMandateIdAndContributionDateBetween(
    ...
    logger.info(
        "Completed scheduled task: generateMonthlySipContributions. Created {} new contributions.",
        count);
  }
}
