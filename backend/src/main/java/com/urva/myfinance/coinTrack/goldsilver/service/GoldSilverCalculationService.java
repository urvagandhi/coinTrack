...
public class GoldSilverCalculationService {
  ...
  public void calculateFields(GoldSilverInvestment investment) {
    BigDecimal ratePerGram =
        investment.getRatePerGram() != null ? investment.getRatePerGram() : BigDecimal.ZERO;
    BigDecimal netWeight =
        investment.getNetWeight() != null ? investment.getNetWeight() : BigDecimal.ZERO;
    ...
    BigDecimal makingChargePercent =
        investment.getMakingChargePercent() != null
            ? investment.getMakingChargePercent()
            : BigDecimal.ZERO;
    ...
    BigDecimal stoneOtherCharges =
        investment.getStoneOtherCharges() != null
            ? investment.getStoneOtherCharges()
            : BigDecimal.ZERO;
    BigDecimal totalAmount =
        metalAmount
            .add(makingChargeAmount)
            .add(stoneOtherCharges)
            .setScale(2, RoundingMode.HALF_UP);
    investment.setTotalAmount(totalAmount);

    // gstAmount = totalAmount * (gstPercent / 100)
    BigDecimal gstPercent =
        investment.getGstPercent() != null ? investment.getGstPercent() : new BigDecimal("3.00");
    investment.setGstPercent(gstPercent);
    BigDecimal gstAmount =
        totalAmount.multiply(gstPercent).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
  ...
  public void recalculateMarketValue(GoldSilverInvestment investment) {
    BigDecimal netAmount =
        investment.getNetAmount() != null ? investment.getNetAmount() : BigDecimal.ZERO;
    BigDecimal netWeight =
        investment.getNetWeight() != null ? investment.getNetWeight() : BigDecimal.ZERO;
    BigDecimal currentMarketRate =
        investment.getCurrentMarketRate() != null
            ? investment.getCurrentMarketRate()
            : BigDecimal.ZERO;

    // currentValue = currentMarketRate * netWeight
    BigDecimal currentValue =
        currentMarketRate.multiply(netWeight).setScale(2, RoundingMode.HALF_UP);
    ...
    if (netAmount.compareTo(BigDecimal.ZERO) > 0) {
      returnPercent =
          profitLoss
              .divide(netAmount, 4, RoundingMode.HALF_UP)
