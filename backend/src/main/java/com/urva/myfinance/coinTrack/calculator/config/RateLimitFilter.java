...
/**
 * Rate limiting filter for calculator endpoints. Limits: 60 requests per minute per IP address.
 *
 * <p>Note: In-memory buckets — single-instance only (Render free tier). For multi-instance: migrate
...
public class RateLimitFilter extends OncePerRequestFilter {
  ...
  /**
   * Extract real client IP, handling reverse proxies (Render LB). X-Forwarded-For: client, proxy1,
   * proxy2 — take first (client).
   */
