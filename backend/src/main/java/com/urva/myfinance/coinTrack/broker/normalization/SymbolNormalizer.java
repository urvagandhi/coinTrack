 ...
 *
 * <p>Input formats by broker: Zerodha NSE equity: "RELIANCE-EQ" + "NSE" → "NSE:RELIANCE" Zerodha
 * NFO: "NIFTY24DECFUT" + "NFO" → "NFO:NIFTY24DECFUT" Zerodha BSE: "500325" + "BSE" → "BSE:500325"
 * Angel One: "RELIANCE" + "NSE" → "NSE:RELIANCE" Upstox: "NSE_EQ|INE002A01018" → passed through as
 * "NSE:RELIANCE" (requires external ISIN→symbol mapping for full resolution)
 *
 * <p>Output: Always "EXCHANGE:BASESYMBOL" with uppercase exchange and symbol.
 *
...
public final class SymbolNormalizer {
  ...
  /**
   * Zerodha: strip "-EQ" suffix for NSE equity, keep NFO symbols as-is. BSE uses script codes
   * (numeric) which pass through unchanged.
   */
  ...
  /** Angel One: symbols are already clean (e.g. "RELIANCE"). Just uppercase and trim. */
  private static String normalizeAngelOne(String rawSymbol) {
    return rawSymbol.trim().toUpperCase();
  }

  /**
   * Upstox: instrument_key format is "NSE_EQ|INE002A01018". If the raw symbol contains "|", extract
   * the part before it as the symbol. The ISIN (after "|") is extracted separately by the mapper.
