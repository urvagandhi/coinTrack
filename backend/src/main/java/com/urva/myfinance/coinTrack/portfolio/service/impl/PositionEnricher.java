...
/**
 * Enriches canonical positions → SummaryPositionDTO. Uses stored P&L fields (trust broker) — does
 * NOT recompute from market data.
 */
@Component
public class PositionEnricher {

  public List<SummaryPositionDTO> enrich(List<CanonicalPosition> positions) {
    return positions.stream().map(this::enrichSingle).toList();
  }
  ...
  private SummaryPositionDTO enrichSingle(CanonicalPosition p) {
    ...
    BigDecimal totalPnL =
        p.getTotalPnL() != null ? p.getTotalPnL() : unrealizedPnL.add(realizedPnL);
    ...
    rawMap.put(
        "instrument_type", p.getInstrumentType() != null ? p.getInstrumentType().name() : "");
