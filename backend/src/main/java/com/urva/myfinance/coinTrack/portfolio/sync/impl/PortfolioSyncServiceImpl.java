...
public class PortfolioSyncServiceImpl implements PortfolioSyncService {
  ...
  @Autowired
  public PortfolioSyncServiceImpl(
  ...
  public void syncUser(String userId) {
    ...
    for (BrokerAccount account : accounts) {
      if (Boolean.TRUE.equals(account.getIsActive())
          && !result.staleBrokers().contains(account.getBroker())) {
    ...
    // Create sync log
    SyncStatus status =
        result.syncErrors().isEmpty() ? SyncStatus.SUCCESS : SyncStatus.PARTIAL_FAILURE;
    String message =
        result.syncErrors().isEmpty()
            ? "Sync complete"
            : "Sync completed with errors: "
                + result.syncErrors().stream()
  ...
  public SyncLog runFullSyncForAccount(BrokerAccount account) {
    if (!syncSafetyService.tryAccountLock(account.getId())) {
      ...
      return createLog(
          ...
          account.getBroker(),
          SyncStatus.FAILURE,
          "Sync locked: Already in progress",
          0L);
    }
  ...
  private SyncLog executeSyncLogic(BrokerAccount account) {
    ...
    try {
      ...
      long duration = Duration.between(startTime, LocalDateTime.now(INDIA_ZONE)).toMillis();
      return createLog(
          userId, broker, SyncStatus.FAILURE, "Sync failed: " + e.getMessage(), duration);
    }
  }

  /**
   * Persists aggregated canonical data to MongoDB. Upserts by compound unique keys as defined in
   * canonical model indexes.
   */
  private void persistAggregatedData(String userId, AggregatedPortfolio result) {
    ...
    for (CanonicalHolding h : result.holdings()) {
      h.setUserId(userId);
      holdingRepository
          .findByUserIdAndBrokerAccountIdAndIsin(userId, h.getBrokerAccountId(), h.getIsin())
    ...
    for (Map.Entry<Broker, CanonicalFunds> entry : result.funds().entrySet()) {
      ...
      f.setUserId(userId);
      fundsRepository
          .findByUserIdAndBrokerAccountId(userId, f.getBrokerAccountId())
    ...
    for (CanonicalMfHolding mf : result.mfHoldings()) {
      mf.setUserId(userId);
      mfHoldingRepository
          .findByUserIdAndBrokerAccountIdAndIsin(userId, mf.getBrokerAccountId(), mf.getIsin())
  ...
  private SyncLog createLog(
      String userId, Broker broker, SyncStatus status, String message, Long duration) {
    SyncLog log =
        SyncLog.builder()
            .userId(userId)
            .broker(broker)
