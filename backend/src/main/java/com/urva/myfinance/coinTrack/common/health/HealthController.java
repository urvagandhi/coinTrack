...
public class HealthController {
    ...
    public ResponseEntity<?> health(
            @org.springframework.web.bind.annotation.RequestHeader(value = "Accept", defaultValue = "application/json") String acceptHeader) {
        ...
        try {
            ...
            if (acceptHeader != null && acceptHeader.contains("text/html")) {
                return ResponseEntity.status(status).contentType(org.springframework.http.MediaType.TEXT_HTML)
    ...
    private String generateHtml(Map<String, Object> response) {
        ...
        sb.append("<tr><th>Overall Status</th><td><span class='").append(statusClass).append("'>").append(mainStatus)
                .append("</span></td></tr>");
        ...
        if (response.containsKey("checks")) {
            ...
            for (Map.Entry<String, Object> entry : checks.entrySet()) {
                String sectionTitle = entry.getKey().substring(0, 1).toUpperCase() + entry.getKey().substring(1)
                        + " Metrics";
                    ...
                    for (Map.Entry<String, Object> detail : details.entrySet()) {
                        ...
                        if (val instanceof Map) {
                            ...
                            ((Map<String, Object>) val).forEach((k, v) -> {
                                nested.append("<li><strong style='color:#7f8c8d;'>").append(k).append(":</strong> ")
                                        .append(v).append("</li>");
                            });
                            nested.append("</ul>");
                            ...
                            if ("status".equalsIgnoreCase(key) || "status".equalsIgnoreCase(detail.getKey())) {
                                String sClass = "UP".equals(val) ? "status-up"
                                        : ("DOWN".equals(val) ? "status-down" : "");
                                sb.append("<tr><th>").append(key).append("</th><td><span class='").append(sClass)
                                        .append("'>").append(val).append("</span></td></tr>");
                            } else {
        ...
        if (response.containsKey("error")) {
            sb.append(
                    ...
                    .append(response.get("error")).append("</p></div>");
        }
