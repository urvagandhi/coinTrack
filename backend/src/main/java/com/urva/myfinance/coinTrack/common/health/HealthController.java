...
/**
 * Enhanced health check endpoint for monitoring services like Render. Provides comprehensive status
...
public class HealthController {
  ...
  /**
   * Comprehensive health check endpoint. Returns 200 OK if all systems are healthy, 503 if any
  ...
  private String generateHtml(Map<String, Object> response) {
    ...
    sb.append(
        "body{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background-color:#f4f7f6;color:#333;margin:0;padding:0;display:flex;height:100vh;} ");
    sb.append(
        ".sidebar{width:50%;background-color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:50px;position:fixed;height:100%;box-sizing:border-box;border-right:1px solid #eaeaea;} ");
    ...
    sb.append(
        ".back-btn{padding:12px 30px;background-color:#007bff;color:#fff;text-decoration:none;border-radius:30px;font-size:1.1rem;font-weight:600;transition:all 0.2s;} ");
    sb.append(
        ".back-btn:hover{background-color:#0056b3;transform:translateY(-2px);box-shadow:0 4px 10px rgba(0,123,255,0.3);} ");
    sb.append(
        ".main-content{margin-left:50%;width:50%;padding:60px;box-sizing:border-box;overflow-y:auto;background-color:#f4f7f6;} ");
    sb.append(".dashboard-header{margin-bottom:40px;color:#2c3e50;font-size:2rem;} ");
    sb.append(
        ".card{background:#fff;border-radius:12px;padding:30px;box-shadow:0 4px 15px rgba(0,0,0,0.03);margin-bottom:30px;transition:transform 0.2s;} ");
    sb.append(".card:hover{transform:translateY(-2px);} ");
    sb.append(
        "h2{color:#34495e;border-bottom:2px solid #f1f4f6;padding-bottom:12px;margin-top:0;font-size:1.4rem;} ");
    sb.append(
        "table{width:100%;border-collapse:collapse;} th,td{padding:12px 15px;text-align:left;border-bottom:1px solid #f8f9fa;font-size:1.05rem;} ");
    sb.append("th{color:#7f8c8d;font-weight:600;width:35%;} td{color:#2c3e50;} ");
    sb.append(
        ".status-up{color:#27ae60;background:#eafaf1;padding:5px 12px;border-radius:20px;font-weight:bold;font-size:0.9rem;display:inline-block;} ");
    sb.append(
        ".status-down{color:#e74c3c;background:#fdedec;padding:5px 12px;border-radius:20px;font-weight:bold;font-size:0.9rem;display:inline-block;} ");
    ...
    sb.append("<p>Service Health Dashboard<br/>Version ")
        .append(response.get("version"))
        .append("</p>");
    ...
    sb.append("<table><tr><th>Service</th><td>")
        .append(response.get("service"))
        .append("</td></tr>");
    sb.append("<tr><th>Overall Status</th><td><span class='")
        .append(statusClass)
        .append("'>")
        .append(mainStatus)
        .append("</span></td></tr>");
    sb.append("<tr><th>Uptime (ms)</th><td>").append(response.get("uptime")).append("</td></tr>");
    sb.append("<tr><th>Timestamp</th><td>")
        .append(response.get("timestamp"))
    ...
    if (response.containsKey("checks")) {
      ...
      for (Map.Entry<String, Object> entry : checks.entrySet()) {
        String sectionTitle =
            entry.getKey().substring(0, 1).toUpperCase() + entry.getKey().substring(1) + " Metrics";
        ...
        if (entry.getValue() instanceof Map) {
          ...
          for (Map.Entry<String, Object> detail : details.entrySet()) {
            String key =
                detail.getKey().substring(0, 1).toUpperCase() + detail.getKey().substring(1);
            ...
            if (val instanceof Map) {
              ...
              ((Map<String, Object>) val)
                  .forEach(
                      (k, v) -> {
                        nested
                            .append("<li><strong style='color:#7f8c8d;'>")
                            .append(k)
                            .append(":</strong> ")
                            .append(v)
                            .append("</li>");
              ...
              if ("status".equalsIgnoreCase(key) || "status".equalsIgnoreCase(detail.getKey())) {
                String sClass =
                    "UP".equals(val) ? "status-up" : ("DOWN".equals(val) ? "status-down" : "");
                sb.append("<tr><th>")
                    .append(key)
                    .append("</th><td><span class='")
                    .append(sClass)
                    .append("'>")
                    .append(val)
                    .append("</span></td></tr>");
    ...
    if (response.containsKey("error")) {
      sb.append(
              "<div class='card' style='border-left: 5px solid #e74c3c;'><h2>🚨 Critical Error</h2><p style='color:#e74c3c;font-weight:bold;'>")
          .append(response.get("error"))
          .append("</p></div>");
    }
  ...
  /**
   * Simple health check endpoint for basic monitoring. Always returns 200 OK with minimal
   * information.
   *
  ...
  private Map<String, Object> getApplicationStatus() {
    ...
    app.put(
        "startTime",
        LocalDateTime.ofInstant(Instant.ofEpochMilli(startTime), ZoneOffset.UTC).toString());
  ...
  /**
   * Lightweight health check for Render keep-alive (root level). Prevents caching to ensure fresh
   * status.
   */
