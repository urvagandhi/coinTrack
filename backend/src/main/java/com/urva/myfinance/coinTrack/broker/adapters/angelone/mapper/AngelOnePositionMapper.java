 ...
 *
 * <p>Angel One quirks: - All numeric fields are Strings — parsed via PriceNormalizer -
...
public class AngelOnePositionMapper {
  ...
  public CanonicalPosition toCanonical(
      AngelOnePositionRaw raw, String userId, String brokerAccountId) {
    ...
    BigDecimal avgBuyPrice =
        PriceNormalizer.toBigDecimal(raw.getBuyavgprice(), "buyavgprice", Broker.ANGELONE);

    // Average sell price from sellavgprice (String)
    BigDecimal avgSellPrice =
        PriceNormalizer.toBigDecimal(raw.getSellavgprice(), "sellavgprice", Broker.ANGELONE);
    ...
    BigDecimal realizedPnL =
        PriceNormalizer.toBigDecimal(
            raw.getRealisedprofitloss(), "realisedprofitloss", Broker.ANGELONE);

    // Unrealized P&L
    BigDecimal unrealizedPnL =
        PriceNormalizer.toBigDecimal(
            raw.getUnrealisedprofitloss(), "unrealisedprofitloss", Broker.ANGELONE);

    // Total P&L
    BigDecimal totalPnL =
        raw.getTotalprofitloss() != null
            ? PriceNormalizer.toBigDecimal(
                raw.getTotalprofitloss(), "totalprofitloss", Broker.ANGELONE)
    ...
    if (raw.getMultiplier() != null && !raw.getMultiplier().isBlank()) {
      try {
        ...
        log.warn(
            "Angel One: could not parse multiplier='{}' for symbol={}, defaulting to 1",
            raw.getMultiplier(),
            symbol);
      }
    }
    ...
    BigDecimal closePrice =
        raw.getClose() != null && !raw.getClose().isBlank()
