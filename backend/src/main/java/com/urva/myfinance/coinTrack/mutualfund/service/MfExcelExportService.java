...
/**
 * MfExcelExportService — single-responsibility service for generating the 5-sheet Mutual Fund Excel
 * workbook (§8 of the engineering spec).
 *
 * <p>It reads all data for the user, delegates aggregation to MfSchemeAggregationService, and
 * passes the results to MfExcelExporter. This keeps export concerns separated from the
 * aggregation/calculation logic in MfSchemeAggregationService.
 */
@Service
public class MfExcelExportService {
  ...
  /**
   * Builds and returns the 5-sheet Excel workbook for the given user. Sheets: 1. MF Investment
   * (Scheme Summary) 2. Investment & Valuation (Valuation Snapshots) 3. Lumpsum Investment 4.
   * Redemption Investment 5. SIP Details (Contributions)
   */
  public ResponseEntity<byte[]> exportToExcel(String userId) {
    ...
    Map<String, MfScheme> schemeMap =
        schemes.stream().collect(Collectors.toMap(MfScheme::getId, s -> s));
    ...
    return MfExcelExporter.export(
        summaries, snapshots, lumpsums, redemptions, sips, schemeMap, overallSummary);
  }
}
