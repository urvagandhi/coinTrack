...
public class EpfInterestAccrualService {
  ...
  public EpfInterestAccrualService(
      EpfInterestRateConfig epfInterestRateConfig, MongoTemplate mongoTemplate) {
  ...
  public EpfInterestAccrualResult calculateAccruedInterest(String userId, String financialYear) {
    logger.debug(
        "Calculating EPF/EPS interest accrual for user: {}, FY: {}", userId, financialYear);
    ...
    if (epfInterestRateConfig.getInterestRates() != null) {
      interestRate =
          epfInterestRateConfig.getInterestRates().stream()
    ...
    Query openingQuery =
        new Query(Criteria.where("userId").is(userId).and("transactionDate").lte(previousFyEnd))
            .with(
    ...
    Query fyTxnsQuery =
        new Query(
                Criteria.where("userId").is(userId).and("transactionDate").gte(fyStart).lte(fyEnd))
            .with(
    ...
    List<EpfTransaction> transactions =
        allFyTransactions.stream()
            .filter(t -> t.getRemarks() == null || !t.getRemarks().equals(creditRemarks))
            .toList();

    // 5. Compute interest for EPF
    BigDecimal accruedEpfInterest =
        calculateInterestForPool(openingEpfBalance, transactions, annualRate, true);

    // 6. Compute interest for EPS
    BigDecimal accruedEpsInterest =
        calculateInterestForPool(openingEpsBalance, transactions, annualRate, false);
  ...
  private BigDecimal calculateInterestForPool(
    ...
    for (EpfTransaction txn : transactions) {
      ...
      if (monthIndex >= 1 && monthIndex <= 12) {
        if (isEpfPool) {
          BigDecimal employeePart =
              txn.getEmployeeContribution() != null
                  ? txn.getEmployeeContribution()
                  : BigDecimal.ZERO;
          BigDecimal employerPart =
              txn.getEmployerEpfContribution() != null
                  ? txn.getEmployerEpfContribution()
                  : BigDecimal.ZERO;
          BigDecimal vpfPart = txn.getVpfAmount() != null ? txn.getVpfAmount() : BigDecimal.ZERO;
          BigDecimal withdrawalPart =
              txn.getWithdrawalAmount() != null ? txn.getWithdrawalAmount() : BigDecimal.ZERO;

          monthlyContributions[monthIndex] =
              monthlyContributions[monthIndex].add(employeePart).add(employerPart).add(vpfPart);
          monthlyWithdrawals[monthIndex] = monthlyWithdrawals[monthIndex].add(withdrawalPart);
        } else {
          BigDecimal epsPart =
              txn.getEmployerEpsContribution() != null
                  ? txn.getEmployerEpsContribution()
                  : BigDecimal.ZERO;
