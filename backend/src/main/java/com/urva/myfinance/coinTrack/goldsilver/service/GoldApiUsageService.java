...
/**
 * Monitors GoldAPI.io usage statistics, health status, and enforces the 100 req/month quota limit
 * with a configurable safety buffer.
 *
 * <p>This service wraps two GoldAPI endpoints:
 *
 * <ul>
 *   <li>{@code /api/status} — lightweight health check
 *   <li>{@code /api/stat} — usage statistics (today, yesterday, this month, last month)
 * </ul>
 *
 * <p><strong>Important:</strong> The /api/stat call itself counts towards the quota, so usage is
 * cached for 30 minutes to avoid burning requests on monitoring.
 */
@Service
public class GoldApiUsageService {
  ...
  /**
   * Checks if the GoldAPI service is healthy and responding. A response with {@code result: true}
  ...
  /**
   * Fetches current API usage statistics from GoldAPI. Results are cached for 30 minutes to avoid
   * wasting quota on monitoring.
   *
  ...
  public GoldApiUsageDTO getUsageStats(boolean forceRefresh) {
    if (!forceRefresh
      ...
      logger.debug(
          "Returning cached GoldAPI usage stats (cached {}s ago)",
    ...
    try {
      ...
      if (response.statusCode() != 200) {
        logger.error(
            "GoldAPI stat endpoint returned HTTP {}: {}", response.statusCode(), response.body());
  ...
  /**
   * Determines whether a rate fetch should be allowed based on the current monthly usage vs the
   * configured quota limit (minus safety buffer).
   *
   * <p>The effective limit is {@code monthlyLimit - safetyBuffer} (default: 95). This reserves a
   * small buffer for manual refreshes and stat checks.
   *
  ...
  public boolean isWithinQuota() {
    ...
    if (currentMonthUsage >= monthlyLimit) {
      logger.error(
          "🚫 GoldAPI monthly quota EXHAUSTED! Used {}/{} requests. Blocking all further API calls.",
          currentMonthUsage,
          monthlyLimit);
      return false;
    }
    ...
    if (currentMonthUsage >= effectiveLimit) {
      logger.warn(
          "⚠️ GoldAPI quota nearly exhausted! Used {}/{} requests (safety buffer: {}). Blocking scheduled fetches.",
          currentMonthUsage,
          monthlyLimit,
          safetyBuffer);
      return false;
    ...
    logger.info(
        "GoldAPI quota check passed: {}/{} requests used this month ({} remaining)",
        currentMonthUsage,
        monthlyLimit,
        monthlyLimit - currentMonthUsage);
    return true;
  }
  ...
  private void logUsageWarnings(GoldApiUsageDTO usage) {
    ...
    if (usagePercent >= 90) {
      logger.error(
          "🔴 CRITICAL: GoldAPI usage at {}/{} ({}%). Only {} requests remaining!",
          used, monthlyLimit, usagePercent, remaining);
    } else if (usagePercent >= 75) {
      logger.warn(
          "🟡 WARNING: GoldAPI usage at {}/{} ({}%). {} requests remaining.",
          used, monthlyLimit, usagePercent, remaining);
    } else if (usagePercent >= 50) {
      logger.info(
          "🟢 GoldAPI usage at {}/{} ({}%). {} requests remaining.",
          used, monthlyLimit, usagePercent, remaining);
    } else {
      logger.debug(
          "GoldAPI usage: {}/{} requests this month. {} remaining.", used, monthlyLimit, remaining);
    }
  }
}
