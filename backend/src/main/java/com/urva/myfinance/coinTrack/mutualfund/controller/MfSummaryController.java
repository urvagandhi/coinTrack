...
public class MfSummaryController {
  ...
  @Autowired private MfExcelExportService excelExportService;
  @Autowired private PortfolioDashboardService dashboardService;
  ...
  public ResponseEntity<ApiResponse<DashboardSummaryDto>> getDashboardSummary(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(summary, "Fetched dashboard summary successfully"));
  }
        ...
        schemes.stream()
            ...
            .filter(
                sm ->
                    includeRedeemed
                        || (sm.getStatuses() != null
                            && !sm.getStatuses().contains(FundStatus.FULLY_REDEEMED)))
            .collect(Collectors.toList());

    return ResponseEntity.ok(
        ApiResponse.success(summaries, "Fetched scheme summaries successfully"));
  }
  ...
  @GetMapping("/export")
  public ResponseEntity<byte[]> exportToExcel(@AuthenticationPrincipal UserPrincipal userDetails) {
    try {
