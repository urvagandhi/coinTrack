 ...
 * TOTP 2FA service.
 *
 * <p>Changed: Replaced TotpEncryptionUtil with EncryptionUtil.encrypt/decrypt(text, totpKey) static
 * overloads. Single encryption implementation, separate key for TOTP secrets.
 */
@Service
public class TotpService {
  ...
  public void saveBackupCodes(User user, List<String> plaintextCodes, int version) {
    ...
    logger.info(
        "Saved {} backup codes for user {} (generation: {})",
        hashedCodes.size(),
        user.getId(),
        version);
  }
  ...
  private TotpSetupResponse buildQrResponse(String email, String secret) {
    QrData qrData = new QrData.Builder().label(email).secret(secret).issuer(issuer).build();
