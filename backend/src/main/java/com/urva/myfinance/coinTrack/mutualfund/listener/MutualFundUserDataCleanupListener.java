...
/**
 * Removes all mutual-fund data (schemes, ledgers, holdings, metrics, snapshots) for a deleted user.
 * NAV cache and LTP repositories are shared/global — kept.
 */
@Component
public class MutualFundUserDataCleanupListener {

  private static final Logger logger =
      LoggerFactory.getLogger(MutualFundUserDataCleanupListener.class);
  ...
  public MutualFundUserDataCleanupListener(
      MfSchemeRepository schemeRepository,
  ...
  public void onUserDeleted(UserDeletedEvent event) {
    ...
    try {
      ...
      logger.error(
          "Cascade cleanup failed for mutual-fund data, user {}: {}", userId, e.getMessage());
    }
  }
}
