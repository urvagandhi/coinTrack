...
public class PendingTransactionSettlementService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(PendingTransactionSettlementService.class);
  ...
  @Autowired private MfSchemeRepository schemeRepo;
  ...
  private void processPendingLumpsum() {
    ...
    for (LumpsumTransaction t : pending) {
      ...
      if (optScheme.isEmpty()
          || optScheme.get().getAmfiCode() == null
          || optScheme.get().getAmfiCode().isEmpty()) {
      ...
      BigDecimal nav =
          mfNavService.fetchNavForDate(optScheme.get().getAmfiCode(), t.getApplicableDate());
      if (nav != null) {
        t.setNavPrice(nav);
  ...
  private void incrementRetryLumpsum(LumpsumTransaction t) {
    ...
    if (retries >= MAX_RETRIES) {
      ...
      logger.error(
          "Lumpsum Transaction {} failed after {} retries. Needs manual review.",
          t.getId(),
          MAX_RETRIES);
    } else {
  ...
  private void incrementRetrySip(SipContribution t) {
    ...
    if (retries >= MAX_RETRIES) {
      t.setStatus(TransactionStatus.FAILED);
      logger.error(
          "SIP Contribution {} failed after {} retries. Needs manual review.",
          t.getId(),
          MAX_RETRIES);
    } else {
      t.setRetryCount(retries + 1);
  ...
  private void processPendingRedemption() {
    List<RedemptionTransaction> pending =
        redemptionRepo.findByStatus(TransactionStatus.PENDING_NAV);
    for (RedemptionTransaction t : pending) {
      ...
      if (nav != null) {
        ...
        if (t.getRedemptionUnit() != null) {
          ...
          if (fifoResult.ltcgUnits.compareTo(BigDecimal.ZERO) > 0
              && fifoResult.stcgUnits.compareTo(BigDecimal.ZERO) == 0) {
            t.setGainType(GainType.LTCG);
          } else if (fifoResult.stcgUnits.compareTo(BigDecimal.ZERO) > 0
              && fifoResult.ltcgUnits.compareTo(BigDecimal.ZERO) == 0) {
            t.setGainType(GainType.STCG);
          } else if (fifoResult.ltcgUnits.compareTo(BigDecimal.ZERO) > 0
              && fifoResult.stcgUnits.compareTo(BigDecimal.ZERO) > 0) {
  ...
  private void incrementRetryRedemption(RedemptionTransaction t) {
    ...
    if (retries >= MAX_RETRIES) {
      t.setStatus(TransactionStatus.FAILED);
      logger.error(
          "Redemption Transaction {} failed after {} retries. Needs manual review.",
          t.getId(),
          MAX_RETRIES);
    } else {
      t.setRetryCount(retries + 1);
