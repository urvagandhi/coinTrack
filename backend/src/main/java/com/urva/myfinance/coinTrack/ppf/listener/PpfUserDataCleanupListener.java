...
/**
 * Removes PPF ledger transactions for a deleted user.
 * PPF settings live embedded in the user document — removed with the user itself.
 */
@Component
public class PpfUserDataCleanupListener {
    ...
    public void onUserDeleted(UserDeletedEvent event) {
        try {
            ...
            logger.error("Cascade cleanup failed for PPF transactions, user {}: {}", event.userId(), e.getMessage());
        }
    }
}
