...
class MarketDataServiceImplTest {
  ...
  void getPrices_partialCacheHit_fetchesMissing() {
    ...
    when(priceRepository.findBySymbolIn(List.of(SYMBOL_A, SYMBOL_B))).thenReturn(List.of(cached));
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(Collections.emptyList());

    // Also check fallback for SYMBOL_B
    when(holdingRepository.findBySymbolIn(List.of(SYMBOL_B))).thenReturn(Collections.emptyList());
  ...
  void getPrices_noZerodhaAccounts_fallsBackToCanonical() {
    when(priceRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(Collections.emptyList());
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(Collections.emptyList());
    ...
    com.urva.myfinance.coinTrack.broker.core.canonical.CanonicalHolding holding =
        com.urva
            ...
            .build();
    when(holdingRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(List.of(holding));
  ...
  void getPrices_noZerodhaNoCanonical_returnsEmptyForMissing() {
    when(priceRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(Collections.emptyList());
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(Collections.emptyList());
    when(holdingRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(Collections.emptyList());
  ...
  void getPrices_expiredToken_skipped() {
    ...
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(List.of(expiredAccount));
    when(holdingRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(Collections.emptyList());
  ...
  void getPrices_inactiveAccount_skipped() {
    ...
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(List.of(inactiveAccount));
    when(holdingRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(Collections.emptyList());
  ...
  void getPrices_nullAccessToken_skipped() {
    ...
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(List.of(noTokenAccount));
    when(holdingRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(Collections.emptyList());
  ...
  void getPrice_missingSymbol_returnsStaleZeroPrice() {
    when(priceRepository.findBySymbolIn(List.of("UNKNOWN"))).thenReturn(Collections.emptyList());
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(Collections.emptyList());
    when(holdingRepository.findBySymbolIn(List.of("UNKNOWN"))).thenReturn(Collections.emptyList());
  ...
  void warmupPrices_completesWithoutThrowing() {
    when(priceRepository.findBySymbolIn(anyList())).thenReturn(Collections.emptyList());
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(Collections.emptyList());
    when(holdingRepository.findBySymbolIn(anyList())).thenReturn(Collections.emptyList());
  ...
  void fallbackToCanonical_staleWithin24h_isUsed() {
    ...
    when(priceRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(List.of(staleButUsable));
  ...
  void fallbackToCanonical_cacheOlderThan24h_discarded() {
    ...
    when(priceRepository.findBySymbolIn(List.of(SYMBOL_A))).thenReturn(List.of(tooOld));
  ...
  void getPrices_permissionDeniedAccount_skipped() {
    ...
    when(brokerAccountRepository.findByBroker(Broker.ZERODHA)).thenReturn(List.of(validAccount));
