...
class BrokerSessionTest {
  ...
  void isExpired_PastExpiry_ReturnsTrue() {
    BrokerSession session =
        new BrokerSession(
            "acc-1", Broker.ZERODHA, "token", Instant.now().minusSeconds(60), Map.of());
  ...
  void isExpired_FutureExpiry_ReturnsFalse() {
    BrokerSession session =
        new BrokerSession(
            "acc-1", Broker.ZERODHA, "token", Instant.now().plusSeconds(3600), Map.of());
  ...
  void isExpiringSoon_WithinOneHour_ReturnsTrue() {
    BrokerSession session =
        new BrokerSession(
            "acc-1", Broker.ZERODHA, "token", Instant.now().plusSeconds(1800), Map.of());
  ...
  void isExpiringSoon_MoreThanOneHour_ReturnsFalse() {
    BrokerSession session =
        new BrokerSession(
            "acc-1", Broker.ZERODHA, "token", Instant.now().plusSeconds(7200), Map.of());
  ...
  void recordFields_Accessible() {
    ...
    BrokerSession session =
        new BrokerSession(
            "acc-1", Broker.ANGELONE, "jwt-token", Instant.ofEpochMilli(1700000000000L), metadata);
  ...
  void isExpiringSoon_NullMetadata_Works() {
    BrokerSession session =
        new BrokerSession("acc-1", Broker.UPSTOX, "token", Instant.now().plusSeconds(7200), null);
    assertFalse(session.isExpiringSoon());
  }
}
