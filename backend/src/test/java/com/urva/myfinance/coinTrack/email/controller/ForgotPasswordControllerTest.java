...
class ForgotPasswordControllerTest {
  ...
  void requestPasswordReset_blankId_400() {
    ResponseEntity<?> resp =
        controller.requestPasswordReset(Map.of("identifier", "  "), httpRequest);
    assertEquals(HttpStatus.BAD_REQUEST, resp.getStatusCode());
  }
  ...
  void requestPasswordReset_notFound_200() {
    ...
    when(httpRequest.getHeader("User-Agent")).thenReturn("test");
    ResponseEntity<?> resp =
        controller.requestPasswordReset(Map.of("identifier", "unknown"), httpRequest);
  ...
  void requestPasswordReset_found_200() {
    ...
    when(httpRequest.getHeader("X-Forwarded-For")).thenReturn(null);
    when(httpRequest.getHeader("User-Agent")).thenReturn("test");
    when(emailTokenService.createToken(any(), eq(EmailToken.PURPOSE_PASSWORD_RESET), any()))
        .thenReturn("token123");
    when(emailConfig.getPasswordResetUrl("token123")).thenReturn("https://reset.link");

    ResponseEntity<?> resp =
        controller.requestPasswordReset(Map.of("identifier", "test@example.com"), httpRequest);
  ...
  void requestPasswordReset_emailFails_200() {
    ...
    doThrow(new RuntimeException("email down"))
        .when(emailService)
        .sendPasswordResetLink(any(), anyString());
  ...
  void verifyResetToken_userNotFound_400() {
    EmailToken dbToken =
        EmailToken.builder()
            ...
            .build();
    when(emailTokenService.validateToken("tok", EmailToken.PURPOSE_PASSWORD_RESET))
        .thenReturn(dbToken);
  ...
  void resetPassword_weakPass_400() {
    ResponseEntity<?> resp =
        controller.resetPassword(null, Map.of("newPassword", "weak"), httpRequest);
    assertEquals(HttpStatus.BAD_REQUEST, resp.getStatusCode());
  }
  ...
  void resetPassword_noAuth_401() {
    ResponseEntity<?> resp =
        controller.resetPassword(null, Map.of("newPassword", "Strong1!@#"), httpRequest);
