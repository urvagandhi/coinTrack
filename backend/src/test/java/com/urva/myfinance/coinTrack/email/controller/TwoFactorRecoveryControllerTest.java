...
class TwoFactorRecoveryControllerTest {
  ...
  void request2FARecovery_notFound_200() {
    ...
    when(httpRequest.getHeader("User-Agent")).thenReturn("test");
    ResponseEntity<?> resp =
        controller.request2FARecovery(Map.of("identifier", "unknown"), httpRequest);
  ...
  void request2FARecovery_no2FA_200() {
    ...
    ResponseEntity<?> resp =
        controller.request2FARecovery(Map.of("identifier", "test@example.com"), httpRequest);
    assertEquals(HttpStatus.OK, resp.getStatusCode());
  }

  @Test
  @DisplayName(
      "request2FARecovery: user found, 2FA enabled, email not verified → 200 (no enumeration)")
  ...
  void request2FARecovery_valid_200() {
    ...
    when(userRepository.findByEmail("test@example.com")).thenReturn(user);
    when(emailTokenService.createToken(any(), eq(EmailToken.PURPOSE_2FA_RECOVERY), any()))
        .thenReturn("token");
  ...
  void request2FARecovery_emailFails_200() {
    ...
    doThrow(new RuntimeException("email down"))
        .when(emailService)
        .send2FARecoveryLink(any(), anyString());
  ...
  void verify2FARecovery_userNotFound_400() {
    EmailToken dbToken =
        EmailToken.builder().id("t1").userId("u1").purpose(EmailToken.PURPOSE_2FA_RECOVERY).build();
    when(emailTokenService.validateToken("tok", EmailToken.PURPOSE_2FA_RECOVERY))
        .thenReturn(dbToken);
  ...
  void verify2FARecovery_alertFails_200() {
    ...
    doThrow(new RuntimeException("email down"))
        .when(emailService)
        .sendSecurityAlert(any(), anyString());
