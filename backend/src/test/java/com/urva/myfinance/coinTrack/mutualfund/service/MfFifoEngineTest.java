...
class MfFifoEngineTest {
  ...
  @Mock private LumpsumTransactionRepository lumpsumRepository;
  @Mock private SipContributionRepository sipRepository;
  @Mock private RedemptionTransactionRepository redemptionRepository;
  @Mock private MfSchemeRepository schemeRepository;

  @InjectMocks private MfFifoEngine fifoEngine;
  ...
  @BeforeEach
  void setUp() {
    when(sipRepository.findByUserIdAndSchemeId(anyString(), anyString()))
        .thenReturn(new ArrayList<>());
    when(redemptionRepository.findByUserIdAndSchemeId(anyString(), anyString()))
        .thenReturn(new ArrayList<>());
  }
  ...
  void calculateRedemptionCost_sameDay_differentCreatedAt() {
    ...
    when(lumpsumRepository.findByUserIdAndSchemeId(USER_ID, SCHEME_ID))
    ...
    assertEquals(
        0,
        new BigDecimal("10000").compareTo(result.totalCostValue),
        "Should consume earlier lot first");
  }
  ...
  void calculateRedemptionCost_debtFund_2Years_stcg() {
    ...
    assertEquals(
        0,
        new BigDecimal("100").compareTo(result.stcgUnits),
        "Should be STCG (Debt threshold = 3 years)");
  ...
  void calculateRedemptionCost_equityFund_2Years_ltcg() {
    ...
    assertEquals(
        0,
        new BigDecimal("0").compareTo(result.stcgUnits),
