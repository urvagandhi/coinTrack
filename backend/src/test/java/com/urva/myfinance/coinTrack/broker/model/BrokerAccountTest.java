...
class BrokerAccountTest {
  ...
  void hasCredentials_Zerodha_WithApiKey_ReturnsTrue() {
    BrokerAccount account =
        BrokerAccount.builder().broker(Broker.ZERODHA).zerodhaApiKey("api-key-123").build();
  ...
  void hasCredentials_Zerodha_NullApiKey_ReturnsFalse() {
    BrokerAccount account = BrokerAccount.builder().broker(Broker.ZERODHA).build();
  ...
  void hasCredentials_Zerodha_EmptyApiKey_ReturnsFalse() {
    BrokerAccount account =
        BrokerAccount.builder().broker(Broker.ZERODHA).zerodhaApiKey("").build();
    assertFalse(account.hasCredentials());
  }
  ...
  void hasCredentials_Upstox_MissingSecret_ReturnsFalse() {
    BrokerAccount account =
        BrokerAccount.builder().broker(Broker.UPSTOX).upstoxApiKey("up-key").build();
    assertFalse(account.hasCredentials());
  }
  ...
  void isTokenExpired_AngelOne_NullExpiry_ReturnsTrue() {
    BrokerAccount account = BrokerAccount.builder().broker(Broker.ANGELONE).build();
    assertTrue(account.isTokenExpired());
  }
  ...
  void isTokenExpired_Generic_NullExpiry_ReturnsTrue() {
    BrokerAccount account = BrokerAccount.builder().broker(Broker.UPSTOX).build();
    assertTrue(account.isTokenExpired());
  }
  ...
  void hasCredentials_NullBroker_ReturnsFalse() {
    BrokerAccount account = BrokerAccount.builder().build();
    assertFalse(account.hasCredentials());
  }
  ...
  void hasValidToken_Zerodha_NullExpiry_ReturnsFalse() {
    BrokerAccount account =
        BrokerAccount.builder().broker(Broker.ZERODHA).zerodhaAccessToken("token").build();
    assertFalse(account.hasValidToken());
  }
}
