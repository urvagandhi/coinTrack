...
public class SipCalculationTest {
  ...
  @Autowired private SettlementDateCalculator calculator;
  ...
  public void testSbiSmallCap() {
    ...
    String amfiCode =
        "125497"; // I'll need to check the exact AMFI code for "SBI Small Cap - Direct Plan -
    // Growth"
    ...
    while (!currentDate.isAfter(endDate)) {
      ...
      if (nav != null) {
        ...
        System.out.printf(
            "Date: %s, Applicable: %s, NAV: %s, Units: %s%n",
            currentDate, applicableDate, nav, units);
      } else {
        System.out.printf(
            "Date: %s, Applicable: %s - NAV NOT FOUND!%n", currentDate, applicableDate);
      }
