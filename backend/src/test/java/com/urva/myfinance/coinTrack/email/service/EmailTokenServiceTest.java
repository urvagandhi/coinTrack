...
class EmailTokenServiceTest {
  ...
  void setUp() {
    ...
    sampleUser =
        User.builder().id("u1").username("testuser").email("test@example.com").name("Test").build();
  ...
  void createToken_createsAndSaves() {
    String token =
        emailTokenService.createToken(sampleUser, EmailToken.PURPOSE_EMAIL_VERIFY, httpRequest);
  ...
  void validateToken_purposeMismatch_throws() {
    EmailToken dbToken =
        EmailToken.builder()
            .id("tid")
            .userId("u1")
            .purpose(EmailToken.PURPOSE_EMAIL_VERIFY)
            .used(false)
    ...
    assertThrows(
        ...
        () ->
            emailTokenService.validateToken(
                createToken("tid", "u1", "WRONG_PURPOSE"), EmailToken.PURPOSE_EMAIL_VERIFY));
  }
