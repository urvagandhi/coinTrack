...
class SipContributionSchedulerTest {
  ...
  @Mock private SipContributionRepository sipContributionRepository;

  @Mock private SipContributionService sipContributionService;

  @InjectMocks private SipContributionScheduler scheduler;
  ...
  void generateMonthlySipContributions_skipsIfExists() {
    ...
    when(sipContributionRepository.existsBySipMandateIdAndContributionDateBetween(
            eq("m1"), any(LocalDate.class), any(LocalDate.class)))
        .thenReturn(true);

    scheduler.generateMonthlySipContributions();

    verify(sipContributionService, never())
        .createContribution(anyString(), any(SipContribution.class));
  }
}
