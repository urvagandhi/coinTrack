...
class PortfolioSyncServiceImplTest {
  ...
  class SyncUser {
    ...
    void partialFailure() {
      ...
      verify(syncLogRepository)
          .save(argThat(l -> SyncStatus.PARTIAL_FAILURE.equals(l.getStatus())));
    }
    ...
    void staleBroker() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      when(aggregationService.aggregateForUser("u1"))
          .thenReturn(aggWithStale(Set.of(Broker.ZERODHA)));
    ...
    void duplicateUsersSkipped() {
      ...
      BrokerAccount a2 =
          BrokerAccount.builder()
              ...
              .angelOneApiKey("k2")
              .angelOneClientCode("c2")
              .encryptedAngelOnePassword("p2")
              .encryptedAngelOneTotpSecret("t2")
  ...
  class TriggerManualRefreshForUser {
    ...
    void noCredentials() {
      BrokerAccount bare =
          BrokerAccount.builder().id("acc-bare").userId("u1").broker(Broker.ZERODHA).build();
  ...
  class PersistAggregatedData {
    ...
    void newHoldings() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      CanonicalHolding h = CanonicalHolding.builder().isin("INE123").symbol("RELIANCE").build();
    ...
    void existingHoldings() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      CanonicalHolding h = CanonicalHolding.builder().isin("INE123").symbol("RELIANCE").build();
      CanonicalHolding existing =
          CanonicalHolding.builder().id("existing-id").isin("INE123").build();
      when(aggregationService.aggregateForUser("u1")).thenReturn(aggWithHoldings(h));
      when(holdingRepository.findByUserIdAndBrokerAccountIdAndIsin(any(), any(), any()))
    ...
    void newPositions() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      CanonicalPosition p = CanonicalPosition.builder().symbol("RELIANCE").build();
    ...
    void existingPositions() {
      ...
      CanonicalPosition existing =
          CanonicalPosition.builder().id("pos-existing").symbol("RELIANCE").build();
      when(aggregationService.aggregateForUser("u1")).thenReturn(aggWithPosition(p));
      when(positionRepository.findByUserIdAndBrokerAccountIdAndSymbolAndInstrumentType(
              any(), any(), any(), any()))
          .thenReturn(Optional.of(existing));
    ...
    void newMfHoldings() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      CanonicalMfHolding mf =
          CanonicalMfHolding.builder().isin("INF123").fundName("Test Fund").build();
    ...
    void existingMfHoldings() {
      ...
      CanonicalMfHolding mf =
          CanonicalMfHolding.builder().isin("INF123").fundName("Test Fund").build();
      CanonicalMfHolding existing =
          CanonicalMfHolding.builder().id("mf-existing").isin("INF123").build();
      when(aggregationService.aggregateForUser("u1")).thenReturn(aggWithMfHolding(mf));
      when(mfHoldingRepository.findByUserIdAndBrokerAccountIdAndIsin(any(), any(), any()))
          .thenReturn(Optional.of(existing));
    ...
    void funds() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      CanonicalFunds funds = CanonicalFunds.builder().brokerAccountId("acc1").build();
      when(aggregationService.aggregateForUser("u1"))
          .thenReturn(
              new AggregatedPortfolio(
                  Collections.emptyList(),
                  Collections.emptyList(),
                  Map.of(Broker.ZERODHA, funds),
                  Collections.emptyList(),
                  Collections.emptyList(),
                  Instant.now(),
                  Collections.emptySet()));
    ...
    void existingFunds() {
      when(brokerAccountRepository.findByUserId("u1")).thenReturn(List.of(activeAccount));
      CanonicalFunds funds = CanonicalFunds.builder().brokerAccountId("acc1").build();
      CanonicalFunds existing = CanonicalFunds.builder().id("funds-existing").build();
      when(aggregationService.aggregateForUser("u1"))
          .thenReturn(
              new AggregatedPortfolio(
                  Collections.emptyList(),
