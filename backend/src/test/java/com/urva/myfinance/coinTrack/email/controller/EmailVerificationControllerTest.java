...
class EmailVerificationControllerTest {
  ...
  void verifyEmail_userNotFound_400() {
    EmailToken dbToken =
        EmailToken.builder().id("t1").userId("u1").purpose(EmailToken.PURPOSE_EMAIL_VERIFY).build();
    when(emailTokenService.validateToken("tok", EmailToken.PURPOSE_EMAIL_VERIFY))
        .thenReturn(dbToken);
  ...
  void verifyEmail_emailChange_200() {
    EmailToken dbToken =
        EmailToken.builder()
            .id("t1")
            .userId("u1")
            .purpose(EmailToken.PURPOSE_EMAIL_CHANGE_VERIFY)
            .newEmail("new@example.com")
            .build();
    when(emailTokenService.validateToken("tok", EmailToken.PURPOSE_EMAIL_CHANGE_VERIFY))
        .thenReturn(dbToken);
    User user = User.builder().id("u1").email("old@example.com").build();
    when(userRepository.findById("u1")).thenReturn(Optional.of(user));

    ResponseEntity<?> resp =
        controller.verifyEmail(Map.of("token", "tok", "type", "change"), httpRequest);
    assertEquals(HttpStatus.OK, resp.getStatusCode());
  ...
  void verifyEmail_emailChange_alertFails_200() {
    ...
    when(emailTokenService.validateToken("tok", EmailToken.PURPOSE_EMAIL_CHANGE_VERIFY))
        .thenReturn(dbToken);
    User user =
        User.builder().id("u1").username("testuser").name("Test").email("old@example.com").build();
    when(userRepository.findById("u1")).thenReturn(Optional.of(user));
    doThrow(new RuntimeException("email down"))
        .when(emailService)
        .sendSecurityAlert(any(), anyString(), anyMap());
  ...
  void resendVerification_notVerified_200() {
    ...
    when(userDetails.getUsername()).thenReturn("testuser");
    when(userRepository.findByUsername("testuser")).thenReturn(user);
    when(emailTokenService.createToken(any(), eq(EmailToken.PURPOSE_EMAIL_VERIFY), any()))
        .thenReturn("token");
  ...
  void resendVerification_emailFails_200() {
    ...
    doThrow(new RuntimeException("email down"))
        .when(emailService)
        .sendEmailVerification(any(), anyString());
