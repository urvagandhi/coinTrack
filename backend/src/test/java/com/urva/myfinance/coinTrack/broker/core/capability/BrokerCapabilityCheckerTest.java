...
class BrokerCapabilityCheckerTest {
  ...
  @Mock private BrokerAdapter adapter;

  @InjectMocks private BrokerCapabilityChecker checker;
  ...
  void supports_CapabilityPresent_ReturnsTrue() {
    when(adapter.getCapabilities())
        .thenReturn(Set.of(BrokerCapability.EQUITY_HOLDINGS, BrokerCapability.FUNDS));
  ...
  void require_CapabilityAbsent_ThrowsException() {
    ...
    UnsupportedBrokerOperationException ex =
        assertThrows(
