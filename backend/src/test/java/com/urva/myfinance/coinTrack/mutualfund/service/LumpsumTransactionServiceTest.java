...
class LumpsumTransactionServiceTest {
  ...
  @Mock private StatutoryChargesConfig mfChargesConfig;
  ...
  void setUp() {
    ...
    when(settlementDateCalculator.calculateSettlementDate(any(), any()))
        .thenReturn(LocalDate.of(2025, 1, 17));
  ...
  void createTransaction_autoCalculatesUnitsWhenNull() {
    ...
    verify(redemptionTransactionService)
        .recalculateRedemptionsAfterDate(USER_ID, SCHEME_ID, sampleTx.getInvestmentDate());
  }
  ...
  void updateTransaction_schemeChanged() {
    ...
    assertThrows(RuntimeException.class, () -> service.updateTransaction(USER_ID, TX_ID, updated));
  }
