...
class UserServiceTest {
  ...
  @Mock
  private com.urva.myfinance.coinTrack.user.repository.BackupCodeRepository backupCodeRepository;

  @Mock
  private com.urva.myfinance.coinTrack.user.repository.UserDeletionAuditRepository
      userDeletionAuditRepository;
  ...
  @Mock
  private com.urva.myfinance.coinTrack.security.repository.InvalidatedTokenRepository
  ...
  void registerUser_phoneNormalization() {
    ...
    verify(pendingRegistrationRepository)
        .save(argThat(p -> "+919876543210".equals(p.getPhoneNumber())));
  }
  ...
  void completePendingRegistration_notFound_throws() {
    ...
    assertThrows(
        AuthenticationException.class, () -> userService.completePendingRegistration(sampleUser));
  }
  ...
  void completePendingRegistration_success() {
    when(pendingRegistrationRepository.findByUsername("testuser"))
        .thenReturn(Optional.of(new PendingRegistration()));
  ...
  void changePassword_wrongCurrent_throws() {
    ...
    assertThrows(
        RuntimeException.class, () -> userService.changePassword("u1", "wrong", "NewPass1!"));
  }
