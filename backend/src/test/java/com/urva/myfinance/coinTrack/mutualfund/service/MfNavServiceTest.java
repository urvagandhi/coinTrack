...
class MfNavServiceTest {
  ...
  @Mock private MutualFundNavCacheRepository navCacheRepository;

  @Mock private RestTemplate restTemplate;

  @InjectMocks private MfNavService mfNavService;
  ...
  void fetchNavForDate_tigzigHit() {
    ...
    when(restTemplate.getForObject(contains("tigzig.com"), eq(Map.class))).thenReturn(response);
  ...
  void fetchNavForDate_mfapiHit() {
    ...
    when(restTemplate.getForObject(contains("tigzig.com"), eq(Map.class))).thenReturn(null);

    Map<String, String> row = Map.of("date", "15-01-2025", "nav", "520.20");
    Map<String, Object> response = Map.of("status", "SUCCESS", "data", List.of(row));
    when(restTemplate.getForObject(contains("mfapi.in"), eq(Map.class))).thenReturn(response);
  ...
  void fetchNavForDate_missingDate() {
    ...
    // Date returned is later than target date (descending order), and then it skips to earlier
    // date, missing target
