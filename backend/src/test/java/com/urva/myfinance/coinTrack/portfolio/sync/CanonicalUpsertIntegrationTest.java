 ...
 *
 * <p>Tests the exact pattern used by PortfolioSyncServiceImpl.persistAggregatedData(): 1.
 * findByCompoundKey() 2. if found → setId(existing.getId()) 3. save()
 *
 * <p>These tests verify: - Insert creates a new document - Upsert (find-then-save) updates the
 * existing document without creating duplicates - Unique compound index rejects raw duplicates (no
 * upsert logic) - Fields actually get updated on upsert - Null ISIN edge cases - Multi-broker
 * same-stock isolation
 */
@DataMongoTest
@ActiveProfiles("test")
class CanonicalUpsertIntegrationTest {
  ...
  @Autowired private CanonicalHoldingRepository holdingRepo;

  @Autowired private CanonicalPositionRepository positionRepo;

  @Autowired private CanonicalMfHoldingRepository mfHoldingRepo;

  @Autowired private CanonicalFundsRepository fundsRepo;
  ...
  class HoldingUpsertTests {
    ...
    void insertCreatesNewHolding() {
      CanonicalHolding h =
          buildHolding(
              "INE002A01018", "NSE:RELIANCE", new BigDecimal("10"), new BigDecimal("2500.00"));
      holdingRepo.save(h);
    ...
    void upsertUpdatesExistingHolding() {
      ...
      CanonicalHolding h2 =
          buildHolding(
              "INE002A01018", "NSE:RELIANCE", new BigDecimal("15"), new BigDecimal("2600.00"));
      holdingRepo
          .findByUserIdAndBrokerAccountIdAndIsin(USER_ID, ACCOUNT_ZERODHA, "INE002A01018")
    ...
    void upsertOverwritesAllFields() {
      CanonicalHolding h1 =
          buildHolding("INE009A01021", "NSE:INFY", new BigDecimal("20"), new BigDecimal("1500.00"));
      h1.setDayChange(new BigDecimal("25.00"));
      holdingRepo.save(h1);

      // Second sync — new object has no dayChange set (null)
      CanonicalHolding h2 =
          buildHolding("INE009A01021", "NSE:INFY", new BigDecimal("25"), new BigDecimal("1520.00"));
      holdingRepo
          .findByUserIdAndBrokerAccountIdAndIsin(USER_ID, ACCOUNT_ZERODHA, "INE009A01021")
          .ifPresent(existing -> h2.setId(existing.getId()));
      holdingRepo.save(h2);

      CanonicalHolding saved =
          holdingRepo
              .findByUserIdAndBrokerAccountIdAndIsin(USER_ID, ACCOUNT_ZERODHA, "INE009A01021")
              .orElseThrow();
    ...
    void nullIsinHolding() {
      CanonicalHolding h1 =
          buildHolding(null, "NSE:UNKNOWN", new BigDecimal("5"), new BigDecimal("100.00"));
    ...
    private CanonicalHolding buildHolding(
        String isin, String symbol, BigDecimal qty, BigDecimal price) {
  ...
  class PositionUpsertTests {
    ...
    @Test
    @DisplayName(
        "upsert updates existing position by (userId, brokerAccountId, symbol, instrumentType)")
    void upsertUpdatesExistingPosition() {
      ...
      positionRepo
          .findByUserIdAndBrokerAccountIdAndSymbolAndInstrumentType(
  ...
  class MfHoldingUpsertTests {
    ...
    void upsertUpdatesNavAndUnits() {
      ...
      mfHoldingRepo
          .findByUserIdAndBrokerAccountIdAndIsin(USER_ID, ACCOUNT_ZERODHA, "INF090I01EN2")
    ...
    void mfIsinValidation() {
      ...
      var found =
          mfHoldingRepo.findByUserIdAndBrokerAccountIdAndIsin(
              USER_ID, ACCOUNT_ZERODHA, "INF090I01EN2");
      assertThat(found).isPresent();
  ...
  class FundsUpsertTests {
    ...
    void upsertUpdatesAvailableCash() {
      ...
      fundsRepo
          .findByUserIdAndBrokerAccountId(USER_ID, ACCOUNT_ZERODHA)
  ...
  class FullSyncSimulation {
    ...
    void twoConsecutiveSyncsProduceCorrectState() {
      ...
      List<CanonicalHolding> sync1Holdings =
          List.of(
              buildHolding(
                  "INE002A01018", "NSE:RELIANCE", new BigDecimal("10"), new BigDecimal("2500")),
              buildHolding(
                  "INE009A01021", "NSE:INFY", new BigDecimal("20"), new BigDecimal("1500")),
              buildHolding(
                  "INE467B01029", "NSE:TATA STEEL", new BigDecimal("50"), new BigDecimal("120")));
      CanonicalPosition sync1Pos =
          CanonicalPosition.builder()
              .userId(USER_ID)
              .brokerAccountId(ACCOUNT_ZERODHA)
              .brokerType(Broker.ZERODHA)
              .symbol("NSE:RELIANCE")
              ...
              .positionType(PositionType.LONG)
              .quantity(new BigDecimal("5"))
              .avgBuyPrice(new BigDecimal("2510"))
              .lastPrice(new BigDecimal("2550"))
              .lastSyncedAt(Instant.now())
              .build();
      CanonicalFunds sync1Funds =
          CanonicalFunds.builder()
              .userId(USER_ID)
              .brokerAccountId(ACCOUNT_ZERODHA)
              .brokerType(Broker.ZERODHA)
              .availableCash(new BigDecimal("100000"))
              .usedMargin(BigDecimal.ZERO)
              .totalMargin(new BigDecimal("100000"))
              .lastSyncedAt(Instant.now())
              .build();
      ...
      List<CanonicalHolding> sync2Holdings =
          List.of(
              buildHolding(
                  "INE002A01018", "NSE:RELIANCE", new BigDecimal("15"), new BigDecimal("2480")),
              buildHolding(
                  "INE009A01021", "NSE:INFY", new BigDecimal("20"), new BigDecimal("1500")),
              buildHolding(
                  "INE467B01029", "NSE:TATA STEEL", new BigDecimal("50"), new BigDecimal("120")));
      CanonicalPosition sync2Pos =
          CanonicalPosition.builder()
              .userId(USER_ID)
              .brokerAccountId(ACCOUNT_ZERODHA)
              .brokerType(Broker.ZERODHA)
              ...
              .avgBuyPrice(new BigDecimal("2510"))
              .lastPrice(new BigDecimal("2560"))
              .realizedPnL(new BigDecimal("250"))
              .lastSyncedAt(Instant.now())
              .build();
      CanonicalFunds sync2Funds =
          CanonicalFunds.builder()
              .userId(USER_ID)
              .brokerAccountId(ACCOUNT_ZERODHA)
              .brokerType(Broker.ZERODHA)
              .availableCash(new BigDecimal("112750"))
              .usedMargin(BigDecimal.ZERO)
              .totalMargin(new BigDecimal("112750"))
              .lastSyncedAt(Instant.now())
              .build();
    ...
    private void persistSync(
        List<CanonicalHolding> holdings, List<CanonicalPosition> positions, CanonicalFunds funds) {
      for (CanonicalHolding h : holdings) {
        h.setUserId(USER_ID);
        holdingRepo
            .findByUserIdAndBrokerAccountIdAndIsin(USER_ID, h.getBrokerAccountId(), h.getIsin())
      ...
      fundsRepo
          .findByUserIdAndBrokerAccountId(USER_ID, funds.getBrokerAccountId())
