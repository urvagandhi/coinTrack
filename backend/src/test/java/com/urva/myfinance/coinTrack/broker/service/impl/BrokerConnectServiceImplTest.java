...
class BrokerConnectServiceImplTest {
  ...
  void handleCallback_nullToken_throws() {
    assertThrows(
        BrokerException.class, () -> service.handleCallback(USER_ID, Broker.ZERODHA, null));
  }
  ...
  void handleCallback_zerodhaMissingCreds_throws() {
    BrokerAccount account = BrokerAccount.builder().userId(USER_ID).broker(Broker.ZERODHA).build();
    when(accountRepository.findByUserIdAndBroker(USER_ID, Broker.ZERODHA))
        .thenReturn(Optional.of(account));

    assertThrows(
        BrokerException.class, () -> service.handleCallback(USER_ID, Broker.ZERODHA, "reqToken"));
  }
  ...
  void handleCallback_zerodhaSuccess() {
    ...
    when(zerodhaLiveDataService.exchangeToken("reqToken", "apiKey", "encSecret"))
  ...
  void handleCallback_upstoxMissingCreds_throws() {
    BrokerAccount account = BrokerAccount.builder().userId(USER_ID).broker(Broker.UPSTOX).build();
    when(accountRepository.findByUserIdAndBroker(USER_ID, Broker.UPSTOX))
        .thenReturn(Optional.of(account));

    assertThrows(
        BrokerException.class, () -> service.handleCallback(USER_ID, Broker.UPSTOX, "reqToken"));
  }
  ...
  void handleCallback_upstoxSuccess() {
    ...
    when(upstoxBrokerAdapter.authenticate(any()))
        .thenReturn(CompletableFuture.completedFuture(session));
