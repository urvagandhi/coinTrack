...
class AuthControllerTest {
  ...
  void login_authException_returns401() {
    ...
    when(authService.authenticate("testuser", "Pass123!"))
        .thenThrow(
            new AuthenticationException("Too many failed attempts. Try again in 15 minute(s)."));
  ...
  void refresh_validToken_returns200() {
    ...
    when(jwtService.validateAndRotateRefreshToken(
            eq("valid-refresh"), eq(sampleUser), any(), any()))
  ...
  void logout_validToken_returns200() {
    ...
    when(jwtService.extractExpiration("my-token"))
  ...
  void oauth2Google_success_returns200() {
    ...
    when(httpRequest.getHeader("User-Agent")).thenReturn("Mozilla/5.0");
    when(httpRequest.getRemoteAddr()).thenReturn("127.0.0.1");
    when(authService.authenticateGoogle(
            eq("auth-code"), eq("http://localhost/callback"), any(), any()))
