...
class JWTServiceTest {
  ...
  void setUp() {
    jwtService =
        new JWTService(
            "a]valid32charSecretKeyForJwtSigning1!",
            refreshTokenRepository,
            invalidatedTokenRepository);
    sampleUser = User.builder().id("u1").username("testuser").email("test@example.com").build();
  }
  ...
  void generateRefreshToken_saves() {
    ...
    verify(refreshTokenRepository)
        .save(
            argThat(
                rt -> "u1".equals(rt.getUserId()) && rt.getTokenHash() != null && !rt.isRevoked()));
  }
  ...
  void generateRefreshToken_truncatesDevice() {
    ...
    verify(refreshTokenRepository)
        .save(argThat(rt -> rt.getDeviceInfo() != null && rt.getDeviceInfo().length() <= 200));
  }
