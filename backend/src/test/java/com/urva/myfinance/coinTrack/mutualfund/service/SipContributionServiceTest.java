...
class SipContributionServiceTest {
  ...
  @Mock
  private com.urva.myfinance.coinTrack.common.service.TransactionSequenceService
      transactionSequenceService;

  @Mock private MfNavService mfNavService;
  @Mock private com.urva.myfinance.coinTrack.config.StatutoryChargesConfig mfChargesConfig;
  @Mock private SettlementDateCalculator settlementDateCalculator;
  @Mock private RedemptionTransactionService redemptionTransactionService;
  ...
  void getContributions_withSchemeId() {
    when(repository.findByUserIdAndSchemeId(USER_ID, SCHEME_ID))
  ...
  void createContribution_autoCalculatesUnitsWhenNull() {
    ...
    assertEquals(
        ...
        result.getStatus());
    verify(mfNavService)
        .fetchNavForDate(sampleScheme.getAmfiCode(), sampleContribution.getContributionDate());
    verify(redemptionTransactionService)
        .recalculateRedemptionsAfterDate(
            USER_ID, SCHEME_ID, sampleContribution.getContributionDate());
  }
  ...
  void createContribution_schemeNotFound() {
    ...
    assertThrows(
        RuntimeException.class, () -> service.createContribution(USER_ID, sampleContribution));
  }
  ...
  void backfillMandate_weekendsAndHolidays() {
    ...
    when(mfNavService.fetchNavForDate(sampleScheme.getAmfiCode(), app1))
        .thenReturn(new BigDecimal("100"));
    when(mfNavService.fetchNavForDate(sampleScheme.getAmfiCode(), app2))
    ...
    org.mockito.ArgumentCaptor<List<SipContribution>> captor =
