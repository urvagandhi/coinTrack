...
class EpfAnnualCreditSchedulerTest {
  ...
  @Test
  @DisplayName(
      "2. Successfully calculates, saves transaction, and recalculates ledger when interest not credited yet")
  void testSuccessfullyCreditsInterest() {
    when(mongoTemplate.exists(any(Query.class), eq(EpfTransaction.class))).thenReturn(false);

    EpfInterestAccrualResult accrualResult =
        new EpfInterestAccrualResult(new BigDecimal("8250.00"), new BigDecimal("1650.00"));
