...
class UserAuthenticationServiceTest {
  ...
  @Test
  @DisplayName(
      "authenticate: valid user with TOTP enabled+verified → returns tempToken for TOTP login")
  ...
  void authenticate_lockedAccount_throwsException() {
    ...
    assertThrows(
        AuthenticationException.class, () -> authService.authenticate("testuser", "Pass123!"));
  }
  ...
  void authenticateGoogle_newUser_returnsPendingProfile() {
    when(googleOAuthService.exchangeCodeForIdToken("code", "redirect")).thenReturn("idToken");
    Map<String, Object> googleUser =
        Map.of("email", "new@gmail.com", "sub", "goog456", "email_verified", true, "name", "New");
    when(googleOAuthService.verifyIdToken("idToken")).thenReturn(googleUser);
    ...
    PendingRegistration pending =
        PendingRegistration.builder()
            ...
            .email("new@gmail.com")
            .build();
    when(userService.upsertPendingGoogleRegistration("goog456", "new@gmail.com", "New"))
        .thenReturn(pending);
  ...
  void authenticateGoogle_emailCollision_verifiedLinks() {
    when(googleOAuthService.exchangeCodeForIdToken("code", "redirect")).thenReturn("idToken");
    Map<String, Object> googleUser =
        Map.of("email", "test@example.com", "sub", "goog789", "email_verified", true, "name", "G");
    when(googleOAuthService.verifyIdToken("idToken")).thenReturn(googleUser);
  ...
  void authenticateGoogle_emailCollision_unverified_throws() {
    when(googleOAuthService.exchangeCodeForIdToken("code", "redirect")).thenReturn("idToken");
    Map<String, Object> googleUser =
        Map.of("email", "test@example.com", "sub", "goog999", "email_verified", false, "name", "G");
    when(googleOAuthService.verifyIdToken("idToken")).thenReturn(googleUser);
  ...
  void completeGoogleProfile_valid_returnsSetupToken() {
    ...
    PendingRegistration pending =
        PendingRegistration.builder().googleId("google123").email("g@gmail.com").build();
  ...
  void completeGoogleProfile_passwordMismatch_throws() {
    ...
    assertThrows(RuntimeException.class, () -> authService.completeGoogleProfile(req, "d", "ip"));
  }
