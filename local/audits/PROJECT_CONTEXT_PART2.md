...
> **REVISION 2026-09-13 — Bank-aware editable premature-withdrawal penalty + withdrawal-record editing**: (1) **Bank-aware default penalty** — new `fixeddeposit/util/BankPenaltyResolver.java` derives the penalty rate by place/type across all ~1,511 SBI bank codes plus generic banks (Kotak & Yes tenure-tiered by held days, ICICI <1yr/≥1yr, HDFC & majors 1.0, co-op 0.5, small-finance/RRB 1.0, payments bank 0, Post Office 2.0, NBFC Bajaj 2.0, catch-all name containing "bank" → 1.0, empty/no-match → amount-tier 0.5/1.0); a per-request `penaltyRateOverride` is clamped to `[0, interestRate]`. (2) **`penaltyRateApplied` persisted** on the FD doc and used on reconstruction (legacy rows reverse-compute `interestRate − effectiveRateApplied`), so the applied rate can no longer drift from what the UI showed. (3) **Editable withdrawal records** — `PUT /{id}/withdraw` (only valid for `isPrematurelyWithdrawn` FDs; recomputes + persists economics with optional date/penalty input) and `PUT /{id}/withdraw/preview` (dry run, no persist) added to the controller + service; `WithdrawDialog` gains an edit mode (prefills `withdrawalDate` + stored penalty, "Edit Withdrawal"/"Update Withdrawal"); `WithdrawnFdDialog` gains `onEditWithdrawal` + `onDelete` so a prematurely-withdrawn FD can be edited or deleted directly without opening Edit FD. Frontend resolver mirror in `frontend/src/lib/bankPenalty.js`; `api.js` adds `updateWithdraw`/`updateWithdrawPreview`. Tests: PrematureWithdrawalTest **18/18** (new: co-op default, Kotak tenure tiers, override clamp, persisted `penaltyRateApplied`, resolver-category matrix, update-rejected-on-live-FD, update-recomputes-and-persists, update-preview-no-persist), FdMathTest **14/14**, FixedDepositServiceTest **13/13** — BUILD SUCCESS. Module README bumped to **v1.4.0**; backend/README + root README + Part 1 synced.
